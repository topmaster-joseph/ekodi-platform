(() => {
  ...
  function normalize(data={}){
    const cf=data.cloudflare||{}; const gh=data.github||{}; const supabaseProjects=configuredSupabase();
    return {
  ...
  async function load(){const auth=token();let data={};if(auth){try{const response=await fetch(`${API}/api/control/secrets/providers`,{headers:{authorization:`Bearer ${auth}`},cache:'no-store'});if(response.ok)data=await response.json()}catch(error){console.warn('[EKODI Provider Control] inventory fetch failed',error)}}inventory=normalize(data);ensureSelection();renderAll();window.dispatchEvent(new CustomEvent('ekodi-provider-inventory',{detail:snapshot()}));return snapshot()}
