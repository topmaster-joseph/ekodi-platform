...
const REFERENCE_MENU=Object.freeze([
  ...
  {name:'맵슐랭치킨',description:'마요소스와 청양고추의 시그니처',reference_price:23000,image_url:'https://www.ejadam.co.kr/data/editor/2006/741060666f533def13d4e5c0f5c759c5_1590999938_5873.jpg'},
  ...
  {name:'허니팝치킨',description:'허니갈릭과 크런치의 달콤고소함',reference_price:23000,image_url:'https://www.ejadam.co.kr/data/editor/2412/f5d18b0bb0f47150673ee09a0f30de64_1733105087_7106.jpg'},
...
function menuCard(item,channels){
  ...
  const image=safeHttps(listings.find(row=>row.image_url)?.image_url)||safeHttps(item.image_url);
  const verifiedBase=item.base_price!=null;
  const base=verifiedBase?item.base_price:item.reference_price;
  const source=verifiedBase||listings.some(row=>row.price!=null||row.order_url||row.image_url)?'목포대점 확인':'공개 참고가';
  ...
  return `<article class="jd-menu-card"><div class="jd-menu-photo">${image?`<img src="${e(image)}" alt="${e(item.name)}" loading="lazy" referrerpolicy="no-referrer">`:'<span class="jd-food-fallback" aria-hidden="true">🍗</span>'}</div><div class="jd-menu-body"><small>${e(source)}</small><h3>${e(item.name)}</h3><p>${e(item.description||'자담치킨 메뉴')}</p>${base!=null?`<strong class="jd-price">${e(won(base))}</strong>`:''}${platform?`<div class="jd-app-prices">${platform}</div>`:'<div class="jd-app-wait">앱별 목포대점 가격 연결 대기</div>'}</div></article>`;
}
...
export async function renderJadamStorefrontPage(request,env,resolved,slug='jadam'){
  ...
  return new Response(`<!doctype html><html lang="ko" data-store-page="jadam"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="description" content="${e(description)}"><meta name="robots" content="index,follow"><title>${e(title)} | 메뉴 · 가격 · 배달주문</title><link rel="stylesheet" href="/_ekodi/space/jadam-storefront.css?v=20260909-brand-v1"></head><body><main class="jd-page">
