...
const uniq = (value: unknown, max = 12) => [
  ...new Set((Array.isArray(value) ? value : []).map((v) => clip(v, 60)).filter(Boolean)),
].slice(0, max);
const validIntent = (value: unknown) => {
  const intent = String(value || "");
  return INTENTS.has(intent) ? intent : "";
};
...
function overlap(a: string[] = [], b: string[] = []) {
  ...
  return (a || []).filter((v) => right.has(String(v).toLowerCase()));
}
function scoreCandidate(meProfile: any, meConnect: any, profile: any, connect: any) {
  ...
  const sameRegion = meProfile?.region && profile?.region && meProfile.region === profile.region;
...
async function blockedPair(me: string, other: string) {
  const { data } = await admin.from("community_connect_blocks")
...
async function eligibleTarget(targetUserId: string, intent: string) {
  const [{ data: connect }, { data: profile }] = await Promise.all([
    admin.from("community_connect_profiles").select("user_id,intents,discoverable,marriage_enabled").eq("user_id", targetUserId).maybeSingle(),
  ...
  if (intent === "marriage" && !connect.marriage_enabled) return false;
  return true;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (req.method === "GET" && path === "/recommendations") {
      ...
      let query = admin.from("community_connect_profiles")
        .select("user_id,intents,relationship_values,life_priorities,conversation_style,discoverable,marriage_enabled,updated_at")
        .neq("user_id", userId)
        .eq("discoverable", true)
        .contains("intents", [intent])
        .limit(120);
      if (intent === "marriage") query = query.eq("marriage_enabled", true);
      ...
      const ranked = (connectCandidates || []).flatMap((connect) => {
        ...
        if (!profile) return [];
        const match = scoreCandidate(meProfile, meConnect, profile, connect);
        return [{
          ...
          shared_values: match.sharedValues,
          shared_priorities: match.sharedPriorities,
    ...
    if (req.method === "POST" && path === "/interest") {
      ...
      const { data: me } = await admin.from("community_connect_profiles").select("intents,discoverable,marriage_enabled").eq("user_id", userId).maybeSingle();
      if (!me?.discoverable || !(me.intents || []).includes(intent) || (intent === "marriage" && !me.marriage_enabled)) return json(req, { error: "intent_not_enabled" }, 409);
      ...
      const { error } = await admin.from("community_connect_actions").upsert({
        actor_user_id: userId, target_user_id: targetUserId, intent, action: "interested", updated_at: now,
      }, { onConflict: "actor_user_id,target_user_id,intent" });
      if (error) throw error;
      const { data: reciprocal } = await admin.from("community_connect_actions")
        .select("id").eq("actor_user_id", targetUserId).eq("target_user_id", userId).eq("intent", intent).eq("action", "interested").maybeSingle();
      let match = null;
      if (reciprocal) {
        ...
        const { data: saved, error: matchError } = await admin.from("community_connect_matches").upsert({
          user_a_id: userA, user_b_id: userB, intent, status: "active", closed_at: null,
    ...
    if (req.method === "POST" && path === "/pass") {
      ...
      if (!intent || !/^[0-9a-f-]{36}$/i.test(targetUserId) || targetUserId === userId) return json(req, { error: "invalid_target" }, 400);
      const { error } = await admin.from("community_connect_actions").upsert({
        actor_user_id: userId, target_user_id: targetUserId, intent, action: "pass", updated_at: new Date().toISOString(),
      }, { onConflict: "actor_user_id,target_user_id,intent" });
      if (error) throw error;
    ...
    if (req.method === "GET" && path === "/matches") {
      const { data: matches, error } = await admin.from("community_connect_matches")
        ...
        .limit(50);
      if (error) throw error;
      const otherIds = [...new Set((matches || []).map((m) => m.user_a_id === userId ? m.user_b_id : m.user_a_id))];
      const { data: profiles } = otherIds.length ? await admin.from("community_profiles")
        .select("user_id,display_name,bio,region,interests").in("user_id", otherIds) : { data: [] as any[] };
      const profileMap = new Map((profiles || []).map((p) => [p.user_id, p]));
      return json(req, { matches: (matches || []).map((m) => {
        const otherId = m.user_a_id === userId ? m.user_b_id : m.user_a_id;
        const profile = profileMap.get(otherId) || {};
        return { id: m.id, intent: m.intent, status: m.status, created_at: m.created_at, person: {
          user_id: otherId, display_name: profile.display_name || "EKODI 회원", bio: profile.bio || "", region: profile.region || "", interests: profile.interests || [],
    ...
    if (req.method === "POST" && path === "/block") {
      ...
      const { error } = await admin.from("community_connect_blocks").upsert({
        blocker_user_id: userId, blocked_user_id: targetUserId, reason: clip(body.reason, 300),
      }, { onConflict: "blocker_user_id,blocked_user_id" });
      if (error) throw error;
      await admin.from("community_connect_matches").update({ status: "closed", closed_at: new Date().toISOString() })
        .or(`and(user_a_id.eq.${userId},user_b_id.eq.${targetUserId}),and(user_a_id.eq.${targetUserId},user_b_id.eq.${userId})`).eq("status", "active");
    ...
    if (req.method === "POST" && path === "/report") {
      ...
      if (error) throw error;
      if (body.block !== false) {
        await admin.from("community_connect_blocks").upsert({ blocker_user_id: userId, blocked_user_id: targetUserId, reason: `report:${category}` }, { onConflict: "blocker_user_id,blocked_user_id" });
      }
      await writeActivity(userId, "connect.report.created", { category });
      return json(req, { report, blocked: body.block !== false }, 201);
    }
