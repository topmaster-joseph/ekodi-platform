...
async function membership(userId:string){
  ...
  const {data:plan,error:planError}=await admin.from("author_plan_catalog").select("plan_code,display_name,is_paid,monthly_ai_units,max_projects,features").eq("plan_code",planCode).eq("active",true).maybeSingle();if(planError)throw planError;
  const cycleStart=new Date();cycleStart.setUTCDate(1);cycleStart.setUTCHours(0,0,0,0);const cycle=cycleStart.toISOString().slice(0,10);
  ...
  const monthly=Number(plan?.monthly_ai_units||0);const used=Number(quota?.units_reserved||0);const paidActive=Boolean(plan?.is_paid&&row?.status==="active"&&row?.billable_ai_enabled&&row?.paid_until&&new Date(row.paid_until).getTime()>Date.now());
  return {plan:planCode,display_name:String(plan?.display_name||"FREE"),status:String(row?.status||"active"),is_paid:Boolean(plan?.is_paid),paid_ai_active:paidActive,billable_ai_enabled:Boolean(row?.billable_ai_enabled),paid_until:row?.paid_until||null,billing_provider:row?.billing_provider||null,monthly_ai_units:monthly,used_ai_units:used,remaining_ai_units:paidActive?Math.max(monthly-used,0):0,request_count:Number(quota?.request_count||0),max_projects:plan?.max_projects??null,features:plan?.features||{}};
}
...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(req.method==="POST"&&path==="/handoff"){
      ...
      const email=String(auth.user.email||"").trim().toLowerCase();if(!email)return json(req,{error:"email_required"},400);
