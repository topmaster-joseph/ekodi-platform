...
function cors(origin){
  const h={'access-control-allow-headers':'authorization,content-type,apikey','access-control-allow-methods':'GET,OPTIONS','access-control-max-age':'86400','vary':'Origin'};
...
Deno.serve(async req=>{
  ...
  let upstream;
  try{
    upstream=await rpc('church_member_attendance_summary',{
    ...
    return json({error:String(error?.message||error)},503,origin);
  }
