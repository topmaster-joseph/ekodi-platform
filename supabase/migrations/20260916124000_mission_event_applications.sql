...
values (
  '260925-chuseok-open-table',
  'ekodimission',
  '2026 에코디 추석 열린식탁',
  '2026-09-25 16:00:00+09',
...
begin
  ...
  if coalesce(p_event_key,'') !~ '^[0-9]{6}-[a-z0-9][a-z0-9-]{2,79}$' then
    raise exception 'INVALID_EVENT';
  end if;

  select * into v_event from public.mission_events where event_key=p_event_key;
  if not found or not v_event.applications_open or v_event.status='closed' then
    raise exception 'APPLICATION_CLOSED';
  end if;

  if length(trim(coalesce(p_name,''))) < 1 or length(trim(coalesce(p_name,''))) > 80 then
  ...
  if length(trim(coalesce(p_email,''))) > 254 then
    raise exception 'INVALID_EMAIL';
  end if;
  if length(trim(coalesce(p_email,''))) > 0 and trim(p_email) !~ '^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then
    raise exception 'INVALID_EMAIL';
  end if;
  ...
  insert into public.mission_event_applications(
    ...
    photo_consent,privacy_consent,status,submitted_at,updated_at
  ) values (
    p_event_key,
    left(trim(p_name),80),
    left(trim(p_phone),40),
    v_phone,
    left(trim(coalesce(p_email,'')),254),
    p_party_size,
    left(trim(coalesce(p_language,'ko')),24),
    left(trim(coalesce(p_dietary,'')),500),
  ...
  on conflict (event_key,phone_normalized) do update set
    name=excluded.name,
