...
Local account-node scheduling remains Cloud First. When local execution is justified, each node reports bounded CPU/memory telemetry and hardware eligibility; the control plane assigns work only to eligible non-portable nodes, prefers the lowest combined machine/concurrency load, distributes queued jobs across available nodes, and re-leases expired work without weakening branch isolation or production gates. Unknown hardware fails closed until a current agent heartbeat identifies it as eligible.
