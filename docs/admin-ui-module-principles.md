...
- 관리자 내비게이션은 **역할별 2단 구조**를 기본으로 한다.
- 플랫폼 최고관리자 `/admin`은 **플랫폼 전체현황 / 사이트·브랜드 / 사용자·관리자·권한 / 서비스·AI / 콘텐츠·행사·소통 / 운영·배포·장애 / 설정·보안·감사** 7개 통제영역을 사용한다.
- 최고관리자는 선택한 통제영역 아래에 핵심 직접업무 링크를 펼쳐 한 번에 진입하며, 필요 시 좌측 메뉴 독립 세로 스크롤을 허용한다.
- 중간관리자는 해당 사이트·조직·Workspace 고유 `/.../admin`에서 현황·핵심 운영·콘텐츠/고객·정산/보고·사이트/권한처럼 실제 담당업무 중심 메뉴를 사용한다.
- 하위관리자·현장운영자는 오늘 할 일과 고빈도 현장업무를 앞세우고 전역 설정·배포·보안정책·관리자 계정 메뉴는 노출하지 않는다.
- 상단 세부메뉴는 최고관리자 좌측 직접업무와 동기화하거나, 사이트별 작업공간에서 보조 내비게이션으로만 사용한다.
...
- 작업영역은 밝은 배경과 고대비 텍스트를 기본으로 하며, 카드·폼·표의 배경과 경계를 일관되게 유지한다.
- 상태·오류·성공 메시지는 색상만으로 구분하지 않고 문구와 경계를 함께 제공한다.
- 입력 필드와 버튼은 최소 높이와 포커스 상태를 공통화한다.
- 표와 대용량 데이터는 실제 데이터 영역에서만 스크롤하고, 페이지 전체에 중첩 스크롤을 만들지 않는다.
- 사용자에게 보이는 관리자 기본 문구는 한국어를 우선한다. 다국어 게시 관리 기능과 관리자 UI 언어 전환은 별개다.

## 7. 사용자 Shell과 관리자 Shell의 분리
...
1. 좌측 상단 중복 헤더 제거
2. 역할별 좌측 메뉴 고정·최고관리자 필요 시 독립스크롤
...
공통 Admin Shell은 신규 화면뿐 아니라 기존 관리자 화면도 다음 기준으로 런타임 정규화한다.

- `main`, 레거시 `.oa-main` 등 실제 작업영역을 관리자 작업영역으로 식별한다.
- `.heading`, `.hero`, `.intro`, `.oa-head` 등 페이지 제목 영역을 공통 heading 계약으로 정규화한다.
- 상단 세부메뉴가 있으면 가로 스크롤 가능한 단일 행으로 유지해 텍스트 가림을 방지한다.
- 입력·버튼·표·카드의 최소 높이, 포커스 표시, 대비, 경계와 글자 크기를 공통 기준으로 보완한다.
- 모바일에서는 작업영역의 강제 `100dvh` 내부 스크롤을 해제하여 페이지·메뉴·본문이 이중 스크롤되지 않게 한다.
- 1차 고정 메뉴가 화면 높이를 넘어 접근 불가능해질 경우 숨김보다 접근성을 우선하여 임시 스크롤 fallback을 사용하고 `data-ekodi-admin-nav-overflow="true"`로 진단 가능하게 표시한다.
- `window.EKODIAdminUIShell.audit()`은 sidebar, main, heading, subnav, footer, 메뉴 수, overflow fallback 상태를 반환해 운영 점검에서 사용할 수 있게 한다.

이 정규화는 서비스별 기능·권한·데이터 모델을 바꾸지 않으며, 관리자 화면의 가독성·직관성·접근성만 공통화한다.
