...
- 관리자 Shell에는 UI 언어 전환 선택기를 두지 않는다. `[data-ekodi-language-control]` 및 레거시 언어 선택기는 관리자 surface에서 제거한다.
- 공개 사이트의 다국어 번역·게시 상태를 관리하는 관리자 기능은 운영 기능으로 유지하되, 관리자 화면 자체의 언어 선택과는 분리한다.
