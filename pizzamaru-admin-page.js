const PIZZAMARU_STORE_ID='6b1b6ae0-8641-4ee1-8e6a-7cb6019fab27';
...
function clientMain(){
  const STORE_ID='6b1b6ae0-8641-4ee1-8e6a-7cb6019fab27';
  const SUPABASE_URL='https://renzehysxirjilvdxacv.supabase.co';
  const SUPABASE_KEY='sb_publishable_0QjB0WzZbjrd-FJ5D5cR7A_xUkXyOY_';
  const API='https://api.ekodi.kr';
  const SESSION_KEY='ekodi-pizzamaru-admin-session';
  ...
  const META={
    ...
    marketing:['마케팅','피자마루 전용 Marketing AI 운영공간으로 연결합니다.'],
  ...
  function setup(){
    $('pageTitle').textContent=page[0];$('pageCopy').textContent=page[1];document.title=`${page[0]} · 피자마루 목포대점`;
    for(const [key,label] of NAV){const a=document.createElement('a');a.href=key==='overview'?'/pizzamaru/admin':`/pizzamaru/admin/${key}`;a.textContent=label;if(key===section)a.classList.add('active');$('adminNav').append(a)}
  }
  ...
  function loginPanel(message='피자마루 목포대점 운영 권한으로 로그인해야 합니다.'){
    $('summaryCards').innerHTML=[card('운영공간','피자마루 목포대점','store-scoped'),card('데이터 경계','점포 전용','다른 매장과 분리')].join('');
  ...
  function marketingPanel(){$('mainPanel').innerHTML=`<h2>피자마루 Marketing AI</h2><p class="panel-lead">다른 점포와 데이터·권한이 분리된 피자마루 전용 마케팅 운영공간입니다.</p><div class="actions"><a class="button primary" href="/pizzamaru/marketing">전용 Marketing AI 열기</a></div>`}
  ...
  function render(){summary();({overview,menu:menuPanel,orders:ordersPanel,customers:customerPanel,reviews:reviewsPanel,sales:salesPanel,inventory:inventoryPanel,marketing:marketingPanel,work:workPanel,finance:financePanel,connections:connectionsPanel}[section]||overview)();setState('피자마루 점포 운영','live')}
  async function loadData(){
    setState('실데이터 확인 중');
    const [snapshot,menu,connector]=await Promise.all([
      rpc('business_os_store_admin_snapshot',{p_workspace_key:'pizzamaru'}),
      rpc('store_operating_space_snapshot',{p_operating_slug:'pizzamaru'}),
  ...
  async function boot(){
    ...
    try{await loadData()}catch(e){if(e.status===401)return loginPanel();if(e.status===403)return loginPanel('이 계정에는 피자마루 목포대점 운영 권한이 없습니다.');$('summaryCards').innerHTML=[card('운영공간','피자마루 목포대점','데이터 확인 필요')].join('');$('mainPanel').innerHTML=`<div class="empty"><strong>실데이터 연결 확인 필요</strong>${esc(e.message)}</div>`;setState('확인 필요','warn')}
  }
  $('refreshData').onclick=()=>loadData().catch(e=>{setState('확인 필요','warn');$('pageCopy').textContent=e.message});
  $('logout').onclick=()=>{clearSession();location.assign('/pizzamaru/admin')};
  boot();
}

export function isPizzamaruAdminPath(pathname){return /^\/pizzamaru\/admin(?:\/(?:overview|menu|orders|customers|reviews|sales|inventory|marketing|work|finance|connections))?\/?$/i.test(String(pathname||''))}
export function pizzamaruAdminCss(){return new Response(CSS,{headers:{'content-type':'text/css; charset=utf-8','cache-control':'public, max-age=300','x-content-type-options':'nosniff'}})}
export function pizzamaruAdminScript(){return new Response(`(${clientMain.toString()})();`,{headers:{'content-type':'text/javascript; charset=utf-8','cache-control':'no-store','x-content-type-options':'nosniff'}})}
export function pizzamaruAdminPage(){
  const html='<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><title>피자마루 목포대점 관리자</title><link rel="stylesheet" href="/pizzamaru-admin.css"></head><body><header class="topbar"><a class="brand" href="/pizzamaru/admin"><span class="mark">PM</span><span class="brand-copy"><strong>피자마루 목포대점</strong><small>STORE OPERATIONS</small></span></a><div class="top-actions"><a href="/pizzamaru/marketing">마케팅</a><button id="refreshData" type="button">새로고침</button><button id="logout" type="button">로그아웃</button></div></header><div class="layout"><aside class="sidebar"><div class="scope"><small>독립 점포 운영공간</small><strong>피자마루 목포대점</strong><span>PIZZAMARU</span></div><nav id="adminNav"></nav><p class="boundary">다른 점포는 별도 운영공간입니다. 공통 엔진만 공유하고 데이터·권한은 공유하지 않습니다.</p></aside><main><section class="heading"><div><p class="eyebrow">PIZZAMARU STORE ADMIN</p><h1 id="pageTitle">운영 홈</h1><p id="pageCopy">점포 운영 데이터를 확인합니다.</p></div><span class="state" id="pageState">확인 중</span></section><section class="cards" id="summaryCards"></section><section class="panel" id="mainPanel"><div class="empty"><strong>운영 데이터를 불러오고 있습니다.</strong>점포 권한과 실제 연결 상태를 확인합니다.</div></section></main></div><script src="/pizzamaru-admin.js?v=20260906-store-admin" defer></script></body></html>';
  return new Response(html,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store','x-content-type-options':'nosniff','x-frame-options':'DENY','referrer-policy':'strict-origin-when-cross-origin','content-security-policy':"default-src 'self'; style-src 'self'; script-src 'self'; connect-src 'self' https://api.ekodi.kr https://renzehysxirjilvdxacv.supabase.co; frame-ancestors 'none'; base-uri 'self'; form-action 'self'",'x-ekodi-route':'pizzamaru-store-admin','x-ekodi-store-scope':PIZZAMARU_STORE_ID}});
}
