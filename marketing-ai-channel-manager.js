(() => {
  ...
  function youtubeOptions(connections) {
    const rows = connections.filter(row => row.provider === 'youtube' && row.status === 'active');
    if (!rows.length) return '<option value="">YouTube 연결 필요</option>';
    return `<option value="">자동 선택</option>${rows.map(row => `<option value="${Number(row.id)}">${esc(row.display_name)} · ${esc(row.external_id)}</option>`).join('')}`;
  }
  ...
  async function renderManager() {
    ...
    try {
      ...
      const [connectionData,campaignData,youtubeStatus] = await Promise.all([request('/v1/connections'),request('/v1/promotions'),request('/v1/youtube/status')]);
      ...
      const youtubeConnections = connections.filter(row => row.provider === 'youtube' && row.status === 'active').length;
      ...
      const platformAlert = (!platform.metaConfigured || !platform.threadsConfigured || !youtubeStatus.configured) ? `<div class="growth-platform-alert"><b>최초 1회 플랫폼 설정 필요</b><br>사용자별 인증키 입력은 없습니다. 공식 앱 식별정보가 등록된 서비스는 연결 버튼에서 계정 승인만 진행하면 됩니다. YouTube는 Google OAuth 앱 설정 후 최초 한 번 댓글·업로드 권한을 승인하면 이후 refresh token을 암호화 Vault에서 자동 갱신합니다.</div>` : '';
      ...
      view.innerHTML = `<div class="growth-hub" data-growth-hub>
        ...
        <div class="growth-summary"><article><small>무료 게시 연결</small><strong>${activePublish}</strong></article><article><small>YouTube</small><strong>${youtubeConnections}</strong></article><article><small>광고계정</small><strong>${adAccounts}</strong></article><article><small>홍보 캠페인</small><strong>${organic + paid}</strong></article></div>
        <div class="growth-connectors">
          ...
          <section class="growth-card"><header><h4>YouTube 연결</h4><span class="growth-state ${youtubeStatus.configured ? 'active' : 'draft'}">${youtubeStatus.configured ? '준비됨' : '앱 설정 필요'}</span></header><p>Google 공식 승인으로 댓글·업로드 권한을 연결합니다. 장기 refresh token은 서버의 암호화 Vault에만 보관됩니다.</p><button type="button" data-connect="youtube" ${youtubeStatus.configured ? '' : 'disabled'}>Google로 YouTube 연결</button></section>
        ...
        <section class="growth-card"><h4>YouTube 상품 댓글</h4><p>쇼츠 또는 영상에 에코디몰의 실제 상품 링크를 상단댓글 또는 답글로 게시합니다. 외부 도메인과 옛 Mall 서브도메인은 서버에서 거부합니다.</p><form class="growth-form" data-youtube-comment-form><div class="growth-form-grid"><label>YouTube 채널<select name="connectionId">${youtubeOptions(connections)}</select></label><label>영상 ID<input name="videoId" required maxlength="24" placeholder="YouTube 영상 ID"></label><label class="growth-full">상품명<input name="productName" required maxlength="180" placeholder="에코디몰 상품명"></label><label class="growth-full">에코디몰 상품 URL<input name="productUrl" required type="url" placeholder="https://ekodi.kr/mall/..."></label><label class="growth-full">답글 대상 댓글 ID · 선택<input name="parentCommentId" maxlength="180" placeholder="비우면 영상 상단댓글"></label><label class="growth-check growth-full"><input type="checkbox" name="affiliateDisclosure"><span>실제 제휴수수료 대상인 경우에만 고지문 추가</span></label></div><div class="growth-note">허용 링크: <b>https://ekodi.kr/mall</b> 및 그 하위 경로만. 실제 상품 URL을 확인하지 못한 상태에서는 게시하지 않습니다.</div><button type="submit" ${youtubeConnections ? '' : 'disabled'}>YouTube 상품 댓글 게시</button><div class="growth-result"></div></form></section>
      ...
      view.querySelector('[data-youtube-comment-form]')?.addEventListener('submit',async event => {
        event.preventDefault();
        const form = event.currentTarget;
        const button = form.querySelector('button[type="submit"]');
        button.disabled = true;
        setResult(form,'YouTube에 상품 댓글을 게시하는 중입니다.');
        try {
          const connectionId = Number(formValue(form,'connectionId'));
          const body = {videoId:formValue(form,'videoId'),productName:formValue(form,'productName'),productUrl:formValue(form,'productUrl'),parentCommentId:formValue(form,'parentCommentId'),affiliateDisclosure:Boolean(form.querySelector('[name="affiliateDisclosure"]')?.checked)};
          if (Number.isInteger(connectionId) && connectionId > 0) body.connectionId = connectionId;
          const data = await request('/v1/youtube/comments',{method:'POST',body});
          setResult(form,`${data.kind === 'reply' ? '답글' : '상단댓글'} 게시 완료${data.commentId ? ` · ${data.commentId}` : ''}`);
