...
const NEED_ROUTER_TAG='<link rel="stylesheet" href="/need-router.css?v=20260827-need-router-1"><script src="/need-router.js?v=20260827-need-router-1" defer></script>';
...
async function routedMyHome(request,env,route=null){
  ...
  if(!source.includes('/need-router.js'))source=source.replace('</head>',`${NEED_ROUTER_TAG}</head>`);
  const headers=new Headers(asset.headers);
...
export default{
  async fetch(request,env){
    ...
    if(url.pathname==='/health'){
      const cfg=runtimeConfig(env);
      return json(env,{ok:true,service:'ekodi-my',product:'my-ekodi',identity:'person-scoped',creatorPortfolio:true,personalBrandMarketing:true,universalMembership:true,ekodiShell:true,contextModel:'person-space-role',manifestDrivenServices:true,privateWorkspaceRouting:true,privateWorkspacePath:'/w/{workspace_key}/{service}',lifeChannels:true,proactiveUserAi:true,humanGatedOutbound:true,needBasedDiscovery:true,serviceDemandRadar:true,serviceManifestVersion:EKODI_SERVICE_MANIFEST.version,visibleServices:visibleServices().length,privacy:'private-first',dataMode:cfg.dataMode,dataEnabled:cfg.dataEnabled});
    }
