...
function securityHeaders(env={}){
  ...
  return {
    'content-security-policy':`default-src 'self'; script-src 'self' https://cdn.jsdelivr.net; style-src 'self'; img-src 'self' data: blob: https:; worker-src 'self' blob:; connect-src ${connect.join(' ')}; frame-ancestors 'none'; base-uri 'self'; form-action 'self' https://ekodi.kr https://auth.ekodi.kr; object-src 'none'; upgrade-insecure-requests`,
...
export default{
  async fetch(request,env){
    ...
    if(url.pathname==='/presentation')return Response.redirect(new URL('/presentation/',request.url).toString(),307);
