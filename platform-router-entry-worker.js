...
export default {
  async fetch(request,env,ctx){
    ...
    if(host===PUBLIC_HOST&&['GET','HEAD'].includes(request.method)&&(url.pathname==='/connect'||url.pathname==='/connect/')){
      ...
      return new Response(null,{status:302,headers:{location:target.toString(),'cache-control':'no-store','x-content-type-options':'nosniff','x-ekodi-route':'mcp-connect-auth','x-robots-tag':'noindex, nofollow, noarchive'}});
    }
