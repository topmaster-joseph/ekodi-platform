...
const LEGACY_TENANT_READABILITY_ROOTS=new Set(['ekodichurch','ekodimission']);
function legacyTenantReadabilityPath(pathname){const first=String(pathname||'').split('/').filter(Boolean)[0]?.toLowerCase()||'';return LEGACY_TENANT_READABILITY_ROOTS.has(first);}
...
async function routePlatform(request,env,ctx){
    ...
    if(host===PUBLIC_HOST&&['GET','HEAD'].includes(request.method)&&legacyTenantReadabilityPath(url.pathname))return injectEkodiTenantReadability(legacyResponse);
