  ...
  async fetch(request,env,ctx){
    ...
    if(host===PUBLIC_HOST){
      if(request.method==='GET'){
        ...
        if(url.pathname==='/pizzamaru-admin.css')return pizzamaruAdminCss();
        if(url.pathname==='/pizzamaru-admin.js')return pizzamaruAdminScript();
        if(isPizzamaruAdminPath(url.pathname))return pizzamaruAdminPage();
        if(url.pathname==='/yogurt-admin.css')return yogurtAdminCss();
        if(url.pathname==='/yogurt-admin.js')return yogurtAdminScript();
        if(isYogurtAdminPath(url.pathname))return yogurtAdminPage();
