  ...
  async fetch(request,env,ctx){
    ...
    if(host===PUBLIC_HOST){
      ...
      if(request.method==='GET'){
        ...
        if(url.pathname==='/mall/admin'||url.pathname.startsWith('/mall/admin/')){const target=new URL(request.url);target.pathname=url.pathname.replace(/^\/mall(?=\/|$)/i,'/ekodibiz/ekodimall');return new Response(null,{status:308,headers:{location:target.toString(),'cache-control':'no-store','x-content-type-options':'nosniff'}});}
