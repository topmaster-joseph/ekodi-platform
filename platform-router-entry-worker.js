...
async function routePlatform(request,env,ctx){
    ...
    if(host===PUBLIC_HOST&&['GET','HEAD'].includes(request.method)&&(url.pathname==='/pyeonggongmok'||url.pathname.startsWith('/pyeonggongmok/'))){const response=await routePyeonggongmokStatic(request,env);return request.method==='GET'?decorateDiscoveryResponse(response,url.pathname):response;}
