...
const PGM_PREFIX='/pgm';
const PYEONGGONGMOK_LEGACY_PREFIX='/pyeonggongmok';
...
function isPgmPath(pathname){const path=String(pathname||'');return path===PGM_PREFIX||path.startsWith(PGM_PREFIX+'/');}
function isLegacyPyeonggongmokPath(pathname){const path=String(pathname||'');return path===PYEONGGONGMOK_LEGACY_PREFIX||path.startsWith(PYEONGGONGMOK_LEGACY_PREFIX+'/');}
function redirectLegacyPyeonggongmok(request){const source=new URL(request.url);const target=new URL(request.url);target.pathname=source.pathname.replace(/^\/pyeonggongmok(?=\/|$)/,'/pgm');if(target.pathname==='/pgm/')target.pathname='/pgm';return new Response(null,{status:308,headers:{location:target.toString(),'cache-control':'no-store','x-content-type-options':'nosniff','x-ekodi-route':'pgm-canonical'}});}
function pgmAdminPage(request){const source=new URL(request.url);if(!/^\/pgm\/admin\/?$/i.test(source.pathname))return null;if(source.pathname==='/pgm/admin/'){const target=new URL(request.url);target.pathname='/pgm/admin';return new Response(null,{status:308,headers:{location:target.toString(),'cache-control':'no-store','x-content-type-options':'nosniff','x-ekodi-route':'pgm-canonical'}});}const page=workspaceAdminPage();return typeof HTMLRewriter==='function'?injectEkodiShell(page,'space','admin'):page;}
async function routePgmStatic(request,env){if(!env?.ASSETS?.fetch)return new Response('Site assets unavailable',{status:503,headers:{'cache-control':'no-store'}});const source=new URL(request.url);if(source.pathname===PGM_PREFIX+'/'){const target=new URL(request.url);target.pathname=PGM_PREFIX;return new Response(null,{status:308,headers:{location:target.toString(),'cache-control':'no-store','x-content-type-options':'nosniff','x-ekodi-route':'pgm-canonical'}});}const target=new URL(request.url);if(source.pathname===PGM_PREFIX)target.pathname=PGM_PREFIX+'/';const upstream=await env.ASSETS.fetch(new Request(target.toString(),request));const out=new Response(upstream.body,upstream);out.headers.set('x-ekodi-route','pgm-static');out.headers.set('x-content-type-options','nosniff');if(target.pathname.endsWith('.css'))out.headers.set('content-type','text/css; charset=utf-8');else if((out.headers.get('content-type')||'').includes('text/html'))out.headers.set('cache-control','no-store');return out;}
...
async function routePlatform(request,env,ctx){
    ...
    if(host===PUBLIC_HOST&&['GET','HEAD'].includes(request.method)&&isLegacyPyeonggongmokPath(url.pathname))return redirectLegacyPyeonggongmok(request);
    if(host===PUBLIC_HOST&&['GET','HEAD'].includes(request.method)&&isPgmPath(url.pathname)){const admin=pgmAdminPage(request);if(admin)return admin;const response=await routePgmStatic(request,env);return request.method==='GET'?decorateDiscoveryResponse(response,url.pathname):response;}
