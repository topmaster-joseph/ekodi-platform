(() => {
  ...
  function install() {
    ...
    if (!panel) {
      ...
      panel.innerHTML = `
        ...
        <div class="marketing-ai-admin-status" aria-label="MarketingAI 운영 상태">
          <article><small>유료 구독</small><strong id="marketingAiPaid">—</strong><span>PLUS · PRO · AUTO 활성</span></article>
          <article><small>월 반복매출</small><strong id="marketingAiMrr">—</strong><span>활성 월 구독 합계</span></article>
          <article><small>최근 30일 결제</small><strong id="marketingAiCharges">—</strong><span id="marketingAiChargeCount">결제 확인 중</span></article>
          <article><small>플랜 구성</small><strong id="marketingAiPlanMix">—</strong><span>PLUS · PRO · AUTO</span></article>
        </div>
        <div class="marketing-ai-admin-body">
          <section class="marketing-ai-admin-main" aria-labelledby="marketingAiRecentTitle">
            <div class="marketing-ai-section-head">
              <div><span class="marketing-ai-section-kicker">SUBSCRIPTIONS</span><h3 id="marketingAiRecentTitle">최근 MarketingAI 구독</h3></div>
          ...
          <aside class="marketing-ai-admin-side" aria-label="MarketingAI 관리자 요약">
            <section class="marketing-ai-side-card marketing-ai-plan-overview">
              ...
              <div class="marketing-ai-plan-count-grid">
                <div><span>FREE</span><strong id="marketingAiFreeCount">—</strong></div><div><span>PLUS</span><strong id="marketingAiPlusCount">—</strong></div>
                <div><span>PRO</span><strong id="marketingAiProCount">—</strong></div><div><span>AUTO</span><strong id="marketingAiAutoCount">—</strong></div>
              </div>
            ...
            <section class="marketing-ai-side-card marketing-ai-entry-card">
              ...
              <a class="marketing-ai-entry-link" href="${LIVE}" target="_blank" rel="noopener"><span><small>PUBLIC</small><strong>marketing.ekodi.kr</strong></span><b>↗</b></a>
    ...
    function renderSubscriptions(rows = []) {
      ...
      const recent = [...marketing].sort((a, b) => String(b.updated_at || '').localeCompare(String(a.updated_at || ''))).slice(0, 16);
    ...
    function renderCharges(rows = []) {
      ...
      setText('#marketingAiCharges', won(recent.reduce((sum, row) => sum + Number(row.amount || 0), 0)));
    ...
    async function refreshData() {
      ...
      try {
        const [subscriptions, charges] = await Promise.all([api('/api/membership/admin/subscriptions'), api('/api/membership/admin/charges')]);
