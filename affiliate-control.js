...
async function ensureSchema(db) {
  await db.batch([
    db.prepare(`CREATE TABLE IF NOT EXISTS affiliate_providers (
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS affiliate_accounts (
      id TEXT PRIMARY KEY,
    ...
    db.prepare(`CREATE TABLE IF NOT EXISTS affiliate_links (
  ...
  await db.prepare(`INSERT OR IGNORE INTO affiliate_providers
    (provider_key, display_name, provider_kind, connection_mode, enabled, created_at, updated_at)
    VALUES ('coupang_partners', 'Coupang Partners', 'affiliate', 'manual', 1, ?, ?)`)
    .bind(now, now).run();
  await db.prepare(`INSERT OR IGNORE INTO affiliate_accounts
    ...
    VALUES (?, 'coupang_partners', 'internal', 'ekodibiz', '에코디비즈 쿠팡파트너스', 'EKODIBIZ', 'manual_ready', 'manual', '', '', 1, ?, ?)`)
    .bind(DEFAULT_ACCOUNT_ID, now, now).run();
}
...
async function overview(env) {
  const [accounts, links, metrics] = await Promise.all([
    ...
    env.DB.prepare(`SELECT
      ...
      FROM affiliate_links`).first(),
    env.DB.prepare(`SELECT
        COALESCE(SUM(clicks), 0) AS clicks,
        COALESCE(SUM(orders), 0) AS orders,
        COALESCE(SUM(revenue_krw), 0) AS revenue_krw
      FROM affiliate_daily_metrics
      WHERE metric_date >= date('now', '-29 day')`).first(),
  ]);
  return {
    ...
    summary: {
      ...
      clicks30d: Number(metrics?.clicks || 0),
    ...
    capabilities: {
      ...
      automaticDeepLink: false,
      automaticPerformanceSync: false,
      apiStatus: 'provider_contract_not_configured',
    },
  };
}
...
async function handleAccounts(request, env, auth, path) {
  ...
  const now = new Date().toISOString();
  await env.DB.prepare(`UPDATE affiliate_accounts
    SET display_name = ?, default_channel = ?, disclosure_text = ?, enabled = ?, updated_at = ?
    WHERE id = ?`)
    .bind(displayName, defaultChannel, disclosureText, enabled ? 1 : 0, now, match[1]).run();
...
async function handleLinks(request, env, auth, path, url) {
  if (request.method === 'GET' && path === `${PREFIX}/links`) {
    ...
    const rows = await env.DB.prepare(`SELECT * FROM affiliate_links
      ORDER BY id DESC LIMIT ?`).bind(limit).all();
  ...
  if (request.method === 'POST' && path === `${PREFIX}/links`) {
    ...
    const result = await env.DB.prepare(`INSERT INTO affiliate_links
      (account_id, tenant_slug, product_name, destination_url, affiliate_url, channel, campaign_name, status, created_by, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?, ?)`)
      .bind(accountId, tenantSlug, productName, destinationUrl || '', affiliateUrl, channel, campaignName, createdBy, now, now).run();
  ...
  if (archive && request.method === 'POST') {
    ...
    const now = new Date().toISOString();
    await env.DB.prepare("UPDATE affiliate_links SET status = 'archived', updated_at = ? WHERE id = ?")
      .bind(now, Number(archive[1])).run();
...
async function handleMetrics(request, env, auth, path) {
  if (request.method === 'GET' && path === `${PREFIX}/metrics`) {
    const rows = await env.DB.prepare(`SELECT account_id, metric_date, clicks, orders, revenue_krw, source, updated_at
      FROM affiliate_daily_metrics ORDER BY metric_date DESC, id DESC LIMIT 90`).all();
    return json({ metrics: rows.results.map(row => ({
      accountId: row.account_id,
      metricDate: row.metric_date,
      clicks: Number(row.clicks || 0),
      orders: Number(row.orders || 0),
      revenueKrw: Number(row.revenue_krw || 0),
      source: row.source,
      updatedAt: row.updated_at,
  ...
  await env.DB.prepare(`INSERT INTO affiliate_daily_metrics
    (account_id, metric_date, clicks, orders, revenue_krw, source, recorded_by, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, 'manual', ?, ?, ?)
    ON CONFLICT(account_id, metric_date, source) DO UPDATE SET
      clicks = excluded.clicks,
      orders = excluded.orders,
      revenue_krw = excluded.revenue_krw,
      recorded_by = excluded.recorded_by,
      updated_at = excluded.updated_at`)
    .bind(accountId, metricDate, clicks, orders, revenueKrw, recordedBy, now, now).run();
...
export async function handleAffiliateRequest(request, env) {
  ...
  if (request.method === 'GET' && path === `${PREFIX}/overview`) {
    return json(await overview(env), 200, auth.response.headers);
  }
  if (request.method === 'GET' && path === `${PREFIX}/providers`) {
    ...
    return json({ providers: rows.results.map(row => ({
      providerKey: row.provider_key,
      displayName: row.display_name,
