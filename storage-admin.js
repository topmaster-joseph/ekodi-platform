(() => {
  ...
  async function loadDrives(connection,wrap){
    ...
    try{
      ...
      const select=document.createElement('select');select.setAttribute('aria-label',t('Google Drive 선택','Select Google Drive'));
      for(const drive of result.drives||[]){const option=document.createElement('option');option.value=drive.id;option.textContent=`${drive.name}${drive.type==='shared-drive'?t(' · 공유 드라이브',' · Shared Drive'):''}`;if(drive.id===connection.drive_id)option.selected=true;select.append(option);}
      const save=button(t('이 Drive 사용','Use this Drive'),async()=>{try{save.disabled=true;await request(`/connections/${encodeURIComponent(connection.id)}/select`,{method:'POST',body:JSON.stringify({driveId:select.value})});await refresh();}catch(error){showError(error);}finally{save.disabled=false;}});wrap.append(select,save);
    }catch(error){wrap.textContent='';showError(error);}
  }
  async function bootstrap(connection,trigger){if(!confirm(t('선택한 기본 Google Drive에 EKODI 표준 폴더 구조를 생성합니다. 계속할까요?','Create the EKODI standard folder structure in the selected primary Google Drive?')))return;try{trigger.disabled=true;await request(`/connections/${encodeURIComponent(connection.id)}/bootstrap`,{method:'POST'});await refresh();}catch(error){showError(error);}finally{trigger.disabled=false;}}
  ...
  function summary(container){const cards=locale()==='en'?[['Primary source','Google Workspace Drive'],['Secondary storage','Connect another Google account'],['Web deployment files','Replicate to Cloudflare R2 when needed'],['Credentials','AES-GCM encrypted storage']]:[['기본 원본 저장소','Google Workspace Drive'],['보조 저장소','다른 Google 계정 선택 연결'],['웹 배포 파일','Cloudflare R2 필요 시 복제'],['자격증명','AES-GCM 암호화 저장']];const grid=el('div','','storage-summary');for(const [title,copy] of cards){const card=el('div','','storage-card');card.append(el('strong',title),el('span',copy));grid.append(card);}container.append(grid);}
  function renderConnections(container){
    const area=el('div','','storage-connections');if(!(state.connections||[]).length)area.append(el('div',t('연결된 Google Drive 계정이 없습니다. 위 버튼에서 EKODI Workspace 계정을 먼저 연결하세요.','No Google Drive account is connected. Connect the EKODI Workspace account first.'),'storage-banner'));
    for(const connection of state.connections||[]){const card=el('article','','storage-connection'),head=el('div','','storage-connection-head');head.append(el('span',connection.role==='primary'?t('기본 · EKODI','PRIMARY · EKODI'):t('추가 계정','SECONDARY'),'storage-role'),el('span',connection.status==='ready'?t('준비 완료','Ready'):t('설정 중','Configuring'),'storage-state'));card.append(head);card.append(el('h3',connection.display_name||connection.account_email),el('div',connection.account_email,'storage-muted'));const meta=el('div','','storage-meta');meta.append(el('span',`${t('드라이브','Drive')}: ${connection.drive_name||t('선택 전','Not selected')}`),el('span',`${t('상태','Status')}: ${connection.status}`));card.append(meta);const controls=el('div','','storage-control-row');controls.append(button(connection.drive_id?t('Drive 변경','Change Drive'):t('Drive 선택','Select Drive'),()=>loadDrives(connection,controls)));if(connection.role==='primary'&&connection.drive_root_id){const boot=button(connection.status==='ready'?t('폴더 구조 확인/보완','Check/repair folders'):t('EKODI 폴더 구축','Build EKODI folders'),()=>bootstrap(connection,boot),'primary');controls.append(boot);}controls.append(button(t('연결 해제','Disconnect'),()=>disconnect(connection)));card.append(controls);area.append(card);}container.append(area);
  }
  function renderRoutes(container){const box=el('div','','storage-route-box');box.append(el('h3',t('EKODI 기본 아카이브 경로','EKODI default archive paths')));const grid=el('div','','storage-routes');for(const route of state.routes||[]){const row=el('div','','storage-route');row.append(el('span',route.service_key),el('code',`${route.folder_name}${route.folder_id?' ✓':''}`));grid.append(row);}box.append(grid);container.append(box);}
  function paint(){const section=document.querySelector('#storageAdminPanel');if(!section)return;section.textContent='';const head=el('div','','storage-head'),copy=el('div');copy.append(el('div',t('EKODI 저장 아카이브','EKODI CLOUD ARCHIVE'),'storage-kicker'),el('h2',t('저장소','Storage')),el('p',t('원본·문서·영상은 Google Drive에, 웹 서비스용 복제본은 필요할 때 R2에 둡니다.','Keep originals, documents and video in Google Drive, with R2 replicas only when needed for web services.'),'storage-muted'));const actions=el('div','','storage-actions');actions.append(button(t('EKODI 기본 Drive 연결','Connect primary EKODI Drive'),()=>startOAuth('primary'),'primary'),button(t('다른 Google 계정 추가','Add another Google account'),()=>startOAuth('secondary')),button(t('↻ 새로고침','↻ Refresh'),refresh));head.append(copy,actions);section.append(head);const message=el('div',state.configured?t(`기본 허용 조직: ${state.primaryDomains.join(', ')}`,`Allowed primary domains: ${state.primaryDomains.join(', ')}`):t('Google Drive OAuth Secret 설정이 아직 필요합니다.','Google Drive OAuth Secret still needs configuration.'),`storage-banner ${state.configured?'ok':''}`);message.id='storageMessage';section.append(message);summary(section);renderConnections(section);renderRoutes(section);}
