...
async function requestHandoff(request,env,ctx,id){
  const thread=await threadForSubject(env,id,ctx.subject);if(!thread)return json(request,env,{error:'NOT_FOUND'},404);const data=await body(request)||{};const existing=await activeHandoff(env,id);if(existing)return json(request,env,{error:'HANDOFF_ALREADY_OPEN',handoffId:existing.id},409);const now=nowIso();const role=clean(data.targetRole||'manager',80),note=clean(data.note,1000);const inserted=await env.DB.prepare(`INSERT INTO messenger_handoffs(thread_id,requested_by_user_id,target_role,status,note,created_at,updated_at) VALUES(?,?,?,?,?,?,?)`).bind(id,ctx.identity.id,role,'requested',note,now,now).run();const handoffId=Number(inserted.meta?.last_row_id||0);await env.DB.prepare(`UPDATE messenger_threads SET status='waiting_human',updated_at=? WHERE id=?`).bind(now,id).run();await recordEvent(env,id,'human.review_requested','human',ctx.principal.id,{handoffId,manual:true,targetRole:role});return json(request,env,{ok:true,handoffId,status:'requested'},201);
}
