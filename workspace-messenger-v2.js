...
async function ensureAutomaticHandoff(env,threadId,principal,requesterId,triage){
  const existing=await activeHandoff(env,threadId);if(existing)return existing;
  const now=nowIso();const inserted=await env.DB.prepare(`INSERT INTO messenger_handoffs(thread_id,requested_by_user_id,target_role,status,note,created_at,updated_at) VALUES(?,?,?,?,?,?,?)`).bind(threadId,requesterId,'manager','requested',`자동 분류: ${triage.reasons.join(', ')||'human_review'}`,now,now).run();
  const handoffId=Number(inserted.meta?.last_row_id||0);await env.DB.prepare(`UPDATE messenger_threads SET status='waiting_human',updated_at=? WHERE id=?`).bind(now,threadId).run();await recordEvent(env,threadId,'human.review_requested','system','ekodi-triage',{handoffId,priority:triage.priority,reasons:triage.reasons,principalId:principal.id});return {id:handoffId,status:'requested'};
}
...
async function listThreads(request,env,ctx){
  ...
  return json(request,env,{subject:ctx.subject,threads:result.results||[]});
}
...
async function readThread(request,env,ctx,id){
  ...
  return json(request,env,{subject:ctx.subject,thread,messages:(messages.results||[]).map(row=>({...row,metadata:safeParse(row.metadata_json,{})})),handoffs:handoffs.results||[],channels:channels.results||[],processing:outbox.results||[]});
}
