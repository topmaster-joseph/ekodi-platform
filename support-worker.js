...
import { assessNeedState } from './support/need-sensing.js';
import { officialSourceStatus, fetchBizinfoNotices, fetchVerifiedInstitutionNotices, fetchCombinedOpportunityNotices } from './support/sources.js';

export const BENEFIT_RADAR_CAPABILITY=Object.freeze({id:'support.benefit-radar',generation:10,contract:'ekodi.benefit-radar.v1',actionTier:'assist',execution:'information-recommendation-and-prep-only',highImpact:'human-gated',consent:'consent-first',ordinaryUserMode:'information-only',commercialSubject:'ekodibiz'});

const SPECIALIST_PATHS=new Set(OPPORTUNITY_SERVICES.map(service=>service.path));
const FEATURE_PREFIX='/support';
function featurePath(url){if(url.pathname===FEATURE_PREFIX||url.pathname===`${FEATURE_PREFIX}/`)return '/';if(url.pathname.startsWith(`${FEATURE_PREFIX}/`))return url.pathname.slice(FEATURE_PREFIX.length)||'/';return url.pathname}
...
function enrichBrief(profile,brief,needContext={}){const needAssessment=assessNeedState(profile,needContext);return{...brief,capability:BENEFIT_RADAR_CAPABILITY,needAssessment,followUpQuestions:needAssessment.questions,notificationEligible:needAssessment.proactiveEligible,proactivePolicy:{requiresExplicitConsent:true,activityContextDefault:false,sensitiveInferenceDefault:false,externalDataDefault:false}}}

export default{async fetch(request,env){
  const url=new URL(request.url);
  const pathname=featurePath(url);
  if(url.hostname==='ekodi.kr'&&!(url.pathname===FEATURE_PREFIX||url.pathname.startsWith(`${FEATURE_PREFIX}/`)))return fetch(request);
  if(pathname==='/health')return json({ok:true,service:'ekodi-support-opportunity',surface:'support-platform',canonicalPath:FEATURE_PREFIX,stages:SUPPORT_STAGES,specialistServices:OPPORTUNITY_SERVICES.map(({id,path,label})=>({id,path,label})),sharedOpportunityCore:true,specialistWorkspace:true,officialSourceRequired:true,officialSources:officialSourceStatus(env).map(({id,mode})=>({id,mode})),submissionExecution:false,humanGateRequired:true,needSensing:'consent-first',activityContextDefault:false,sensitiveInferenceDefault:false,centralIdentityEnabled:centralIdentityConfig(env).enabled,dataMode:runtimeConfig(env).dataMode,ekodiShell:true,capability:BENEFIT_RADAR_CAPABILITY},200,env);
  if(pathname==='/config.js')return new Response(`window.EKODI_SUPPORT_CONFIG=${JSON.stringify(runtimeConfig(env))};`,{headers:{'content-type':'application/javascript; charset=utf-8','cache-control':'no-store',...securityHeaders(env)}});
  if(pathname==='/api/services'&&request.method==='GET')return json({services:OPPORTUNITY_SERVICES,sharedOpportunityCore:true,specialistWorkspace:true,capability:BENEFIT_RADAR_CAPABILITY},200,env);
  if(pathname==='/api/sources/status'&&request.method==='GET')return json({sources:officialSourceStatus(env)},200,env);
  if(pathname==='/api/need-assessment'&&request.method==='POST'){const p=await body(request);if(!p?.profile)return json({error:'profile_required'},400,env);return json({capability:BENEFIT_RADAR_CAPABILITY,assessment:assessNeedState(p.profile,p.needContext||{})},200,env)}
  if(pathname==='/api/opportunities'&&request.method==='GET'){const result=await getOfficialNotices(env,url);return json(result,result.ok?200:result.reason==='credential_required'?503:502,env)}
  if(pathname==='/api/opportunities/match'&&request.method==='POST'){const p=await body(request);if(!p?.profile)return json({error:'profile_required'},400,env);let notices=p.notices;if(!Array.isArray(notices)){const upstream=await fetchCombinedOpportunityNotices(env,{limit:p.limit||100,category:p.category||'',hashtags:p.hashtags||'',now:p.now||''});if(!upstream.ok)return json(upstream,upstream.reason==='credential_required'?503:502,env);notices=upstream.items}return json({capability:BENEFIT_RADAR_CAPABILITY,matches:rankOpportunities(p.profile,notices,{minScore:p.minScore,now:p.now,serviceId:p.serviceId}),needAssessment:assessNeedState(p.profile,p.needContext||{}),source:Array.isArray(p.notices)?'provided':'combined',serviceId:p.serviceId||'all'},200,env)}
  if(pathname==='/api/proactive-brief'&&request.method==='POST'){const p=await body(request);if(!p?.profile)return json({error:'profile_required'},400,env);let notices=Array.isArray(p.notices)?p.notices:null;if(!notices){const upstream=await fetchCombinedOpportunityNotices(env,{limit:p.limit||100,category:p.category||'',hashtags:p.hashtags||'',now:p.now||''});if(!upstream.ok){const brief=buildProactiveBrief(p.profile,[],p.projects||[],{minScore:p.minScore,now:p.now,serviceId:p.serviceId});return json({...upstream,brief:enrichBrief(p.profile,brief,p.needContext||{})},upstream.reason==='credential_required'?503:502,env)}notices=upstream.items}const brief=buildProactiveBrief(p.profile,notices,p.projects||[],{minScore:p.minScore,now:p.now,serviceId:p.serviceId});return json(enrichBrief(p.profile,brief,p.needContext||{}),200,env)}
  if(pathname==='/api/change-analysis'&&request.method==='POST'){const p=await body(request);if(!p)return json({error:'invalid_json'},400,env);return json(analyzeGuidanceChange(p.previous,p.current),200,env)}
  if(pathname==='/api/opportunity-score'&&request.method==='POST'){const p=await body(request);if(!p)return json({error:'invalid_json'},400,env);return json({score:scoreOpportunity(p.profile,p.notice)},200,env)}
