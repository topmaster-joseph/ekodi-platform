...
export default{async fetch(request,env){
  ...
  if(url.pathname==='/api/services'&&request.method==='GET')return json({services:OPPORTUNITY_SERVICES,sharedOpportunityCore:true,specialistWorkspace:true,capability:BENEFIT_RADAR_CAPABILITY},200,env);
  if(url.pathname==='/api/sources/status'&&request.method==='GET')return json({sources:officialSourceStatus(env)},200,env);
  if(url.pathname==='/api/need-assessment'&&request.method==='POST'){const p=await body(request);if(!p?.profile)return json({error:'profile_required'},400,env);return json({capability:BENEFIT_RADAR_CAPABILITY,assessment:assessNeedState(p.profile,p.needContext||{})},200,env)}
