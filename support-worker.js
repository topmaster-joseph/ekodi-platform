...
function runtimeConfig(env){const sources=officialSourceStatus(env);return{dataMode:env.DATA_MODE||'isolated-staging',authUrl:env.AUTH_URL||'https://auth.ekodi.kr/?site=support',centralIdentity:centralIdentityConfig(env),officialSourceRequired:true,officialSources:sources.map(({id,name,mode})=>({id,name,mode})),specialistServices:OPPORTUNITY_SERVICES.map(({id,path,label,sourceStatus})=>({id,path,label,sourceStatus})),submissionExecution:false,humanGateRequired:true,persistence:'browser-local-first',proactiveBriefing:true,needSensing:'consent-first',activityContextDefault:false,sensitiveInferenceDefault:false,externalDataDefault:false,sharedOpportunityCore:true,specialistWorkspace:true}}
...
export default{async fetch(request,env){
  ...
  if(url.pathname==='/api/need-assessment'&&request.method==='POST'){const p=await body(request);if(!p?.profile)return json({error:'profile_required'},400,env);return json({assessment:assessNeedState(p.profile,p.needContext||{})},200,env)}
