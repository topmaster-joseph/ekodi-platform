...
export async function callEkodiMcpTool(name,args,request,env,dependencies={}){
  ...
  if(name==='ekodi_my_ai_status'){
    ...
    const response=await userAiStatusForIdentity(new Request(`https://ekodi.kr/api/api/user-ai/status?site=${encodeURIComponent(site)}`),env,identity);
  ...
  if(name==='ekodi_my_services'){
    const response=await membershipPortfolioForIdentity(new Request('https://ekodi.kr/api/api/membership/portfolio'),env,identity);
