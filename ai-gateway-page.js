...
function providerName(value,mode){const name=String(value||'').trim();if(name==='openai')return 'OpenAI';if(name==='cloudflare-workers-ai')return 'Cloudflare Workers AI';if(name)return name;if(mode==='ai')return 'AI Provider';return 'EKODI Core'}
...
function renderStatus(data){const gateway=data.gateway||{};const providers=Array.isArray(data.providers)&&data.providers.length?data.providers:[data.workersAi?{id:data.workersAi.id||'cloudflare-workers-ai',...data.workersAi}:null,data.openai?{id:data.openai.id||'openai',...data.openai}:null].filter(Boolean);const mode=String(gateway.mode||'core');const active=providers.find(item=>item&&item.available&&item.configured!==false&&!gateway.providerDisabled)||null;const aiReady=Boolean(active);const activeName=providerName(active?.id,aiReady?'ai':mode);state('gatewayState',mode==='ai'?'AI 사용 가능':mode==='free_assist'?'Fallback 준비':'Core 전용',mode==='ai'?'ok':'warn');$('gatewayMeta').textContent='정책 '+(gateway.policyVersion||'-')+' · Provider '+Number(gateway.providerCount||0)+'개 · Core 독립 '+(gateway.providerIndependent?'유지':'확인 필요');state('openaiState',aiReady?activeName+' 연결 준비':'Provider 미구성',aiReady?'ok':'warn');$('openaiMeta').textContent='Model '+(active?.model||'-')+' · 실제 응답 Provider는 대화별 표시';state('fallbackState',gateway.providerIndependent?'정상 준비':'확인 필요',gateway.providerIndependent?'ok':'bad');$('fallbackMeta').textContent=gateway.notice||'외부 AI 장애 또는 미설정 시 EKODI Core / Free Assist로 계속 운영';setProvider(active?.id||'',active?.model||'',aiReady?'ai':mode);overall(aiReady?(active?.id==='openai'?'OpenAI 구성 정상 · 실제 호출 확인 가능':activeName+' 준비 완료 · 실제 호출 확인 가능'):gateway.providerIndependent?'Core 정상 · AI Provider 확인 필요':'AI Gateway 확인 필요',aiReady?'ok':'');stamp()}
