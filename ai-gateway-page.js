...
<body>
  ...
  <main class="wrap">
    ...
    <div class="summary"><span class="dot" id="overallDot"></span><strong id="overallText">상태 확인 대기</strong><span id="checkedAt"></span></div>

    <section class="login" id="signedOut">
      <h2>관리자 인증이 필요합니다</h2>
      <p>AI 공급자 상태와 실제 호출 테스트는 EKODI 관리자에게만 공개됩니다.</p>
      <div class="actions"><a class="button primary" href="${loginUrl}">Google 관리자 인증</a><a class="button" href="https://admin.ekodi.kr/#ai-ops">Admin AI Ops</a></div>
    </section>
...
function acceptHandoff(){const hash=new URLSearchParams(location.hash.replace(/^#/,''));const value=hash.get('ekodi_admin_token');if(!value)return false;setToken(value);history.replaceState({},document.title,location.pathname+location.search);return true}
function authHeaders(json=false){const headers={};const value=token();if(value)headers.authorization='Bearer '+value;if(json)headers['content-type']='application/json';return headers}
async function request(path,options={}){const response=await fetch(path,{...options,headers:{...authHeaders(Boolean(options.body)),...(options.headers||{})},cache:'no-store'});let data={};try{data=await response.json()}catch{}if(!response.ok)throw Object.assign(new Error(data.error||('HTTP '+response.status)),{status:response.status,data});return data}
function signedIn(on){$('signedIn').hidden=!on;$('signedOut').hidden=on}
function state(id,text,tone=''){const el=$(id);el.textContent=text;el.className='state'+(tone?' '+tone:'')}
function overall(text,tone=''){const dot=$('overallDot');dot.className='dot'+(tone?' '+tone:'');$('overallText').textContent=text}
function stamp(){const now=new Date();$('checkedAt').textContent=' · '+now.toLocaleString('ko-KR')}
function renderStatus(data){const gateway=data.gateway||{};const openai=data.openai||{};const mode=String(gateway.mode||'core');const aiReady=Boolean(openai.configured&&openai.available&&!gateway.providerDisabled);state('gatewayState',mode==='ai'?'AI 사용 가능':mode==='free_assist'?'Fallback 준비':'Core 전용',mode==='ai'?'ok':mode==='core'?'warn':'warn');$('gatewayMeta').textContent='정책 '+(gateway.policyVersion||'-')+' · Provider '+Number(gateway.providerCount||0)+'개 · Core 독립 '+(gateway.providerIndependent?'유지':'확인 필요');state('openaiState',openai.configured?(openai.available?'구성됨 · 런타임 준비':'구성 확인 필요'):'미구성',aiReady?'ok':'warn');$('openaiMeta').textContent='Model '+(openai.model||'-')+' · 실제 네트워크 응답은 아래 연결 테스트에서 확인';state('fallbackState',gateway.providerIndependent?'정상 준비':'확인 필요',gateway.providerIndependent?'ok':'bad');$('fallbackMeta').textContent=gateway.notice||'OpenAI 장애 또는 미설정 시 EKODI Core / Free Assist로 계속 운영';overall(aiReady?'OpenAI 구성 정상 · 실제 호출 확인 가능':gateway.providerIndependent?'Core 정상 · AI Provider 확인 필요':'AI Gateway 확인 필요',aiReady?'ok':'');stamp()}
async function refresh(){overall('상태 확인 중');$('refreshBtn').disabled=true;try{const data=await request('/api/control/ai/provider-status');renderStatus(data)}catch(error){if(error.status===401||error.status===403){setToken('');signedIn(false);overall('관리자 인증 필요','bad');return}overall('상태 API 확인 필요','bad');state('gatewayState','확인 실패','bad');$('gatewayMeta').textContent=error.message}finally{$('refreshBtn').disabled=false}}
async function liveTest(){const button=$('testBtn');button.disabled=true;$('testResult').textContent='OpenAI 실제 호출을 확인하고 있습니다…';try{const data=await request('/api/control/ai/assist',{method:'POST',body:JSON.stringify({message:'연결 상태 확인입니다. EKODI AI 연결 확인이라고 짧게 응답해 주세요.',context:{section:'ai-gateway',title:'EKODI AI Gateway',pathname:location.pathname}})});const direct=data.mode==='ai'&&data.provider==='openai';$('testResult').textContent=direct?'✅ 실제 연결 정상\nProvider: OpenAI\nModel: '+(data.model||'-')+'\n응답: '+(data.reply||'정상'):'⚠️ OpenAI 직접 응답 대신 EKODI Fallback 동작\nMode: '+(data.mode||'-')+'\n'+(data.notice||data.reply||'Core 기능은 계속 이용할 수 있습니다.');overall(direct?'OpenAI 실제 호출 정상':'Fallback 동작 · Core 정상',direct?'ok':'');stamp()}catch(error){$('testResult').textContent='❌ 실제 연결 테스트 실패\n'+error.message;overall('연결 테스트 확인 필요','bad')}finally{button.disabled=false}}
async function validate(){if(!token()){signedIn(false);overall('관리자 인증 필요');return}try{await request('/api/session');signedIn(true);await refresh()}catch(error){if(error.status===401||error.status===403){setToken('');signedIn(false);overall('관리자 인증 필요','bad')}else{signedIn(true);overall('세션 확인 지연','bad')}}}
$('refreshBtn')?.addEventListener('click',refresh);$('testBtn')?.addEventListener('click',liveTest);$('logoutBtn')?.addEventListener('click',async()=>{try{await request('/api/logout',{method:'POST'})}catch{}setToken('');signedIn(false);overall('로그아웃 완료')});acceptHandoff();validate();
})();`;
}
