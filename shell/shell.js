...
function canonicalPublicUserSurface(){const host=location.hostname.toLowerCase(),path=location.pathname.toLowerCase();if(host!=='ekodi.kr'&&host!=='www.ekodi.kr')return false;if(path==='/my'||path.startsWith('/my/')||path==='/admin'||path.startsWith('/admin/')||path==='/auth'||path.startsWith('/auth/')||path==='/api'||path.startsWith('/api/')||path==='/mcp'||path.startsWith('/mcp/')||path==='/webhooks'||path.startsWith('/webhooks/')||path==='/health'||path.startsWith('/health/')||path.startsWith('/w/'))return false;return true;}
function memberGateApplies(){if(isPublicSurface()||canonicalPublicUserSurface())return false;const p=memberPolicy();const explicitWorkspace=location.pathname.toLowerCase().startsWith('/w/')||new URLSearchParams(location.search).has('workspace');return Boolean(memberGateMode!=='service-owned'&&p&&p.guestMode==='guide-only'&&p.minimumTier==='free'&&!guestPublicException()&&surface==='workspace'&&explicitWorkspace);}
...
function renderMemberGate(mode='guest'){
  ...
  wrap.innerHTML='<style>*{box-sizing:border-box}.card{width:min(620px,100%);border:1px solid #24425e;border-radius:24px;padding:30px;background:#0b1d2e;box-shadow:0 28px 80px rgba(0,0,0,.42)}.eyebrow{font-size:12px;font-weight:850;letter-spacing:.14em;color:#8ec8ff}.card h1{margin:10px 0 12px;font-size:clamp(30px,7vw,48px);line-height:1.05}.card p{margin:0;color:#b6c5d3;line-height:1.75}.features{margin-top:14px;color:#dce8f2;font-size:14px;font-weight:750}.note{margin-top:18px;padding:14px 16px;border-radius:14px;background:#10263a;color:#dce8f2;font-size:14px}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:22px}a{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 18px;border-radius:12px;text-decoration:none;font-weight:850}.primary{background:#f4f7fb;color:#071522}.secondary{border:1px solid #31506e;color:#dce8f2}.small{margin-top:18px;color:#8296aa;font-size:12px}@media(max-width:560px){.card{padding:24px 20px}.actions{display:grid}.actions a{width:100%}}</style><div class="card"><div class="eyebrow">EKODI COMMON SERVICE</div><h1></h1><p></p><div class="features"></div><div class="note"></div><div class="actions"></div><div class="small">공개는 안내까지, 이용은 무료회원부터.</div></div>';
...
window.EKODIShell={
  ...
  setSurface,
  navigate,
  open:()=>{if(panel&&workspaceUiAvailable())panel.hidden=false;},
  close:closePanel,
  myUrl
};
