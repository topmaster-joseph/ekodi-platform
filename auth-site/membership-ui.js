...
const tenant=(()=>{try{return tenantByOrigin.get(new URL(returnTo).origin)||'';}catch{return '';}})();
...
function membershipBody(planId){return{site,planId,...(tenant?{tenant}:{}),returnTo};}
function membershipQuery(){const query=new URLSearchParams({site});if(tenant)query.set('tenant',tenant);return query.toString();}

async function choose(plan,data){
  if(!data.canManagePlan){notice('현재 역할은 이 조직의 이용등급을 변경할 수 없습니다. 점주·본사·회계 담당 권한에서 변경할 수 있습니다.','warn');return;}
  if(plan.monthlyFee<=0){
    ...
    renderCurrent(result.subscription,plan);
    notice(plan.id==='free'?'무료회원으로 계속 이용할 수 있습니다.':'FLEX가 적용되었습니다. 필요한 기능만 선택해서 이용할 수 있습니다.');
    return;
  }
...
async function cancelSubscription(data){
  if(!data.canManagePlan){notice('현재 역할은 이 조직의 구독을 해지할 수 없습니다.','warn');return;}
...
function renderCurrent(subscription,plan){
  if(!currentEl)return;
  const label=plan?.label||String(subscription?.planId||'FREE').toUpperCase();
  const suffix=subscription?.monthlyFee>0?` · 월 ${won(subscription.monthlyFee)}`:subscription?.planId==='flex'?' · 종량제':' · 무료';
  const ending=subscription?.cancelAtPeriodEnd?' · 해지 예약됨':'';
  currentEl.textContent=`현재 ${label}${suffix}${ending}${tenant?` · ${tenant}`:''}`;
}
function render(data){
  if(!panel||!grid)return;
  panel.classList.remove('hide');
  const currentPlan=data.plans.find(plan=>plan.id===data.subscription.planId)||data.plans[0];
  renderCurrent(data.subscription,currentPlan);
  ...
  else if(!data.canManagePlan)notice('등록된 역할과 이용등급을 확인했습니다. 이 조직의 등급 변경은 점주·본사·회계 담당 권한에서 할 수 있습니다.','warn');
...
async function completeBillingIfNeeded(){
  ...
  try{
    ...
    notice(`${String(result.subscription.planId||'').toUpperCase()} 월 구독이 시작되었습니다.`);
