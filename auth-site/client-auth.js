...
$('serviceBadge').textContent=config.open?(config.kind==='seller'?'무료 판매자 가입':'커뮤니티'):'고객 전용';
...
if(config.kind==='seller'){
  ...
  if(introCopy)introCopy.textContent='Google 계정으로 본인만 확인합니다. 가입 후 EKODI Mall Seller Studio의 무료 기능을 바로 체험할 수 있습니다.';
  $('signedOutCopy').textContent='Google 계정으로 계속하면 무료 Seller 계정으로 연결됩니다. 실제 직접판매와 정산은 별도 판매자 확인 후 활성화됩니다.';
}else if(config.open){
...
async function prepare(){
  ...
  const preparing=config.kind==='seller'?'EKODI Mall 무료 판매자 Google 인증을 준비하고 있습니다.':config.open?'EKODI Community Google 인증을 준비하고 있습니다.':'사전등록 고객용 Google 인증을 준비하고 있습니다.';
  notice(preparing);
  try{
    ...
    window.google.accounts.id.initialize({client_id:challenge.clientId,nonce:challenge.nonce,auto_select:false,use_fedcm_for_prompt:true,callback:async response=>{
      const working=config.kind==='seller'?'Google 계정을 확인하고 Seller Studio로 연결하고 있습니다.':config.open?'Google 계정을 확인하고 Community로 안전하게 연결하고 있습니다.':'Google 계정을 확인하고 고객 관리공간으로 안전하게 연결하고 있습니다.';
      notice(working);
      try{
    ...
    const ready=config.kind==='seller'?'스토어를 운영할 Google 계정으로 계속해 주세요.':config.open?'사용할 Google 계정으로 계속해 주세요.':'Control Center에 등록된 고객 이메일과 같은 Google 계정으로 계속해 주세요.';
    notice(ready);
