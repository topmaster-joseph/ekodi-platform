...
const requestedPlan=['flex','plus','pro','auto'].includes(String(params.get('plan')||'').toLowerCase())?String(params.get('plan')).toLowerCase():null;
const explicitPro=requestedPlan==='pro'||params.get('intent')==='pro';
...
$('signedOutCopy').textContent='Google 계정으로 무료회원이 됩니다. 개인 공간은 보안 연결 후 바로 사용할 수 있고, 사업장·단체 공간도 같은 통합계정으로 연결됩니다.';
...
function cleanUrl(){const q=new URLSearchParams({site,return_to:returnTo});if(requestedPlan)q.set('plan',requestedPlan);if(manageMode)q.set('manage','1');history.replaceState({},document.title,`/?${q.toString()}`)}
...
function launchable(items){return(items||[]).filter(item=>['active','pre_registered','free'].includes(String(item?.status||'')))}
...
async function handoffToService(workspaceKey){if(!currentSession)throw new Error('login_required');if(!workspaceKey)throw new Error('workspace_required');const r=await fetchTimed(MARKETING_API+'/api/marketing/handoff/start',{method:'POST',credentials:'include',headers:{Authorization:`Bearer ${currentSession.access_token}`,'content-type':'application/json'},body:JSON.stringify({return_to:returnTo,workspace_key:workspaceKey}),cache:'no-store'});const text=await r.text();let d={};try{d=text?JSON.parse(text):{}}catch{}if(!r.ok||!d.returnTo)throw Object.assign(new Error(d.error||'handoff_unavailable'),{status:r.status,data:d});location.assign(d.returnTo)}
async function openWorkspace(item,button){if(button){button.disabled=true;button.classList.add('loading')}try{await handoffToService(item.workspace_key)}catch(error){console.error('workspace handoff',error);notice('accessStatus','선택한 공간의 보안 연결이 지연되고 있습니다. 다시 시도해 주세요.','error');show('freeActions',true);if(button){button.disabled=false;button.classList.remove('loading')}}}
async function continueFreeWorkspace(){const item=currentWorkspaces.find(row=>row?.workspace_kind==='personal'&&['active','pre_registered','free'].includes(String(row?.status||'')))||launchable(currentWorkspaces).find(row=>String(row?.plan||'free').toLowerCase()==='free');if(!item){notice('accessStatus','무료 개인 Workspace를 준비하지 못했습니다. 잠시 후 다시 로그인해 주세요.','error');return}try{await handoffToService(item.workspace_key)}catch(error){console.error('free workspace handoff',error);notice('accessStatus','무료 작업공간의 보안 연결이 지연되고 있습니다. 다시 시도해 주세요.','error')}}

async function renderAccess(s){currentSession=s;routing=false;showSignedIn(s);resetPanels();notice('accessStatus','내 Marketing AI 공간을 확인하고 있습니다.');let workspaces;try{workspaces=await loadWorkspaces()}catch(error){console.error('workspace list',error);showAccessFailure('공간 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.');return}renderWorkspacePanel(workspaces);const active=launchable(workspaces);fallbackWorkspaceKey=active[0]?.workspace_key||null;if(active.length===1&&workspaces.length===1&&!requestedPlan&&!manageMode){routing=true;$('serviceBadge').textContent=`${workspacePlanLabel(active[0].plan)} 이용중`;notice('accessStatus','Google 인증이 확인되었습니다. 안전하게 Marketing AI로 연결합니다.');try{await handoffToService(active[0].workspace_key);return}catch(error){console.error('automatic handoff',error);routing=false;show('approvedActions',true);show('freeActions',true);notice('accessStatus','자동 연결이 지연되어 직접 이동 버튼을 열었습니다.','warn')}}else if(active.length){$('serviceBadge').textContent=workspaces.length>1?`${workspaces.length}개 공간`:`${workspacePlanLabel(active[0].plan)} 이용중`;notice('accessStatus',workspaces.length>1?'사용할 개인·사업장·단체 공간을 선택해 주세요.':'Marketing AI 이용 권한이 확인되었습니다.');if(workspaces.length===1)show('approvedActions',true);show('freeActions',true)}else{$('serviceBadge').textContent='Workspace 필요';notice('accessStatus','Marketing AI 개인 Workspace를 준비하지 못했습니다. 다시 로그인하거나 관리자에게 문의해 주세요.','error');show('freeActions',true)}if(requestedPlan==='pro'||explicitPro)show('requestActions',true);await loadLinkedIdentities()}
...
$('continueFree').addEventListener('click',()=>void continueFreeWorkspace());
$('addGoogleAccount').addEventListener('click',()=>void prepareLinkGoogle());
$('continueService').addEventListener('click',async()=>{const btn=$('continueService');btn.disabled=true;try{await handoffToService(fallbackWorkspaceKey)}catch(error){console.error('continue service',error);btn.disabled=false;show('freeActions',true);notice('accessStatus','서비스 보안 연결이 지연되고 있습니다. 다시 시도해 주세요.','warn')}});
...
$('requestAccess').addEventListener('click',async()=>{const btn=$('requestAccess');btn.disabled=true;try{const payload={site:'marketing',role:'store_owner',plan:requestedPlan||'pro',note:$('requestNote').value.trim(),business_name:$('businessName').value.trim(),contact_phone:$('contactPhone').value.trim(),business_number:$('businessNumber').value.trim()};const d=await api('/request',{method:'POST',body:JSON.stringify(payload)});notice('requestStatus',d.already_pending?'이미 검수 중인 신청입니다.':`Marketing AI ${(requestedPlan||'pro').toUpperCase()} 사용신청이 접수되었습니다.`)}catch(error){console.error('pro request',error);notice('requestStatus','사용신청을 처리하지 못했습니다.','error')}finally{btn.disabled=false}});
