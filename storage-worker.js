...
export default {
  async fetch(request,env){
    ...
    if(request.method==='OPTIONS'){
      ...
      const headers=new Headers({'access-control-allow-methods':'GET,HEAD,POST,PUT,DELETE,OPTIONS','access-control-allow-headers':'authorization,content-type,x-ekodi-storage-key,x-request-id','access-control-max-age':'86400','cache-control':'no-store','vary':'Origin'});
    ...
    if(url.pathname.startsWith('/api/storage/v1')){
      ...
      catch(error){console.error('Storage Gateway error',error);return json({error:'EKODI Storage Gateway 처리 중 오류가 발생했습니다.',code:'STORAGE_GATEWAY_ERROR'},500);}
    }
