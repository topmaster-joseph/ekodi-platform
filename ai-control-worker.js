...
function config(env={}){return{platform:'ai-control',mode:env.AI_CONTROL_MODE||'free-first',policyVersion:AI_CONTROL_POLICY.version,missionPolicyVersion:AI_CONTROL_POLICY.missionPolicyVersion,taskOwner:'ekodi_orchestrator',entryAiRole:'entry_point_and_bounded_participant',finalPlatformAuthority:'ekodi_platform_super_administrator',authUrl:env.AUTH_URL||'https://auth.ekodi.kr/?site=ai&return_to=https%3A%2F%2Fai.ekodi.kr%2F',taskExecutionEnabled:env.AI_TASK_EXECUTION_ENABLED==='true',branchAllocationEnabled:env.AI_GITHUB_ORCHESTRATION_ENABLED==='true',humanApprovalRequired:true,platformSuperAdminApprovalEnforced:true,nodePairingEnabled:true}}
...
function superAdminEmails(env={}){const dedicated=clean(env.PLATFORM_SUPER_ADMIN_EMAILS||env.SUPER_ADMIN_EMAILS||env.SUPER_ADMIN_EMAIL).split(',').map(v=>v.trim().toLowerCase()).filter(Boolean);if(dedicated.length)return dedicated;const admins=clean(env.ADMIN_EMAILS||env.ADMIN_EMAIL).split(',').map(v=>v.trim().toLowerCase()).filter(Boolean);return admins.slice(0,1)}
function isPlatformSuperAdmin(user,env={}){return clean(user?.role).toLowerCase()==='super_admin'||superAdminEmails(env).includes(clean(user?.email).toLowerCase())}
...
async function requireAdmin(request,env){
  ...
  return{user:{id:clean(user.id),email,role}};
}
...
async function insertTask(env,task){
  ...
  await env.DB.prepare('INSERT INTO ai_control_tasks (id,title,prompt,mode,state,requested_providers,needs_code_branch,branch,governance_json,mission_policy_version,mission_tier,mission_reason,mission_explanation,analysis_only,entry_ai,task_owner,approval_authority,created_by,created_at,updated_at,approval_state,approved_by,approved_at,result_summary,error) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(task.id,task.title,task.prompt,task.mode,task.state,JSON.stringify(task.requestedProviders),task.needsCodeBranch?1:0,'',JSON.stringify(task.governance||{}),m.policyVersion||'',m.tier||'',m.reason||'',m.explanation||'',m.analysisOnly?1:0,task.entryAi||'ekodi-control-plane','ekodi_orchestrator',task.approvalAuthority||'authorized_human_within_delegated_scope',task.createdBy,task.createdAt,task.updatedAt,'pending','','','','').run();
}
function taskRow(row){if(!row)return null;const tier=row.mission_tier||'';const approvalAuthority=row.approval_authority||'authorized_human_within_delegated_scope';return{id:row.id,title:row.title,prompt:row.prompt,mode:row.mode,state:row.state,requestedProviders:JSON.parse(row.requested_providers||'[]'),needsCodeBranch:Boolean(row.needs_code_branch),branch:row.branch||'',entryAi:row.entry_ai||'ekodi-control-plane',taskOwner:row.task_owner||'ekodi_orchestrator',executionEnvironment:'development',governance:JSON.parse(row.governance_json||'{}'),missionDecision:{policyVersion:row.mission_policy_version||'',tier,reason:row.mission_reason||'',explanation:row.mission_explanation||'',approvalAuthority,analysisOnly:Boolean(row.analysis_only),forbidden:tier==='forbidden',superAdminGate:tier==='super_admin_gate',humanGate:tier==='human_gate',allowModelConsultation:tier!=='forbidden',autonomousActionAllowed:['observe','execute_reversible'].includes(tier),humanApprovalRequired:tier!=='forbidden'},createdBy:row.created_by,createdAt:row.created_at,updatedAt:row.updated_at,approvalAuthority,approvalState:row.approval_state||'pending',approvedBy:row.approved_by||'',approvedAt:row.approved_at||'',resultSummary:row.result_summary?JSON.parse(row.result_summary):null,error:row.error||''}}
...
async function execute(env,id){
  ...
  const approvalAuthority=currentMission.superAdminGate?'ekodi_platform_super_administrator':'authorized_human_within_delegated_scope';
  task={...task,missionDecision:currentMission,approvalAuthority};
  await patchTask(env,id,{state:'allocating',approval_authority:approvalAuthority,updated_at:now(),error:''});
...
export default{async fetch(request,env,ctx){
  ...
  if(url.pathname.startsWith('/api/')){
    ...
    if(request.method==='POST'&&url.pathname==='/api/tasks'){
      ...
      try{const input=normalizeTaskInput(await body(request)||{});const missionDecision=evaluateTaskMissionPolicy(input);const approvalAuthority=missionDecision.superAdminGate?'ekodi_platform_super_administrator':'authorized_human_within_delegated_scope';const stamp=now();const task={...input,missionDecision,approvalAuthority,id:createTaskId(),state:missionDecision.forbidden?'blocked_policy':'queued',createdBy:auth.user.email,createdAt:stamp,updatedAt:stamp};await insertTask(env,task);return json({task},201)}catch(error){return json({error:error.message},error.message==='state_store_unavailable'?503:400)}
    }
    ...
    const approveId=taskId(url.pathname,'/approve');if(request.method==='POST'&&approveId){const task=await getTask(env,approveId);if(!task)return json({error:'task_not_found'},404);if(task.state!=='approval_required')return json({error:'task_not_ready_for_approval'},409);if(task.approvalAuthority==='ekodi_platform_super_administrator'&&!isPlatformSuperAdmin(auth.user,env))return json({error:'platform_super_admin_required',approvalAuthority:task.approvalAuthority},403);const approvedAt=now();await patchTask(env,approveId,{approval_state:'approved',approved_by:auth.user.email,approved_at:approvedAt,state:'completed',updated_at:approvedAt});return json({ok:true,task:await getTask(env,approveId)})}
