...
import {getAiProviderFeedbackMetrics,recordAiProviderFeedback} from './ai-provider-feedback.js';
...
async function providerPerformanceMetrics(env){
  ...
  const feedback=await getAiProviderFeedbackMetrics(env,{since:cutoff,limit:AI_ROUTER_SCORE_POLICY.maxHistoryRuns}).catch(()=>({}));
  const out={};
  ...
  for(const [id,quality] of Object.entries(feedback)){const m=out[id]||(out[id]={totalRuns:0,successfulRuns:0,failedRuns:0,recentRuns:0,recentFailures:0,activeRuns:0,averageLatencyMs:null,_latencyTotal:0,_latencyCount:0});m.qualityScore=quality.qualityScore;m.feedbackRuns=quality.feedbackRuns;m.adoptionSignals=quality.adoptionSignals;m.adoptedRuns=quality.adoptedRuns;m.outcomeSignals=quality.outcomeSignals;}
...
export default{async fetch(request,env,ctx){
  ...
  if(url.pathname.startsWith('/api/')){
    ...
    const feedbackId=taskId(url.pathname,'/feedback');if(request.method==='POST'&&feedbackId){const task=await getTask(env,feedbackId);if(!task)return json({error:'task_not_found'},404);const input=await body(request)||{};const providerId=clean(task.resultSummary?.responseProvider);if(!providerId)return json({error:'feedback_provider_unavailable'},409);try{const feedback=await recordAiProviderFeedback(env,{taskId:feedbackId,providerId,adopted:typeof input.adopted==='boolean'?input.adopted:undefined,outcomeScore:input.outcomeScore,note:input.note,source:'user_feedback',createdBy:auth.user.email});return json({ok:true,feedback})}catch(error){return json({error:clean(error?.message||error)},400)}}
    const approveId=taskId(url.pathname,'/approve');if(request.method==='POST'&&approveId){const task=await getTask(env,approveId);if(!task)return json({error:'task_not_found'},404);if(task.state!=='approval_required')return json({error:'task_not_ready_for_approval'},409);await patchTask(env,approveId,{approval_state:'approved',state:'completed',updated_at:now()});const providerId=clean(task.resultSummary?.responseProvider);if(providerId)await recordAiProviderFeedback(env,{taskId:approveId,providerId,adopted:true,source:'approval',createdBy:auth.user.email}).catch(()=>{});return json({ok:true,task:await getTask(env,approveId)})}
