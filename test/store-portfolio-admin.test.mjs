...
test('router serves cmpmyi common panels and same-origin embedded canonical store admins',async()=>{
  ...
  assert.match(embeddedHtml,/store-admin\.css\?v=20260924-store-nav-v3/);
  const css=await (await platformEntry.fetch(new Request('https://ekodi.kr/store-admin.css'),{},{})).text();
  assert.match(css,/\[data-ekodi-embedded-admin="true"\] main\{max-width:none;padding:9px 11px 18px\}/);
  assert.match(css,/\[data-ekodi-embedded-admin="true"\] \.panel\{padding:11px;min-height:132px/);
