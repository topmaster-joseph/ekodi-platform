...
test('Connect is consent-first and marriage is explicit opt-in', () => {
  ...
  assert.match(api, /marriage_enabled/);
});

test('Connect never reveals contact details in v1 match payloads', () => {
  assert.match(page, /연락처는 상호 관심 뒤에도 자동 공개하지 않습니다/);
  assert.match(api, /contact_revealed: false/);
  const recommendationBlock = api.slice(api.indexOf('path === "/recommendations"'), api.indexOf('path === "/interest"'));
  assert.doesNotMatch(recommendationBlock, /session\.user\.email|email:/);
});

test('Connect provides block and report safety controls', () => {
  assert.match(api, /path === "\/block"/);
  assert.match(api, /path === "\/report"/);
