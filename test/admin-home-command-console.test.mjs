...
test('admin root is a command-only workspace while Campus remains a child route',async()=>{
  ...
  assert.match(dock,/AI_EXECUTION_PREFIX='\\/api\\/control\\/common-services\\/ai'/);
  assert.match(dock,/function dispatchExecutionTask\\(value,c,queued\\)/);
  assert.match(dock,/needsCodeBranch:true/);
  assert.match(dock,/production:false/);
  assert.match(dock,/area:'software_change'/);
  assert.match(dock,/queued\\.status==='ready_for_executor'/);
  assert.match(dock,/tasks\\/\\$\\{encodeURIComponent\\(task\\.id\\)\\}\\/run/);
  assert.match(dock,/독립 개발 브랜치에서만 구현하십시오/);
  assert.match(dock,/production을 직접 변경하거나 배포하지 마십시오/);
