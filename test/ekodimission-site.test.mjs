...
test('EKODI Mission pages are routed as branded private-review workspace surfaces',async()=>{
  for(const [path,marker] of pageCases){const response=await spaceWorker.fetch(new Request(`https://ekodi.kr${path}`),env);assert.equal(response.status,200,path);assert.equal(response.headers.get('x-ekodi-route'),'ekodimission-preview',path);assert.equal(response.headers.get('x-ekodi-independent-site'),'true',path);assert.equal(response.headers.get('x-ekodi-site-class'),'brand-site',path);assert.match(response.headers.get('x-robots-tag')||'',/noindex/i,path);assert.match(await response.text(),new RegExp(marker),path)}
});
...
test('mission production smoke covers every private-review subservice route',async()=>{
  const manifest=JSON.parse(await readFile(new URL('../deploy/manifests/space.worker.json',import.meta.url),'utf8'));const urls=new Set(manifest.worker.requests.map(item=>item.url));for(const [path] of pageCases)assert.ok(urls.has(`https://ekodi.kr${path}`),path);
});
