...
test('authenticated Admin E2E activates only the target demand menu before contextual navigation', () => {
  ...
  assert.match(loader, /DEMAND_KEY_ALIASES=Object\.freeze\(\{'marketing-ai':'marketing','ai-membership':'aimembers'\}\)/);
  assert.match(loader, /const demandKey=value=>DEMAND_KEY_ALIASES/);
  assert.match(loader, /aimembers:\s*\{/);
  assert.match(loader, /marketing:\s*\{/);
  assert.match(loader, /activate:\s*key\s*=>\s*\{\s*const resolved=demandKey\(key\)/);
  assert.match(loader, /key === 'marketing' \? 'marketing-ai' : key === 'aimembers' \? 'ai-membership' : key/);
