...
test('critical admin auth assets cannot remain stale in the browser cache', () => {
  for (const asset of ['/auth-router.js','/admin-auth.js','/author-auth.js','/business-auth.js']) assert.ok(worker.includes(`'${asset}'`));
...
test('guarded production release verifies current central router and mobile-safe admin auth assets', () => {
