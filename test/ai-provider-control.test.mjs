...
test('common provider gateway exposes allowed-origin CORS contract',async()=>{
  const response=await handleAiProviderControl(new Request('https://ekodi.kr/api/ai-modules/v1/providers/generate',{method:'OPTIONS',headers:{origin:'https://ekodi.kr/my'}}),{ALLOWED_ORIGINS:'https://ekodi.kr/my,https://ekodi.kr/admin'});
...
test('common provider gateway blocks unauthenticated generation before provider access',async()=>{
  const response=await handleAiProviderControl(new Request('https://ekodi.kr/api/ai-modules/v1/providers/generate',{method:'POST',headers:{origin:'https://ekodi.kr/my','content-type':'application/json'},body:JSON.stringify({capability:'documents',input:'test'})}),{ALLOWED_ORIGINS:'https://ekodi.kr/my'});
