...
const routingMigration=readFileSync(new URL('../supabase/migrations/20260906006000_store_admin_routing.sql',import.meta.url),'utf8');
...
test('existing first stores are compatibility profiles on one Store Admin Engine',async()=>{
  for(const store of stores){
    ...
    assert.match(html,new RegExp(store.brand));assert.match(html,/\/store-admin\.js\?v=20260924-auth-first-v1/);assert.match(html,/data-ekodi-admin-sidebar/);assert.match(html,/data-ekodi-authority-scope="tenant"/);assert.match(html,/data-ekodi-admin-layout="two-level"/);assert.match(html,/id="sectionNav"[^>]*data-ekodi-admin-subnav/);assert.match(html,/data-ekodi-admin-nav-mode="primary"/);assert.match(html,/data-ekodi-admin-page-heading/);
  }
  ...
  assert.match(script,/관리 메뉴와 운영 데이터를 표시하지 않습니다/);assert.match(script,/setAdminGate\('authorized'\)/);
...
test('Store Admin keeps every requested route behind the auth-first gate before JavaScript authorization',async()=>{
  const profile={slug:'jadam',name:'자담치킨 목포대점',id:'4b1e5933-b9ae-4cb9-9d31-dcbb0a5b25aa',mark:'JD',brand:'JADAM CHICKEN'};
  for(const pathname of ['/jadam/admin/menu','/jadam/admin/delivery','/jadam/admin/connections']){
    ...
    assert.match(html,/id="adminApp" class="admin-app" hidden/);
    assert.match(html,/관리자 인증 확인 중/);
    assert.match(html,/인증 후 허용된 관리 화면이 표시됩니다/);
    assert.doesNotMatch(html,/<h1 id="pageTitle">(?:메뉴 · 가격|배달플랫폼|연결관리)<\/h1>/);
    assert.doesNotMatch(html,/<title>(?:메뉴 · 가격|배달플랫폼|연결관리) ·/);
  }
  const script=await storeAdminScript().text();
  assert.match(script,/state\.menu=menu;if\(!canSection\(section,role\)\)return permissionPanel\(\);[\s\S]*setAdminGate\('authorized'\)/);
  assert.match(script,/setAdminGate\('signed-out'/);
  assert.match(script,/setAdminGate\('forbidden'/);
});
