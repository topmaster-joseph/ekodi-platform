...
test('experience is a registered common-service boundary at ekodi.kr/experience',()=>{
  ...
  assert.ok(boundary.domains.includes('ekodi.kr/experience'));
...
test('experience worker health and catalog are public GET-only contracts',async()=>{
  const env={};
  const health=await worker.fetch(new Request('https://ekodi.kr/experience/health'),env);
  ...
  const catalogResponse=await worker.fetch(new Request('https://ekodi.kr/experience/api/catalog'),env);
  assert.equal(catalogResponse.status,200);
  const catalog=await catalogResponse.json();
  assert.equal(catalog.safety.productionWrites,false);
  const post=await worker.fetch(new Request('https://ekodi.kr/experience/api/catalog',{method:'POST'}),env);
