...
test('Primary AI Ops does not auto-hydrate Governance, Health or Deployments',async()=>{
  ...
  const devotionalStart=demand.indexOf('devotional:',aiopsStart);
  assert.ok(aiopsStart >= 0 && devotionalStart > aiopsStart, 'AI Ops feature block must precede devotional');
  const aiops=demand.slice(aiopsStart,devotionalStart);
