...
test('site admins use each managed site canonical path plus /admin', async () => {
  ...
  assert.equal(isWorkspaceAdminPath('/ekodibiz/ekodimall/admin/'), false);
  assert.equal(isWorkspaceAdminPath('/ekodibiz/ekodimall/admin/channel-settings'), false);
  ...
  assert.match(js, /workspace=\(standaloneMall\?'ekodimall'/);
  assert.match(js, /adminBase=standaloneMall\?'\/ekodimall\/admin':service\?/);
...
test('top-level EKODIMALL admin is parsed as the Mall service surface', async () => {
  ...
  assert.match(js, /standaloneMall\?'ekodimall'/);
