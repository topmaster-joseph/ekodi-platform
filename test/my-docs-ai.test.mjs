...
test('Docs AI exposes a human-controlled editable workspace',()=>{
  assert.match(html,/contenteditable="true"/);assert.match(html,/id="aiPreview"/);assert.match(html,/id="applyAi"/);assert.match(html,/AI는 바로 덮어쓰지 않습니다/);assert.match(html,/data-export="docx"/);assert.match(html,/data-export="hwpx"/);assert.match(html,/\.hwpx/);assert.match(html,/id="versionPanel"/);assert.match(html,/id="aiQuota"/);assert.match(html,/id="dropOverlay"/);assert.match(html,/긴 문서는 자동 분할/);assert.match(html,/HWPX/);
});
test('document client keeps auth, sanitization and storage boundaries explicit',()=>{
  assert.match(js,/document_files/);assert.match(js,/document_versions/);assert.match(js,/personal:\$\{session\.user\.id\}/);assert.match(js,/script,style,iframe,object,embed,form,input,button,meta,link/);assert.match(js,/mammoth@1\.9\.1\/mammoth\.browser\.min\.js/);assert.doesNotMatch(js,/mammoth@1\.9\.1\/\+esm/);assert.match(js,/docx@8\.5\.0/);assert.match(js,/importHwpx/);assert.match(js,/createHwpxBlob/);assert.match(js,/loadVersions/);assert.match(js,/extractRawText/);assert.match(js,/ensureEditorVisible/);assert.match(js,/dragstart/);assert.match(js,/dropOverlay/);assert.match(js,/document-ai-api/);assert.match(hwpx,/application\/hwp\+zip/);assert.match(hwpx,/MAX_HWPX_BYTES=20\*1024\*1024/);
});
