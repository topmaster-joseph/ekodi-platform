...
test('canonical Mall and child-service publishing routes use site-owned admins', async()=>{
  ...
  assert.equal(isWorkspaceAdminPathShape('/ekodibiz/ekodimall/admin/'),false);
  assert.equal(isWorkspaceAdminPathShape('/ekodibiz/ekodimall/admin/channel-settings/'),false);
  assert.equal(isWorkspaceAdminPathShape('/admin/ekodimall/channel-settings/'),false);
  assert.equal(isWorkspaceAdminPathShape('/ekodimall/admin/channels/'),true);
