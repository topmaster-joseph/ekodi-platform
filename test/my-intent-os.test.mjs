...
test('My EKODI exposes the canonical capability sources without tracked projections',async()=>{
 const registry=await (await worker.fetch(new Request('https://ekodi.kr/capability-registry.json'),env)).json();
 const packs=await (await worker.fetch(new Request('https://ekodi.kr/workspace-packs.json'),env)).json();
...
test('My EKODI Intent OS returns a capability-first plan without executing it',async()=>{
 const request=new Request('https://ekodi.kr/api/intent/plan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text:'상인회 회원 행사와 홍보를 준비해줘',audience:'organization'})});
...
test('Intent OS preserves sovereign gates for high-impact capabilities',async()=>{
 const request=new Request('https://ekodi.kr/api/intent/plan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text:'무역 거래와 계약을 준비해줘',audience:'business'})});
...
test('Intent OS rejects empty or oversized requests before routing',async()=>{const empty=await worker.fetch(new Request('https://ekodi.kr/api/intent/plan',{method:'POST',headers:{'content-type':'application/json'},body:'{}'}),env);assert.equal(empty.status,400);const large=await worker.fetch(new Request('https://ekodi.kr/api/intent/plan',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text:'가'.repeat(1201)})}),env);assert.equal(large.status,400)});
