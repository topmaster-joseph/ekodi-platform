...
test('brand-independent workspace homes receive progressive focus without shared chrome',()=>{
  assert.match(router,/space-storefront'[\s\S]{0,1400}injectEkodiTenantReadability\(routed\)[\s\S]{0,500}injectEkodiProgressiveHome\(branded\)/);
  assert.match(router,/independent-workspace-site'[\s\S]{0,1400}injectEkodiTenantReadability\(routed\)[\s\S]{0,500}injectEkodiProgressiveHome\(branded\)/);
  ...
  assert.doesNotMatch(router,/space-storefront'[\s\S]{0,1400}injectEkodiShell\(routed/);
  assert.doesNotMatch(router,/independent-workspace-site'[\s\S]{0,1400}injectEkodiShell\(routed/);
});
