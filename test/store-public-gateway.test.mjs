...
import { DISCOVERY_PUBLIC_ROUTES } from '../discovery-layer.js';
...
test('cmpmyi is reserved and guarded while stores remains a compatibility route',async()=>{
  const [router,stage,prod]=await Promise.all([read('platform-router-entry-worker.js'),read('.github/workflows/stage-shared-site-shell.yml'),read('.github/workflows/deploy-site-core.yml')]);
  ...
  assert.ok(router.includes('legacyStoreGatewayRedirect(request)'));
  ...
  const discoveryPaths=new Set(DISCOVERY_PUBLIC_ROUTES.map(route=>route.path));
  for(const path of ['/cmpmyi','/jadam','/pizzamaru','/yogurt'])assert.ok(discoveryPaths.has(path),path);
  assert.equal(discoveryPaths.has('/stores'),false);
});
