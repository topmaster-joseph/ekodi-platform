...
html[data-ekodi-site-subject="mnubiz"][data-ekodi-user-ui] body{background:#f7f3ea!important}
...
a{color:inherit}
.mnubiz-shell{width:min(1240px,calc(100% - 40px));margin-inline:auto}
...
html[data-ekodi-site-subject="mnubiz"][data-ekodi-user-ui] body>main.mnubiz-main{width:100%!important;max-width:none!important;margin:0!important}
...
.mnubiz-hero:after{content:"";position:absolute;width:min(520px,54vw);aspect-ratio:1;right:max(-170px,calc((100vw - 1240px)/2 - 120px));top:-190px;border:1px solid rgba(167,122,53,.2);border-radius:50%;box-shadow:0 0 0 54px rgba(167,122,53,.045),0 0 0 108px rgba(13,44,74,.025);pointer-events:none}
...
.mnubiz-section{padding:clamp(42px,5vw,64px) 0;border-bottom:1px solid #d9d1c2!important;background:#f7f3ea!important;color:#10233b!important}
...
.mnubiz-card{display:block;min-height:190px;padding:24px;border-right:1px solid #d9d1c2!important;border-bottom:1px solid #d9d1c2!important;background:#fffdf8!important;color:#10233b!important;text-decoration:none}
...
.mnubiz-card h3{margin:28px 0 8px;font-size:21px;letter-spacing:-.03em;color:#0d2c4a!important}
...
html[data-ekodi-site-subject="mnubiz"] .ekodi-user-ui-footer{margin-top:0!important;background:#0d2c4a!important;color:#fffdf8!important;-webkit-text-fill-color:#fffdf8!important;font-size:12.5px!important}
html[data-ekodi-site-subject="mnubiz"] .ekodi-user-ui-footer__inner{padding:18px 0 20px!important;gap:7px!important}
html[data-ekodi-site-subject="mnubiz"] .ekodi-user-ui-footer :is(a,.ekodi-user-ui-footer__brand,.ekodi-user-ui-footer__business,.ekodi-user-ui-footer__address){color:#fffdf8!important;-webkit-text-fill-color:#fffdf8!important}
html[data-ekodi-site-subject="mnubiz"] .ekodi-user-ui-footer :is(.ekodi-user-ui-footer__copyright,.ekodi-user-ui-footer__scope,.ekodi-user-ui-footer__separator,.ekodi-user-ui-footer__service){color:#d8d3c9!important;-webkit-text-fill-color:#d8d3c9!important;opacity:1!important}
.mnubiz-character-zone{position:absolute;right:max(24px,calc((100vw - 1240px)/2));bottom:42px;width:170px;height:190px;z-index:2;pointer-events:none}
...
@media(max-width:860px){
  .mnubiz-shell{width:min(100% - 28px,1240px)}
...
@media(max-width:520px){
  .mnubiz-shell{width:min(100% - 22px,1240px)}
...
export function renderMnuBizPublicPage(){
  ...
  const html=`<!doctype html><html lang="ko" data-ekodi-ui-surface="user-public" data-ekodi-site-subject="mnubiz" data-ekodi-user-header="default" data-ekodi-user-ai-entry="off"><head>
...
<main class="mnubiz-main" id="main">
  <section class="mnubiz-hero">
    <div class="mnubiz-shell">
      ...
      <div class="mnubiz-actions"><a class="mnubiz-button mnubiz-button--primary" href="#network">동문 네트워크 보기</a><a class="mnubiz-button" href="#news">소식 · 행사 보기</a></div>
      <div class="mnubiz-meta" aria-label="동문회 운영 구조">
        ...
        <div><strong>NEWS & RECORDS</strong><span>소식 · 행사 · 동문 활동 기록</span></div>
      </div>
    </div>
  ...
  <section class="mnubiz-section" id="about">
    <div class="mnubiz-shell">
      <div class="mnubiz-section__head"><div><p class="mnubiz-section__eyebrow">ABOUT</p><h2>국립목포대학교 경영동문회</h2></div><p class="mnubiz-section__desc">동문 소식과 관계, 진로와 사업 경험을 축적하고 필요한 사람과 자연스럽게 연결하는 동문 네트워크를 지향합니다.</p></div>
      <div class="mnubiz-grid">
        <a class="mnubiz-card" href="#network"><span class="mnubiz-card__index">01 · CONNECT</span><h3>동문 연결</h3><p>졸업생과 재학생, 선후배가 필요한 관계를 찾고 서로의 경험을 나눌 수 있도록 연결합니다.</p></a>
        <a class="mnubiz-card" href="#network"><span class="mnubiz-card__index">02 · CAREER</span><h3>진로 · 사업 네트워크</h3><p>취업, 창업, 경영, 협업 등 동문이 가진 현장 경험을 필요한 사람과 연결하는 기반을 만듭니다.</p></a>
        <a class="mnubiz-card" href="#news"><span class="mnubiz-card__index">03 · ARCHIVE</span><h3>행사 · 소식 · 기록</h3><p>모임과 행사, 공지, 동문 소식을 축적하되 개인정보와 비공개 회원 정보는 공개 화면과 분리합니다.</p></a>
      </div>
    </div>
  </section>
  ...
  <section class="mnubiz-section" id="network">
    <div class="mnubiz-shell">
      <div class="mnubiz-section__head"><div><p class="mnubiz-section__eyebrow">NETWORK</p><h2>동문 네트워크</h2></div><p class="mnubiz-section__desc">동문 관계와 참여, 모임과 추천 정보를 경영동문회 범위에서 안전하게 연결하고 관리합니다.</p></div>
      <div class="mnubiz-network">
        <div class="mnubiz-network__primary"><p class="mnubiz-section__eyebrow">ALUMNI NETWORK</p><h3>관계가 필요한 순간에 연결되는 동문회</h3><p>동문 연결, 참여, 모임, 추천과 관계 기능을 한 흐름으로 이어갑니다. 공개 정보와 회원 전용 활동은 권한에 따라 분리합니다.</p><a class="mnubiz-button" href="#news">소식 · 행사 보기</a></div>
  ...
  <section class="mnubiz-section" id="news">
    <div class="mnubiz-shell">
      <div class="mnubiz-section__head"><div><p class="mnubiz-section__eyebrow">NEWS & EVENTS</p><h2>소식 · 행사</h2></div><p class="mnubiz-section__desc">동문회에서 확인한 공지와 일정을 공개하며, 행사 신청과 참여 기록도 같은 흐름에서 연결합니다.</p></div>
