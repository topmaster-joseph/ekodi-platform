(() => {
  ...
  async function load(panel){
    show(panel,'Creator AI 요금제와 구독상태를 불러오는 중입니다.');
    ...
    if(!data.billingReady)show(panel,`요금 설정은 저장할 수 있지만 자동결제 계약키가 준비될 때까지 실제 신규 결제는 시작되지 않습니다. 구독 ${subCount}건 · 현재 유료 AI ${activeCount}명`,'warn');
    else show(panel,`Creator Billing 연결 정상 · 구독 ${subCount}건 · 현재 유료 AI ${activeCount}명`);
  }
  ...
  function install(){
    if(mounted||!token()||document.querySelector('#authorBillingAdmin'))return Boolean(document.querySelector('#authorBillingAdmin'));
    const books=document.querySelector('#booksAdminSection');
    const financePane=books?.querySelector('[data-books-pane="finance"]');
    if(!books||!financePane)return false;
    mounted=true;
    ...
    panel.innerHTML=`
      <div class="author-billing-admin-head">
        <div><small>CREATOR AI · BOOKS BILLING</small><h3>Creator AI 유료회원 요금</h3><p>Books의 Creator AI 구독 가격과 신규 결제 활성 여부를 관리합니다.</p></div>
        <button type="button" class="secondary compact" data-author-billing-refresh>↻ 새로고침</button>
      </div>
      <div class="author-billing-admin-status" data-author-billing-status>요금제와 구독상태를 확인하고 있습니다.</div>
      ...
      <p class="author-billing-admin-note">Creator Billing은 Books 업무에 속합니다. 공통 Finance에는 결제·회계 관제만 유지합니다. 가격 변경은 새 구독부터 적용됩니다.</p>`;
    financePane.append(panel);
  ...
  function tryInstall(){if(!install())setTimeout(install,80);}
  window.addEventListener('ekodi-feature-installed',event=>{if(event.detail?.section==='books')tryInstall();});
  window.addEventListener('ekodi-admin-ready',tryInstall,{once:true});
