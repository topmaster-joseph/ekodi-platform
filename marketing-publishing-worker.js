...
function cors(request, env) {
  ...
  if (!allowed) {
    try {
      ...
      allowed = host === 'ekodi.kr' || host === 'ekodi.kr/marketing' || host === 'ekodi.kr/my' || /^[a-z0-9-]+\.ai\.ekodi\.kr$/i.test(host);
    } catch {}
  }
  const headers = {
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/admin' || url.pathname === '/admin/') return Response.redirect('https://ekodi.kr/admin/?route=marketing-ai&source=ekodi.kr/marketing-publish-api',307);
