...
function runtimeStateFor(service, ageMs, maxEvidenceAgeMs) {
  const declaredState = String(service?.state || '').toLowerCase();
...
function confidenceFor(runtimeState, service, ageMs, maxEvidenceAgeMs) {
  ...
  if (runtimeState === 'operational' || runtimeState === 'degraded' || runtimeState === 'offline') {
...
export function normalizeServiceTruth(service, options = {}) {
  ...
  return {
    ...
    group: service.group || '',
    declaredState: service.state || '',
    monitorEnabled: Boolean(service.monitorEnabled),
    runtimeState,
    confidence: confidenceFor(runtimeState, service, ageMs, maxEvidenceAgeMs),
    evidence: {
      source: 'control_api_service_snapshot',
...
export { DEFAULT_MAX_EVIDENCE_AGE_MS };
