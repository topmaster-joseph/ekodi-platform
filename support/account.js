...
const loginUrl=()=>{
  const target=new URL(config.authUrl||'https://auth.ekodi.kr/?site=support');
  target.searchParams.set('site','support');
  target.searchParams.set('return_to',cleanReturnUrl());
  return target.href;
};
...
function announce(profile,session){
  ...
  window.dispatchEvent(new CustomEvent('ekodi:support-account',{detail:{signedIn:true,displayName,email,status:profile?.profile?.status||'active'}}));
}
...
if(action){
  action.addEventListener('click',async()=>{
    if(state.session&&state.client){
      ...
      window.dispatchEvent(new CustomEvent('ekodi:support-account',{detail:{signedIn:false}}));
      return;
    }
