...
async function continueExecution(text){
 ...
 try{
  const response=await fetch('/api/intent/execute',{method:'POST',headers:{authorization:`Bearer ${token}`,'content-type':'application/json'},body:JSON.stringify({text,audience:audience?.value||'person',workspace_key:workspaceKey})});
