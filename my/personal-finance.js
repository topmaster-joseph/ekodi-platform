...
let state={summary:null,accounts:[],transactions:[],controls:null,insights:[],pendingRows:[],pendingFile:''};
...
function render(){
  ...
  root.innerHTML=`${summaryHtml(s)}${controlsHtml()}<div class="finance-columns"><section><div class="finance-subhead"><h3>내 계좌</h3><span>${state.accounts.length}개</span></div>${accountList()}</section><section><div class="finance-subhead"><h3>최근 거래</h3><span>${state.transactions.length}건</span></div>${transactionList()}</section></div>${formsHtml()}${importHtml()}<div class="finance-subhead"><h3>근거 기반 재무 AI 인사이트</h3><span>읽기·추천 전용</span></div>${insightList()}`;
  bind();
...
async function load(){
  ...
  status('개인재무 불러오는 중');
  try{
    const [summary,accounts,transactions,controls,insights]=await Promise.all([api('/summary'),api('/accounts'),api('/transactions?limit=50'),api('/controls'),api('/insights')]);
    state={...state,summary,accounts:accounts.accounts||[],transactions:transactions.transactions||[],controls,insights:insights.insights||[]};
    render();
