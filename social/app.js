...
async function init(){const res=await fetch('/channels.json',{cache:'no-store'});if(!res.ok)throw new Error('채널 레지스트리를 불러오지 못했습니다.');registry=await res.json();if(!registry.organizations?.some(org=>org.id===activeOrg))activeOrg=registry.organizations?.[0]?.id||'';renderTabs();await activate(activeOrg,false)}
function renderTabs(){els.tabs.innerHTML=registry.organizations.map(org=>`<button type="button" role="tab" tabindex="${org.id===activeOrg?'0':'-1'}" aria-selected="${org.id===activeOrg}" class="org-tab ${org.id===activeOrg?'active':''}" data-org="${esc(org.id)}"><strong>${esc(org.name)}</strong><span>${esc(org.shortName)}</span></button>`).join('');const buttons=[...els.tabs.querySelectorAll('[data-org]')];buttons.forEach((btn,index)=>{btn.addEventListener('click',()=>activate(btn.dataset.org));btn.addEventListener('keydown',event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();let next=index;if(event.key==='ArrowLeft')next=(index-1+buttons.length)%buttons.length;if(event.key==='ArrowRight')next=(index+1)%buttons.length;if(event.key==='Home')next=0;if(event.key==='End')next=buttons.length-1;buttons[next].focus();activate(buttons[next].dataset.org)})})}
function syncUrl(orgId){const url=new URL(location.href);url.searchParams.set('org',orgId);history.replaceState({org:orgId},'',url)}
async function activate(orgId,sync=true){const token=++activationToken;const org=registry.organizations.find(item=>item.id===orgId);if(!org)return;activeOrg=orgId;activeProvider='all';latestPosts=[];if(sync)syncUrl(orgId);renderTabs();els.summary.innerHTML=`<div><p class="eyebrow">${esc(org.shortName)}</p><h2>${esc(org.name)}</h2><p>${esc(org.description)}</p></div><a class="website-link" href="${esc(org.website)}" target="_blank" rel="noopener">공식 홈페이지 ↗</a>`;renderChannels(org);renderFilters(org.channels||[]);hideFeatured();els.feed.innerHTML='';els.status.textContent='최신 콘텐츠를 불러오고 있습니다.';els.status.hidden=false;try{const res=await fetch(`/api/feed?org=${encodeURIComponent(orgId)}`,{headers:{accept:'application/json'}});const payload=await res.json().catch(()=>({}));if(token!==activationToken)return;if(!res.ok)throw new Error(payload.error||'콘텐츠를 불러오지 못했습니다.');latestPosts=Array.isArray(payload.posts)?payload.posts:[];renderFeatured(payload);renderFeed()}catch(error){if(token!==activationToken)return;hideFeatured();els.status.hidden=false;els.status.innerHTML=`<strong>콘텐츠 연동 준비 중</strong><span>${esc(error.message)}</span>`}}
