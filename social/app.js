...
async function init(){
  ...
  renderTabs();
  await activate(activeOrg);
}

function renderTabs(){
  els.tabs.innerHTML=registry.organizations.map(org=>`<button type="button" role="tab" aria-selected="${org.id===activeOrg}" class="org-tab ${org.id===activeOrg?'active':''}" data-org="${esc(org.id)}"><strong>${esc(org.name)}</strong><span>${esc(org.shortName)}</span></button>`).join('');
  els.tabs.querySelectorAll('[data-org]').forEach(btn=>btn.addEventListener('click',()=>activate(btn.dataset.org)));
}

async function activate(orgId){
  activeOrg=orgId;activeProvider='all';latestPosts=[];renderTabs();
  const org=registry.organizations.find(item=>item.id===orgId);if(!org)return;
  ...
  els.feed.innerHTML='';els.status.textContent='최신 콘텐츠를 불러오고 있습니다.';els.status.hidden=false;
  try{
...
function renderFilters(channels){
  ...
  els.filters.innerHTML=items.map(provider=>`<button type="button" class="filter ${provider===activeProvider?'active':''}" data-provider="${esc(provider)}">${provider==='all'?'전체':provider}</button>`).join('');
...
function renderFeed(){
  ...
  if(!posts.length){
    els.feed.innerHTML='';els.status.hidden=false;
    els.status.innerHTML='<strong>표시할 최신 콘텐츠가 없습니다.</strong><span>채널 API가 연결되거나 새 콘텐츠가 확인되면 자동으로 이 영역에 나타납니다.</span>';
    return;
  }
  els.status.hidden=true;
  els.feed.innerHTML=posts.map(post=>`<article class="post-card">
    <a class="thumb" href="${esc(post.url)}" target="_blank" rel="noopener">${post.thumbnail?`<img src="${esc(post.thumbnail)}" alt="" loading="lazy">`:'<span class="thumb-fallback">EKODI</span>'}<span class="provider-badge">${esc(post.provider)}</span></a>
    <div class="post-body"><div class="post-meta"><span>${esc(post.channelName||'EKODI')}</span><time>${esc(fmtDate(post.publishedAt))}</time></div><h3><a href="${esc(post.url)}" target="_blank" rel="noopener">${esc(post.title)}</a></h3>${post.description?`<p>${esc(post.description).slice(0,180)}</p>`:''}</div>
  </article>`).join('');
}
