...
export async function runEkodiCommandQueue(env = {}, options = {}) {
  ...
  for (let index = 0; index < limit; index += 1) {
    const task = requestedTaskId
      ? (index === 0 ? await claimEkodiCommandTask(env, requestedTaskId, { leaseMs: 180000 }) : null)
  ...
  return Object.freeze({
    ...
    targetedTaskId: requestedTaskId || null,
...
async function discoverSystemHealthPulse(env = {}) {
  ...
  return Object.freeze({
    ...
    delegation: Object.freeze({ allowed: true, reversible: true, audited: true, preflightVerified: true, verificationDefined: true }),
...
export async function runEkodiPulseSchedule(env = {}, options = {}) {
  ...
  const observed = Object.freeze({ autonomousHealth, readiness, ledger: preQueueLedger });
  const previousObserved = previousAutonomousHealth
    ? Object.freeze({ autonomousHealth: previousAutonomousHealth, readiness, ledger: preQueueLedger })
    : null;
...
export const EKODI_PULSE_RUNTIME = Object.freeze({
  ...
  automaticTriggers: Object.freeze(['telemetry-snapshot', 'desired-state-drift', 'queued-command-task', 'degraded-system-health', 'maintenance-signal-evaluation']),
