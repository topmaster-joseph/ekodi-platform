...
function menuCard(item, meta, channels, index) {
  ...
  const listingHtml = listings.slice(0, 4).map((row) => `<div class="rs-price-row"><span>${escapeHtml(row.label)}</span><strong>${row.price != null ? escapeHtml(won(row.price)) : '가격 확인'}</strong>${row.order_url ? `<a href="${escapeHtml(row.order_url)}" target="_blank" rel="noopener noreferrer">주문</a>` : '<i></i>'}</div>`).join('');
...
function orderCards(channels, meta, phone) {
  ...
  return `${official ? `<a class="rs-order-card official" href="${escapeHtml(official)}" target="_blank" rel="noopener noreferrer"><i>공</i><span><b>${escapeHtml(meta.brand)} 공식 주문</b><small>공식 주문 화면에서 매장을 확인하세요</small></span><strong>→</strong></a>` : ''}${phone ? `<a class="rs-order-card phone" href="${escapeHtml(telUrl(phone))}"><i>☎</i><span><b>전화 주문</b><small>${escapeHtml(phone)}</small></span><strong>→</strong></a>` : ''}`;
}
...
export async function renderRestaurantStorefrontPage(request, env, resolved, slug) {
  ...
  const chromeHeader = chrome.header || {};
  const chromeFooter = chrome.footer || {};
  ...
  const menu = (Array.isArray(data?.menu) ? data.menu : []).filter((item) => item?.name).slice(0, 12);
  const channels = normalizeChannels(data?.channels);
  const image = heroImage(menu, meta, channels);
  ...
  const menuHtml = menu.length ? menu.slice(0, 6).map((item, index) => menuCard(item, meta, channels, index)).join('') : `<div class="rs-info-card" style="grid-column:1/-1"><h3>목포대점 메뉴를 연결하고 있습니다.</h3><p style="margin:0;color:var(--muted);font-size:10px;line-height:1.6">확인되지 않은 메뉴명이나 가격을 임의로 만들지 않습니다. 본사 메뉴 또는 주문 앱에서 현재 판매 메뉴를 확인해 주세요.</p></div>`;
...
<section class="rs-main"><div class="rs-shell"><div id="menu" class="rs-section-head"><h2>대표 메뉴</h2><p>${escapeHtml(meta.brand)} 목포대점에서 확인된 메뉴와 앱별 가격을 함께 보여줍니다. 쿠폰·배달비·옵션·실시간 판매 여부는 각 주문 앱의 최종 화면이 기준입니다.</p></div><div class="rs-showcase"><aside class="rs-promo"><strong>${escapeHtml(meta.heroLine)}</strong><p>메뉴와 가격, 지도와 주문 경로를 여러 화면에서 헤매지 않도록 한 페이지로 정리했습니다.</p><div class="rs-chips">${categoryChips(menu, meta)}</div></aside><div class="rs-menu">${menuHtml}</div><aside id="order" class="rs-order-side"><h3>배달앱 주문</h3><p>목포대점으로 확인된 주문 링크를 우선합니다.</p>${orderHtml || '<div class="rs-order-foot">검증된 주문 링크를 연결하고 있습니다.</div>'}<div class="rs-order-foot">앱 가격·쿠폰·배달비·영업상태는 주문 직전 각 앱에서 다시 확인해 주세요.</div></aside></div>
<div id="store" class="rs-info"><article class="rs-info-card"><h3>${escapeHtml(meta.brand)} ${escapeHtml(meta.branch)} 안내</h3><div class="rs-info-grid"><div class="rs-info-item"><small>매장 위치</small><b>${escapeHtml(address || '매장 주소를 확인하고 있습니다.')}</b></div><div class="rs-info-item"><small>전화</small>${phone ? `<a href="${escapeHtml(telUrl(phone))}">${escapeHtml(phone)}</a>` : '<b>전화번호 확인 중</b>'}</div><div class="rs-info-item"><small>영업시간</small><b>${escapeHtml(hours || '영업시간은 전화로 확인해 주세요.')}</b></div><div class="rs-info-item"><small>길찾기</small>${map ? `<a href="${escapeHtml(map)}" target="_blank" rel="noopener noreferrer">네이버지도에서 보기 →</a>` : '<b>위치 확인 중</b>'}</div></div></article><article class="rs-info-card"><h3>EKODI 검증 원칙</h3><div class="rs-trust"><div><b>매장 단위 데이터</b><small>${escapeHtml(meta.brand)} 목포대점 데이터만 분리해 사용합니다.</small></div><div><b>확인되지 않은 값은 숨김</b><small>임의 메뉴·임의 가격 대신 검증 완료 전 상태를 명확히 표시합니다.</small></div><div><b>주문 단계 재확인</b><small>최종 판매가격과 주문 가능 여부는 배달앱 또는 매장 확인을 기준으로 합니다.</small></div></div></article></div></div></section>
<footer class="rs-footer"><b>${escapeHtml(chromeFooter.brand||title)}</b><small>${escapeHtml(chromeFooter.operator?.name||meta.category)} · ${escapeHtml(chromeFooter.copyright||'메뉴 · 가격 · 주문 연결')}</small></footer></main><nav class="rs-mobile"><a href="#menu">메뉴</a>${phone ? `<a href="${escapeHtml(telUrl(phone))}">전화</a>` : '<a href="#store">매장</a>'}${map ? `<a href="${escapeHtml(map)}" target="_blank" rel="noopener noreferrer">지도</a>` : '<a href="#store">안내</a>'}<a href="${escapeHtml(mobileOrder)}"${mobileOrder.startsWith('http') ? ' target="_blank" rel="noopener noreferrer"' : ''}>주문</a></nav></body></html>`;
