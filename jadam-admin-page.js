const JADAM_STORE_ID='4b1e5933-b9ae-4cb9-9d31-dcbb0a5b25aa';
...
function clientMain(){
  const STORE_ID='4b1e5933-b9ae-4cb9-9d31-dcbb0a5b25aa';
  const SUPABASE_URL='https://renzehysxirjilvdxacv.supabase.co';
  const SUPABASE_KEY='sb_publishable_0QjB0WzZbjrd-FJ5D5cR7A_xUkXyOY_';
  const API='https://api.ekodi.kr';
  const SESSION_KEY='ekodi-jadam-admin-session';
  ...
  const META={
    ...
    marketing:['마케팅','자담치킨 전용 Marketing AI 운영공간으로 연결합니다.'],
  ...
  function setup(){
    $('pageTitle').textContent=page[0];$('pageCopy').textContent=page[1];document.title=`${page[0]} · 자담치킨 목포대점`;
    for(const [key,label] of NAV){const a=document.createElement('a');a.href=key==='overview'?'/jadam/admin':`/jadam/admin/${key}`;a.textContent=label;if(key===section)a.classList.add('active');$('adminNav').append(a)}
  }
  ...
  function loginPanel(message='자담치킨 목포대점 운영 권한으로 로그인해야 합니다.'){
    $('summaryCards').innerHTML=[card('운영공간','자담치킨 목포대점','store-scoped'),card('데이터 경계','점포 전용','다른 매장과 분리')].join('');
  ...
  function summary(){
    ...
    $('summaryCards').innerHTML=[card('오늘 매출',won(m.sales),'완료 주문 집계'),card('오늘 주문',num(o.count),'개인정보 미노출'),card('재방문율',m.repeatRate==null?'—':`${m.repeatRate}%`,'목표 '+(m.targetRepeatRate??45)+'%'),card('메뉴 원장',num(menu.canonicalItems||0),`연결 ${connectedCount()}개`)].join('');
  }
  function overview(){
  ...
  function marketingPanel(){$('mainPanel').innerHTML=`<h2>자담치킨 Marketing AI</h2><p class="panel-lead">피자마루·요거트퍼플과 분리된 자담치킨 전용 마케팅 운영공간입니다.</p><div class="actions"><a class="button primary" href="/jadam/marketing">전용 Marketing AI 열기</a></div>`}
  ...
  function render(){summary();({overview,menu:menuPanel,orders:ordersPanel,customers:customerPanel,reviews:reviewsPanel,sales:salesPanel,inventory:inventoryPanel,marketing:marketingPanel,work:workPanel,finance:financePanel,connections:connectionsPanel}[section]||overview)();setState('자담 점포 운영','live')}
  async function loadData(){
    setState('실데이터 확인 중');
    const [snapshot,menu,connector]=await Promise.all([
      rpc('business_os_store_admin_snapshot',{p_workspace_key:'jadam'}),
      control(`/api/store/menu?store=${encodeURIComponent(STORE_ID)}`),
  ...
  async function boot(){
    ...
    try{await loadData()}catch(e){if(e.status===401)return loginPanel();if(e.status===403)return loginPanel('이 계정에는 자담치킨 목포대점 운영 권한이 없습니다.');$('summaryCards').innerHTML=[card('운영공간','자담치킨 목포대점','데이터 확인 필요')].join('');$('mainPanel').innerHTML=`<div class="empty"><strong>실데이터 연결 확인 필요</strong>${esc(e.message)}</div>`;setState('확인 필요','warn')}
  }
  $('refreshData').onclick=()=>loadData().catch(e=>{setState('확인 필요','warn');$('pageCopy').textContent=e.message});
  $('logout').onclick=()=>{clearSession();location.assign('/jadam/admin')};
  boot();
}

export function isJadamAdminPath(pathname){return /^\/jadam\/admin(?:\/(?:overview|menu|orders|customers|reviews|sales|inventory|marketing|work|finance|connections))?\/?$/i.test(String(pathname||''))}
export function jadamAdminCss(){return new Response(CSS,{headers:{'content-type':'text/css; charset=utf-8','cache-control':'public, max-age=300','x-content-type-options':'nosniff'}})}
export function jadamAdminScript(){return new Response(`(${clientMain.toString()})();`,{headers:{'content-type':'text/javascript; charset=utf-8','cache-control':'no-store','x-content-type-options':'nosniff'}})}
export function jadamAdminPage(){
  const html='<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="robots" content="noindex,nofollow"><title>자담치킨 목포대점 관리자</title><link rel="stylesheet" href="/jadam-admin.css"></head><body><header class="topbar"><a class="brand" href="/jadam/admin"><span class="mark">JD</span><span class="brand-copy"><strong>자담치킨 목포대점</strong><small>STORE OPERATIONS</small></span></a><div class="top-actions"><a href="/jadam/marketing">마케팅</a><button id="refreshData" type="button">새로고침</button><button id="logout" type="button">로그아웃</button></div></header><div class="layout"><aside class="sidebar"><div class="scope"><small>독립 점포 운영공간</small><strong>자담치킨 목포대점</strong><span>JADAM CHICKEN</span></div><nav id="adminNav"></nav><p class="boundary">피자마루 목포대점과 요거트퍼플 목포대점은 별도 운영공간입니다. 공통 엔진만 공유하고 데이터·권한은 공유하지 않습니다.</p></aside><main><section class="heading"><div><p class="eyebrow">JADAM STORE ADMIN</p><h1 id="pageTitle">운영 홈</h1><p id="pageCopy">점포 운영 데이터를 확인합니다.</p></div><span class="state" id="pageState">확인 중</span></section><section class="cards" id="summaryCards"></section><section class="panel" id="mainPanel"><div class="empty"><strong>운영 데이터를 불러오고 있습니다.</strong>점포 권한과 실제 연결 상태를 확인합니다.</div></section></main></div><script src="/jadam-admin.js?v=20260906-store-admin" defer></script></body></html>';
  return new Response(html,{headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store','x-content-type-options':'nosniff','x-frame-options':'DENY','referrer-policy':'strict-origin-when-cross-origin','content-security-policy':"default-src 'self'; style-src 'self'; script-src 'self'; connect-src 'self' https://api.ekodi.kr https://renzehysxirjilvdxacv.supabase.co; frame-ancestors 'none'; base-uri 'self'; form-action 'self'",'x-ekodi-route':'jadam-store-admin','x-ekodi-store-scope':JADAM_STORE_ID}});
}
