...
Treat session termination, execution-window limits, temporary tool/connector failures, and rate limits as a recoverable interruption rather than completion. Preserve a checkpoint containing task/branch/commit identity, completed and pending work, validation/deployment state, blocking dependency, and next executable step; resume from that checkpoint and try authorized fallback paths before escalation. Do not widen authority or weaken production verification.
