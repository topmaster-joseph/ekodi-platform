import { recordFreeProviderOutcomeAndAlert, reserveFreeDailyRequest } from './ai-free-quota.js';
  ...
  return Object.freeze({
    ...
    async invoke({prompt='' }={}){
      ...
      try{
        ...
        const quota=Object.freeze({remainingRequests:headerNumber(response.headers,'x-ratelimit-remaining-requests')??reservation.remaining,remainingTokens:headerNumber(response.headers,'x-ratelimit-remaining-tokens'),resetAt:reservation.resetAt});
        await recordFreeProviderOutcomeAndAlert(env,'groq-free',{ok:true,quota}).catch(()=>{});
        return Object.freeze({text:output,model,quota});
      }catch(error){
        await recordFreeProviderOutcomeAndAlert(env,'groq-free',{ok:false,error}).catch(()=>{});
        throw error;
      }
    }
  });
