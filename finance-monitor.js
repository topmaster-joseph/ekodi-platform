...
// Policy Fund Management: discovery -> eligibility -> application -> execution -> extension/refinance -> close.
...
function ensurePolicyFundStyles() {
  ...
  style.textContent = `
    ...
    .policy-fund-head h3{margin:3px 0 5px;font-size:1.3rem}.policy-fund-head p{margin:0;max-width:760px;opacity:.75}
    ...
    .policy-fund-toolbar{display:flex;gap:8px;align-items:end;flex-wrap:wrap;margin:14px 0}.policy-fund-toolbar label{display:grid;gap:4px;min-width:220px}
    ...
    .policy-fund-table-title{margin:22px 0 8px}.policy-fund-panel .finance-table td{vertical-align:top}
...
function ensurePolicyFundPanel() {
  ...
  panel.innerHTML = `
    <div class="policy-fund-head"><div><small>POLICY FUND · LIFECYCLE</small><h3>소상공인 정책자금 · 연장관리</h3><p>지원사업 발견부터 적격검토, 서류, 신청, 실행 대출, 만기·상환연장·대환과 사후관리까지 사업자별로 이어서 관리합니다.</p></div><button class="secondary" id="policyFundRefresh" type="button">↻ 정책자금 새로고침</button></div>
    <div class="policy-fund-grid"><article><small>확보 준비</small><strong id="policyPreparing">0</strong><span>적격후보 · 준비</span></article><article><small>신청 진행</small><strong id="policySubmitted">0</strong><span>접수 · 보완</span></article><article><small>실행 잔액</small><strong id="policyBalance">₩0</strong><span>정책자금 대출</span></article><article><small>90일 내 만기</small><strong id="policyDue90">0</strong><span>연장·대환 사전점검</span></article><article><small>연장관리</small><strong id="policyExtensions">0</strong><span>열린 연장·대환 건</span></article></div>
    <div class="finance-note good policy-fund-source" id="policyOfficialSource"><span>최신 공식 공고를 확인하는 중입니다.</span></div>
    <div class="policy-fund-toolbar"><label>사업자/조직 ID<input id="policyOrgFilter" placeholder="전체 또는 EKODIBIZ 등"></label><button class="secondary" id="policyOrgApply" type="button">사업자별 보기</button><span id="policyFundGenerated"></span></div>
    <div class="policy-fund-forms">
      <details><summary>자금 신청 건 등록</summary><form class="policy-fund-form" id="policyApplicationForm"><label>사업자/조직 ID<input name="organizationId" required></label><label>정책자금<select name="programId" id="policyProgramSelect" required></select></label><label>신청 희망액<input name="requestedAmount" type="number" min="0" step="10000"></label><label>다음 행동<input name="nextAction" placeholder="서류 준비, 접수 등"></label><label>기한<input name="nextActionDue" type="date"></label><button class="primary" type="submit">준비 건 등록</button></form></details>
      <details><summary>실행 대출 등록</summary><form class="policy-fund-form" id="policyLoanForm"><label>사업자/조직 ID<input name="organizationId" required></label><label>취급기관<input name="lender" placeholder="소진공/은행 등"></label><label>실행액<input name="principal" type="number" min="0" step="10000" required></label><label>현재 잔액<input name="balance" type="number" min="0" step="10000"></label><label>금리(%)<input name="annualRate" type="number" min="0" max="100" step="0.01"></label><label>만기일<input name="maturityOn" type="date" required></label><button class="primary" type="submit">대출 등록 · 만기알림 생성</button></form></details>
      <details><summary>연장 · 대환 관리 등록</summary><form class="policy-fund-form" id="policyExtensionForm"><label>대출<select name="loanId" id="policyLoanSelect" required></select></label><label>유형<select name="kind"><option value="repayment-extension">상환연장</option><option value="maturity-extension">만기연장</option><option value="refinance">대환</option><option value="deferment">상환유예</option><option value="other">기타</option></select></label><label>사유/확인사항<input name="reason"></label><label>변경 예정 만기<input name="newMaturityOn" type="date"></label><button class="primary" type="submit">검토 건 등록</button></form></details>
    </div>
    <p class="policy-fund-message" id="policyFundMessage" role="status"></p>
    <h4 class="policy-fund-table-title">확보 · 신청 파이프라인</h4><div class="finance-table-wrap"><table class="finance-table"><thead><tr><th>사업자</th><th>정책자금</th><th>상태</th><th>적격</th><th class="right">희망/승인액</th><th>다음 행동</th></tr></thead><tbody id="policyApplicationRows"></tbody></table></div>
    <h4 class="policy-fund-table-title">실행 대출 · 만기</h4><div class="finance-table-wrap"><table class="finance-table"><thead><tr><th>사업자</th><th>취급기관</th><th class="right">잔액</th><th>금리</th><th>만기</th><th>상태</th></tr></thead><tbody id="policyLoanRows"></tbody></table></div>
  ...
  document.querySelector('#policyOrgApply')?.addEventListener('click', () => loadPolicyFundManagement(true));
  bindPolicyFundForms();
}
...
function renderPolicyFundData(overview, programs, portfolio) {
  document.querySelector('#policyPreparing').textContent = String(overview.applications.preparing || 0);
  ...
  if (latest) {
    ...
    text.textContent = `${latest.title} · ${latest.noticeNumber || ''} ${latest.revisionLabel || ''} · 공식원본 확인 ${financeDate(latest.sourceCheckedAt)}`;
    sourceBox.append(text);
  ...
  if (loanSelect) {
    ...
    loanSelect.replaceChildren(...(portfolio.loans || []).map(item => {
      const option = document.createElement('option'); option.value = item.id; option.textContent = `${item.organizationId} · ${item.lender || '정책자금'} · ${item.maturityOn || '만기미정'}`; return option;
    }));
  ...
  renderPolicyFundRows('#policyApplicationRows',(portfolio.applications || []).map(item => [
    item.organizationId,item.programTitle,policyFundStatus(item.status),policyFundStatus(item.eligibilityStatus),
  ...
  renderPolicyFundRows('#policyLoanRows',(portfolio.loans || []).map(item => [
    item.organizationId,item.lender || '—',{ right:true,text:financeKRW(item.balance || 0)},item.annualRate == null ? '—' : `${item.annualRate}%`,item.maturityOn || '—',policyFundStatus(item.status)
  ]),6,'등록된 정책자금 실행 대출이 없습니다.');
  renderPolicyFundRows('#policyExtensionRows',(portfolio.extensions || []).map(item => [
    item.loanId,policyFundStatus(item.kind),policyFundStatus(item.eligibilityStatus),policyFundStatus(item.status),`${item.originalMaturityOn || '—'} / ${item.newMaturityOn || '—'}`,item.reason || '—'
  ]),6,'등록된 연장·대환 관리 건이 없습니다.');
  renderPolicyFundRows('#policyTaskRows',(portfolio.tasks || []).map(item => [
    item.organizationId,item.title,item.dueOn || '—',item.priority,policyFundStatus(item.status)
  ]),5,'예정된 정책자금 관리 업무가 없습니다.');
}
...
async function loadPolicyFundManagement(force = false) {
  ...
  try {
    const organizationId = document.querySelector('#policyOrgFilter')?.value.trim() || '';
    const suffix = organizationId ? `?organizationId=${encodeURIComponent(organizationId)}` : '';
    const [overview, programData, portfolio] = await Promise.all([
      policyFundRequest('/overview'), policyFundRequest('/programs'), policyFundRequest(`/portfolio${suffix}`)
    ]);
    renderPolicyFundData(overview, programData.programs || [], portfolio);
    policyFundLastLoadedAt = Date.now();
    policyFundMessage('');
  } catch (error) {
    policyFundMessage(`정책자금 관리 연결을 확인해야 합니다: ${error.message}`,'error');
  } finally {
...
function bindPolicyFundForms() {
  ...
  applicationForm?.addEventListener('submit', async event => {
    event.preventDefault(); const values = Object.fromEntries(new FormData(applicationForm));
    try {
      await policyFundRequest('/applications',{ method:'POST',body:JSON.stringify({ ...values,status:'preparing',eligibilityStatus:'needs-review',requestedAmount:values.requestedAmount || null }) });
      policyFundMessage('정책자금 준비 건을 등록했습니다. 공식 요건 확인 후 적격상태를 갱신하세요.','good'); applicationForm.reset(); await loadPolicyFundManagement(true);
    } catch (error) { policyFundMessage(error.message,'error'); }
  });
  loanForm?.addEventListener('submit', async event => {
    event.preventDefault(); const values = Object.fromEntries(new FormData(loanForm));
    try {
      await policyFundRequest('/loans',{ method:'POST',body:JSON.stringify({ ...values,status:'repaying',balance:values.balance || values.principal,annualRate:values.annualRate || null }) });
      policyFundMessage('실행 대출을 등록했습니다. 만기 D-90·60·30·14·7 관리업무가 자동 생성됩니다.','good'); loanForm.reset(); await loadPolicyFundManagement(true);
    } catch (error) { policyFundMessage(error.message,'error'); }
  });
  extensionForm?.addEventListener('submit', async event => {
    event.preventDefault(); const values = Object.fromEntries(new FormData(extensionForm));
    try {
      await policyFundRequest('/extensions',{ method:'POST',body:JSON.stringify({ ...values,status:'candidate',eligibilityStatus:'needs-review',newMaturityOn:values.newMaturityOn || null }) });
      policyFundMessage('연장·대환 검토 건을 등록했습니다. 공식 제도와 현재 채무상태를 확인해 적격여부를 확정하세요.','good'); extensionForm.reset(); await loadPolicyFundManagement(true);
    } catch (error) { policyFundMessage(error.message,'error'); }
  });
}
