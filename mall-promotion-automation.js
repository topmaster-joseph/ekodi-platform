...
function slug(value) {
  return clean(value, 160).toLowerCase().replace(/[^a-z0-9가-힣_-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 120);
}
...
export function campaignKey(runDate, provider, productRowId) {
  return `mall-${String(runDate).replaceAll('-', '')}-${slug(provider)}-${Number(productRowId)}`;
}
function parseJsonObject(text) {
  const source = clean(text, 12_000);
...
async function schemaReady(env) {
  ...
  try {
    const result = await env.DB.prepare(`SELECT name FROM sqlite_master WHERE type='table' AND name IN (
      'affiliate_storefront_products','affiliate_storefront_clicks','affiliate_promotion_runs','affiliate_promotion_visits',
      'marketing_oauth_connections','marketing_publish_channels','marketing_content_items','marketing_publication_jobs',
      'marketing_publish_policies','service_subscriptions'
    )`).all();
...
async function autonomyGate(env) {
  const [policy, subscription] = await Promise.all([
    env.DB.prepare(`SELECT mode,max_daily_posts FROM marketing_publish_policies WHERE subject_type=? AND subject_key=?`)
      .bind(SUBJECT_TYPE, SUBJECT_KEY).first(),
    env.DB.prepare(`SELECT plan_id,status FROM service_subscriptions WHERE subject_type=? AND subject_key=? AND site='marketing'`)
      .bind(SUBJECT_TYPE, SUBJECT_KEY).first(),
  ]);
  ...
  return {
    allowed: policyReady && entitlementReady,
    policy: policy?.mode || 'missing',
    plan: subscription?.plan_id || 'missing',
    reason: !policyReady ? 'AUTONOMOUS_POLICY_REQUIRED' : !entitlementReady ? 'MARKETING_AUTO_ENTITLEMENT_REQUIRED' : '',
  };
}
...
async function candidateProducts(env, limit = 6) {
  const result = await env.DB.prepare(`SELECT p.id,p.product_id,p.product_name,p.price_krw,p.category,p.selection_score,p.is_rocket,p.is_free_shipping,
      COALESCE(SUM(c.clicks),0) AS clicks_7d
    FROM affiliate_storefront_products p
    LEFT JOIN affiliate_storefront_clicks c ON c.product_row_id=p.id AND c.click_date >= date('now','-6 day')
    WHERE p.account_id=? AND p.storefront_slug=? AND p.status='active'
    GROUP BY p.id
    ORDER BY clicks_7d DESC, p.selection_score DESC, p.id DESC
...
export function fallbackContent(product, provider) {
  ...
  const rocket = Number(product?.is_rocket || 0) ? '로켓배송 여부도 함께 확인해 보세요.' : '';
  const shipping = Number(product?.is_free_shipping || 0) ? '무료배송 여부도 상품 페이지에서 확인할 수 있습니다.' : '';
  const hooks = {
    facebook: `오늘의 ${category} 추천: ${name}`,
    instagram: `${category} 고를 때 눈여겨볼 오늘의 한 가지`,
    threads: `오늘의 실용 아이템 하나: ${name}`,
  };
  const bodies = {
    facebook: `${name}\n필요한 기능과 현재 조건을 한 번에 비교해 보세요. ${rocket} ${shipping}`,
    instagram: `${name}\n구매 전 가격·배송·상품 정보를 다시 확인하고, 내 상황에 맞는지 비교해 보세요.`,
    threads: `${name}\n필요한 사람에게는 꽤 실용적인 선택지가 될 수 있습니다. 구매 전 최신 가격과 배송조건은 꼭 다시 확인하세요.`,
  };
  return {
    title: hooks[provider] || hooks.facebook,
    caption: `${bodies[provider] || bodies.facebook}\n\n에코디몰에서 자세히 보기\n\n${AFFILIATE_DISCLOSURE}`.replace(/\s+\n/g, '\n').trim(),
    mode: 'rules',
  };
}
...
async function aiContent(env, product, provider) {
  ...
  const message = [
    '에코디몰의 유기적 SNS 홍보문을 작성하세요.',
    `채널: ${provider}`,
    `상품명: ${clean(product.product_name, 180)}`,
    `카테고리: ${clean(product.category, 60)}`,
    `현재 관측 가격: ${Number(product.price_krw || 0)}원 (가격은 변동 가능하므로 본문에서 확정 가격을 약속하지 마세요.)`,
    `최근 7일 에코디몰 클릭: ${Number(product.clicks_7d || 0)}`,
    '과장, 최저가·효능·품질 보장, 허위 후기, 긴급 구매 압박을 금지합니다.',
    '광고 문구보다 구매 판단에 도움되는 짧은 정보형 문구를 우선합니다.',
    'URL과 해시태그는 넣지 마세요.',
    'JSON만 반환하세요: {"title":"80자 이내","caption":"700자 이내"}',
  ].join('\n');
  try {
    const result = await ai.invoke({
      taskName: 'ekodi-mall-organic-promotion',
      context: { message, page: { section:'marketing', title:'EKODI Mall organic promotion', pathname:'/mall' } },
    });
    const parsed = parseJsonObject(result.text);
    const title = clean(parsed?.title, 120);
    const caption = clean(parsed?.caption, 900);
    if (!title || !caption) return fallback;
    return {
      title,
      caption: `${caption}\n\n에코디몰에서 자세히 보기\n\n${AFFILIATE_DISCLOSURE}`,
      mode: 'ai',
      model: clean(result.model, 120),
    };
  } catch (error) {
    console.error('EKODI Mall promotion AI fallback', String(error?.message || error));
    return fallback;
  }
}
...
function providerSecret(env, provider) {
  if (provider === 'threads') return String(env.THREADS_APP_SECRET || env.META_APP_SECRET || '');
...
async function fetchJson(url, init = {}) {
  ...
  if (!response.ok || data?.error) throw new Error(clean(data?.error?.message || data?.message || `HTTP_${response.status}`, 500));
  return data;
}
...
async function publishFacebook(env, connection, content) {
  const token = await decryptToken(env, connection.provider, connection.token_ciphertext);
...
async function publishInstagram(env, connection, content) {
  const token = await decryptToken(env, connection.provider, connection.token_ciphertext);
  const create = new URLSearchParams({image_url:content.imageUrl, caption:`${content.caption}\n\n${content.linkUrl}`, access_token:token});
  const container = await fetchJson(`${graphBase(env)}/${encodeURIComponent(connection.external_id)}/media`, {method:'POST', body:create});
  if (!container.id) throw new Error('INSTAGRAM_CONTAINER_FAILED');
  const publish = new URLSearchParams({creation_id:String(container.id), access_token:token});
  const data = await fetchJson(`${graphBase(env)}/${encodeURIComponent(connection.external_id)}/media_publish`, {method:'POST', body:publish});
  let permalink = '';
  if (data.id) {
    const detail = new URL(`${graphBase(env)}/${data.id}`);
    detail.searchParams.set('fields','permalink');
    detail.searchParams.set('access_token',token);
    const info = await fetchJson(detail.href).catch(() => ({}));
    permalink = safeUrl(info.permalink);
  }
  return {id:String(data.id || ''), url:permalink, data};
}
async function publishThreads(env, connection, content) {
  const token = await decryptToken(env, connection.provider, connection.token_ciphertext);
  const form = new URLSearchParams({
    media_type:'IMAGE',
    image_url:content.imageUrl,
    text:`${clean(content.caption, 420)}\n\n${content.linkUrl}`,
    access_token:token,
  });
  ...
  const publish = new URLSearchParams({creation_id:String(container.id),access_token:token});
  const data = await fetchJson(`https://graph.threads.net/v1.0/${encodeURIComponent(connection.external_id)}/threads_publish`, {method:'POST',body:publish});
  let permalink = '';
  if (data.id) {
    const detail = new URL(`https://graph.threads.net/v1.0/${data.id}`);
    detail.searchParams.set('fields','permalink');
    detail.searchParams.set('access_token',token);
    const info = await fetchJson(detail.href).catch(() => ({}));
    permalink = safeUrl(info.permalink);
...
async function executeProvider(env, connection, content) {
  if (connection.provider === 'facebook') return publishFacebook(env,connection,content);
  if (connection.provider === 'instagram') return publishInstagram(env,connection,content);
  if (connection.provider === 'threads') return publishThreads(env,connection,content);
...
async function ensurePublishChannel(env, connection) {
  ...
  await env.DB.prepare(`INSERT INTO marketing_publish_channels(subject_type,subject_key,provider,channel_type,display_name,external_account_id,credential_ref,status,config_json,last_check_at,last_error,created_at,updated_at)
    ...
    .bind(SUBJECT_TYPE,SUBJECT_KEY,connection.provider,channelType,clean(connection.display_name,120),clean(connection.external_id,160),safeJson({credentialMode:'oauth-vault',oauthConnectionId:Number(connection.id)}),now,now,now).run();
  const row = await env.DB.prepare(`SELECT id FROM marketing_publish_channels WHERE subject_type=? AND subject_key=? AND provider=? AND channel_type=? AND external_account_id=?`)
    .bind(SUBJECT_TYPE,SUBJECT_KEY,connection.provider,channelType,connection.external_id).first();
  return Number(row?.id || 0);
}

async function insertMarketingLedger(env, connection, content, publishResult) {
  const channelId = await ensurePublishChannel(env, connection);
  if (!channelId) return;
  const now = nowIso();
  const item = await env.DB.prepare(`INSERT INTO marketing_content_items(subject_type,subject_key,title,content_type,caption,asset_url,link_url,content_json,source,approval_state,created_by,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?, 'ai','auto_approved','promotion-ai',?,?)`)
    .bind(SUBJECT_TYPE,SUBJECT_KEY,clean(content.title,240),'social_post',clean(content.caption,12000),content.imageUrl,content.linkUrl,safeJson({campaignKey:content.campaignKey,storefront:STOREFRONT,productRowId:content.productRowId}),now,now).run();
  const contentId = Number(item.meta?.last_row_id || 0);
  if (!contentId) return;
  await env.DB.prepare(`INSERT INTO marketing_publication_jobs(subject_type,subject_key,content_id,channel_id,schedule_kind,scheduled_at,recurrence_rule,status,requested_by,attempt_count,max_attempts,external_post_id,external_post_url,provider_response_json,last_error,published_at,created_at,updated_at)
    VALUES(?,?,?,?,'immediate',?,'','published','ai',1,1,?,?,?,'',?,?,?)`)
    .bind(SUBJECT_TYPE,SUBJECT_KEY,contentId,channelId,now,clean(publishResult.id,240),safeUrl(publishResult.url),safeJson({ok:true,provider:connection.provider}),now,now,now).run();
}

async function upsertRun(env, values) {
  const now = nowIso();
  await env.DB.prepare(`INSERT INTO affiliate_promotion_runs(run_date,product_row_id,product_id,provider,connection_id,campaign_key,status,ai_mode,ai_model,content_json,external_post_id,external_post_url,last_error,published_at,created_at,updated_at)
    VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
...
export async function getMallPromotionStatus(env) {
  ...
  const [gate, connections, latest, today] = await Promise.all([
    autonomyGate(env),
    activeConnections(env),
    env.DB.prepare(`SELECT run_date,status,provider,published_at,last_error,updated_at FROM affiliate_promotion_runs ORDER BY id DESC LIMIT 1`).first().catch(() => null),
  ...
  return {
    ...
    subject:`${SUBJECT_TYPE}:${SUBJECT_KEY}`,
    channels:connections.map(row => row.provider),
    gate,
    today:{planned:Number(today?.planned || 0),published:Number(today?.published || 0),failed:Number(today?.failed || 0)},
    lastRun:latest ? {date:latest.run_date,status:latest.status,provider:latest.provider,publishedAt:latest.published_at || null,error:latest.last_error || '',updatedAt:latest.updated_at} : null,
  };
}
...
export async function runMallPromotionAutomation(env, {reason='cron', force=false} = {}) {
  ...
  if (!gate.allowed) {
    for (const connection of connections) {
      const products = await candidateProducts(env,1);
      const product = products[0];
      if (!product) break;
      await upsertRun(env,{runDate:kst.date,productRowId:product.id,productId:product.product_id,provider:connection.provider,connectionId:connection.id,campaignKey:campaignKey(kst.date,connection.provider,product.id),status:'approval_required',aiMode:'rules',content:{reason},lastError:gate.reason});
    }
  ...
  for (let index = 0; index < connections.length; index += 1) {
    ...
    const product = chooseProduct(products,recent,index);
    if (!product) continue;
    ...
    const content = {
      ...
      imageUrl,
      linkUrl,
      campaignKey:keyValue,
      productRowId:Number(product.id),
      productId:clean(product.product_id,100),
    };
    ...
    try {
      ...
      results.push({provider:connection.provider,status:'published',campaignKey:keyValue,externalPostUrl:safeUrl(published.url)});
  ...
  return {ok:true,status:results.some(row => row.status === 'published') ? 'ran' : results.length ? 'failed' : 'already_done',runDate:kst.date,reason,results};
}

export async function handleMallPromotionRequest(request, env) {
  const url = new URL(request.url);
  const match = url.pathname.match(/^\/r\/mall\/([a-z0-9가-힣_-]{8,160})$/i);
  ...
  await env.DB.prepare(`INSERT INTO affiliate_promotion_visits(campaign_key,visit_date,visits,updated_at) VALUES(?,?,1,?)
    ON CONFLICT(campaign_key,visit_date) DO UPDATE SET visits=affiliate_promotion_visits.visits+1,updated_at=excluded.updated_at`)
    .bind(key,today,nowIso()).run().catch(() => {});
  const target = new URL('https://ekodi.kr/mall');
...
export const MALL_PROMOTION_DEFAULTS = Object.freeze({
  subjectType:SUBJECT_TYPE,
  subjectKey:SUBJECT_KEY,
  storefront:STOREFRONT,
  accountId:ACCOUNT_ID,
  providers:PROVIDERS,
  runAfterKstHour:RUN_AFTER_KST_HOUR,
  maxDailyChannels:MAX_DAILY_CHANNELS,
