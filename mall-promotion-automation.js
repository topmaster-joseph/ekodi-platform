...
export async function runMallPromotionAutomation(env,{reason='cron',force=false}={}){
  ...
  for(const {connection,learning} of selected){
    ...
    const linkUrl='https://ekodi.kr/marketing-connect-api/r/mall/'+encodeURIComponent(keyValue);
    const imageUrl='https://ekodi.kr/api/api/affiliate/public/image/'+Number(product.id)+'?storefront='+STOREFRONT;
