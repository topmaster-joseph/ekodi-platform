...
test('pipeline is idempotent for the same workspace/batch/item/version',async()=>{
  ...
  const renderer={ready:()=>true,async render({job,batch,voice_asset}){renderCalls++;assert.equal(job.id,'workspace-a:2026-09:01:v1');assert.equal(batch.items[0].metadata.voice_asset_key,'2026-09/01/v1/voice.wav');assert.equal(voice_asset.data.toString(),'WAV');return{ok:true,artifacts:[{item_id:'01',path:'/tmp/01.mp4'}]}}};
...
test('identical batch/item/version keys remain isolated by workspace_id',async()=>{
  ...
  const pipeline=createDevotionPipeline({voice:{ready:()=>true,async synthesize(){return{audio:Buffer.from(`v${++calls}`),mime_type:'audio/wav'}}},assets,renderer:{ready:()=>true,async render({batch}){return{workspace_id:batch.workspace_id}}}});
  ...
  assert.equal(assets.map.size,4);
});
test('pipeline reuses stored voice after a renderer failure',async()=>{
  ...
  const renderer={ready:()=>true,async render(){renderCalls++;if(renderCalls===1)throw new Error('temporary render failure');return{ok:true,artifacts:[{path:'/tmp/retry.mp4'}]}}};
...
test('pipeline fails closed with explicit dependency error codes',async()=>{
  ...
  const goodRenderer={ready:()=>true,async render(){return{ok:true}}};
...
test('HTTP adapters keep generic service contracts and inline voice bytes for rendering',async()=>{
  ...
  const fetchImpl=async(url,init)=>{
    ...
    if(url.endsWith('/v1/render'))return new Response(JSON.stringify({ok:true,artifacts:[{path:'/tmp/out.mp4'}]}),{status:200,headers:{'content-type':'application/json'}});
...
test('pipeline core contains no tenant, publisher, Google Drive, Gemini or FFmpeg dependency',async()=>{
  const fs=await import('node:fs/promises');
  const files=['../src/service.js','../src/adapters/http-voice.js','../src/adapters/http-assets.js','../src/adapters/http-renderer.js'];
