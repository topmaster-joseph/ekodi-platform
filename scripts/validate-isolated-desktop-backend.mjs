...
if(policy.guestAgent?.id!=='ekodi-isolated-guest-agent'||policy.guestAgent?.version!=='1.2.0') fail('isolated guest agent identity/version drifted');
...
if(!Array.isArray(policy.guestAgent?.allowedTaskTypes)||!policy.guestAgent.allowedTaskTypes.includes('guest.runtime.probe')||!policy.guestAgent.allowedTaskTypes.includes('guest.ui.probe')||!policy.guestAgent.allowedTaskTypes.includes('guest.session.execute')) fail('guest agent task allowlist drifted');
...
if(policy.activation?.sessionCanaryCommand!=='computer.desktop.session.canary'||policy.activation?.sessionCanaryRequiredBeforeExecution!==true||policy.activation?.sessionCanaryEnablesBoundedExecutionCapability!==true||policy.activation?.generalUnboundedDesktopExecutionAfterSessionCanary!==false) fail('bounded isolated session activation contract drifted');
if(policy.sessionExecutor?.id!=='ekodi-isolated-session-bounded-v1'||policy.sessionExecutor?.version!=='bounded-v1'||policy.sessionExecutor?.command!=='computer.desktop.session.execute'||policy.sessionExecutor?.taskType!=='guest.session.execute') fail('bounded isolated session executor identity drifted');
if(!Array.isArray(policy.sessionExecutor?.allowedOperations)||policy.sessionExecutor.allowedOperations.length!==1||policy.sessionExecutor.allowedOperations[0]!=='ui.text.roundtrip') fail('bounded isolated session operation allowlist drifted');
if(policy.sessionExecutor?.textPattern!=='^EKODI_SESSION_[A-Z0-9_-]{8,64}
...
if(remote.nonDisruptiveExecution?.isolatedDesktop?.sessionCanaryCommand!=='computer.desktop.session.canary'||remote.nonDisruptiveExecution?.isolatedDesktop?.sessionCanaryRequiredBeforeExecution!==true||remote.nonDisruptiveExecution?.isolatedDesktop?.sessionExecutorCommand!=='computer.desktop.session.execute'||remote.nonDisruptiveExecution?.isolatedDesktop?.sessionExecutorScope!=='bounded-v1') fail('remote bounded isolated session executor contract drifted');
if(remote.nonDisruptiveExecution?.isolatedDesktop?.sessionCanaryMayEnableBoundedCapability!==true||remote.nonDisruptiveExecution?.isolatedDesktop?.unboundedDesktopExecutionRemainsForbidden!==true||remote.nonDisruptiveExecution?.isolatedDesktop?.rawSessionInputReturnedInCloudResult!==false) fail('remote bounded isolated session safety/cutover contract drifted');
...
console.log('- bounded-v1 session execution activates only after session canary and exposes hashes/receipts rather than raw input');
||policy.sessionExecutor?.rawInputReturnedInCloudResult!==false||policy.sessionExecutor?.inputOutputHashesRequired!==true) fail('bounded isolated session payload/evidence contract drifted');
for(const key of ['networkAttached','hostInteractiveDesktopUsed','sharedInteractiveDesktop','lowLevelInputInjection','clipboardShared','credentialCollection','hostProfileMounted','generalPurposeShell','arbitraryFilesystemAccess','externalNetworkAccess']) if(policy.sessionExecutor?.[key]!==false) fail(`bounded isolated session negative guard drifted: ${key}`);
for(const key of ['semanticUiAutomation','ephemeralDifferencingDisk','sessionVmCleanupRequired','sessionDiskCleanupRequired','boundedLease']) if(policy.sessionExecutor?.[key]!==true) fail(`bounded isolated session required guard drifted: ${key}`);
if(policy.sessionCanary?.requiresUiCanary!==true||policy.sessionCanary?.executor!=='ekodi-isolated-session-bounded-v1'||policy.sessionCanary?.operation!=='ui.text.roundtrip'||policy.sessionCanary?.expectedResultCode!=='EKODI_SESSION_OK'||policy.sessionCanary?.roundTripMatchRequired!==true||policy.sessionCanary?.hashesMustMatch!==true||policy.sessionCanary?.cleanupRequired!==true) fail('bounded isolated session canary proof contract drifted');
if(remote.nonDisruptiveExecution?.isolatedDesktop?.backendPolicy!=='config/isolated-desktop-backend-policy.json') fail('remote computer policy must bind isolated desktop backend policy');
if(remote.nonDisruptiveExecution?.isolatedDesktop?.probeCommand!=='computer.desktop.probe'||remote.nonDisruptiveExecution?.isolatedDesktop?.canaryCommand!=='computer.desktop.canary'||remote.nonDisruptiveExecution?.isolatedDesktop?.executionCapabilityDefault!==false) fail('remote isolated desktop probe/canary/default gate drifted');
if(remote.nonDisruptiveExecution?.isolatedDesktop?.headlessBackendRequired!==true||remote.nonDisruptiveExecution?.isolatedDesktop?.windowsSandboxForegroundUiNotActivationEligible!==true) fail('remote isolated desktop headless safety guard drifted');
if(remote.nonDisruptiveExecution?.isolatedDesktop?.guestCanaryCommand!=='computer.desktop.guest.canary'||remote.nonDisruptiveExecution?.isolatedDesktop?.guestCanaryRequiredBeforeExecution!==true||remote.nonDisruptiveExecution?.isolatedDesktop?.guestCanaryDoesNotEnableExecution!==true) fail('remote isolated desktop guest proof contract drifted');
