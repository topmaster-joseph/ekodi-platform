...
if(policy.activation?.isolatedDesktopCapabilityDefault!==false||policy.activation?.probeCommand!=='computer.desktop.probe'||policy.activation?.executionCommand!=='computer.desktop.session.execute') fail('isolated desktop activation defaults drifted');
if(policy.activation?.executionCommandMustRemainDisabledUntilVerifiedHeadlessBackend!==true) fail('isolated desktop execution must stay disabled until verified headless backend');
...
if(remote.nonDisruptiveExecution?.isolatedDesktop?.probeCommand!=='computer.desktop.probe'||remote.nonDisruptiveExecution?.isolatedDesktop?.executionCapabilityDefault!==false) fail('remote isolated desktop probe/default gate drifted');
