...
const services = [...block.matchAll(/\{\s*id:\s*'([^']+)'[\s\S]*?basePath:\s*'([^']+)'[\s\S]*?labels:\s*\{\s*ko:\s*'([^']+)'/g)]
  .map(([, id, basePath, name]) => ({ id, basePath, name }));

requireTrue(menu.includes("id: 'service-admins'") && menu.includes("ko: '서비스 관리'"), 'central Admin must expose a dedicated service-management menu group');
...
requireTrue(menu.includes('serviceAdmin: true') && menu.includes('adminHandoff: true'), 'service menus must hand off instead of duplicating service admin UI');
