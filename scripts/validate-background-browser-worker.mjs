...
if(policy.routingPolicy!=='config/virtualization-routing-policy.json') fail('browser worker must bind native virtualization routing policy');
if(policy.provider?.externalFallbackForThisWorker!=='forbidden-while-runtime-healthy'||policy.provider?.mustIdentifyAsNativeInEvidence!==true) fail('browser worker external fallback identity guard drifted');
