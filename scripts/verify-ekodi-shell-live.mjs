...
for(let attempt=1;attempt<=attempts;attempt++){
  ...
  if(health){
    ...
    if(Number(health.adminUIShellVersion)<3)failures.push(`health:adminUIShellVersion:${health.adminUIShellVersion||'missing'}`);
  ...
  if(shellResult.headers?.get?.('x-ekodi-admin-ui-shell')!=='v3')failures.push(`shell:admin-ui-shell:${shellResult.headers?.get?.('x-ekodi-admin-ui-shell')||'missing'}`);
