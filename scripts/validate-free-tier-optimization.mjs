...
expect(policy.schemaVersion===3,'free-tier policy schema must remain v3');
...
expect(policy.deploymentContinuity?.cloudControlIsSecondDeployLane===false,'Cloud Control must not become a second deploy lane');
...
expect(governor.includes('workersBuildMinutesPerMonthFree:3000'),'resource governor must record current Workers Builds free monthly minutes reference');
expect(governor.includes('workersBuildConcurrencyFree:1'),'resource governor must record current Workers Builds free concurrency reference');
