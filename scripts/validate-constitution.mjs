...
if(publicUserSurface.anonymousReadAccessRequired!==true||publicUserSurface.signedInBasicAccess!=='free-by-default'||publicUserSurface.basicCapabilitiesRequirePaidSubscription!==false) fail('public pages must remain guest-readable and signed-in basic capabilities must remain free');
if(publicUserSurface.premiumCapabilities!=='subscription-or-explicit-entitlement'||publicUserSurface.entitlementUpgradeMustNotBlockPublicOrBasicExperience!==true) fail('premium entitlement must be additive and must not block public/basic experience');
if(JSON.stringify(publicUserSurface.adminPathRule?.patterns)!==JSON.stringify(['/admin','*/admin','*/admin/*'])||publicUserSurface.adminPathRule?.authenticationRequired!==true||publicUserSurface.adminPathRule?.authorizationRequired!==true) fail('admin paths must always require authentication and authorization');
if(workspace.publicUserSurfaceDefault?.signedInBasicAccess!=='free-by-default'||workspace.commonServiceUserAccessRule?.freeMemberBasicCapabilitiesRequired!==true) fail('workspace/service policy must preserve free signed-in basic capabilities');
if(workspace.adminAccessRule?.authenticationRequired!==true||workspace.adminAccessRule?.authorizationRequired!==true||workspace.adminAccessRule?.guestAccess!=='forbidden') fail('workspace admin access must remain protected');
