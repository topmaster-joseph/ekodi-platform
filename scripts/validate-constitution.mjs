...
if (constitution.version !== '1.27.0') fail('constitution version must be 1.27.0 with apex-path force execution plus all prior approved amendments');
...
if (automaticExecution.id !== 'AUTOMATIC-EXECUTION-LIFECYCLE-001' || automaticExecution.status !== 'enforced' || automaticExecution.mode !== 'mandatory') fail('automatic execution lifecycle constitutional rule must remain mandatory and enforced');
if (automaticExecution.finalAuthority !== 'ekodi_platform_super_administrator' || automaticExecution.orchestrationOwner !== 'ekodi-orchestrator') fail('automatic execution lifecycle authority/owner drifted');
if (automaticExecution.appliesToAllCurrentAndFutureAutomaticExecution !== true || automaticExecution.defaultExecutionMode !== 'background-only') fail('all current and future automatic execution must remain background-only by default');
for (const flag of ['foregroundWindowDefault','userBrowserTabCreation']) if (automaticExecution[flag] !== false) fail(`automatic execution constitutional flag must remain false: ${flag}`);
for (const flag of ['ownedAutomationSurfaceAutoClose','authRequiredMustNotOpenInteractiveLogin','preserveUserOwnedWindowsAndTabs','temporaryAutomationProfilesMustBeRemoved','backgroundApiPreferredOverBrowserForApiChecks','localOverrideForbidden','serviceOrAgentWaiverForbidden','policyRegressionBlocksCi']) if (automaticExecution[flag] !== true) fail(`automatic execution constitutional guard missing: ${flag}`);
if (automaticExecution.authRequiredDisposition !== 'record-and-close') fail('automatic execution auth-required disposition must remain record-and-close');
for (const reason of ['oauth-or-provider-consent','captcha-or-human-verification','hardware-backed-authentication','os-privileged-consent']) if (!automaticExecution.foregroundAllowedOnlyFor?.includes(reason)) fail(`automatic execution foreground exception missing: ${reason}`);
for (const binding of ['config/remote-computer-execution-policy.json','config/autonomous-execution-fabric-policy.json','config/virtualization-routing-policy.json','config/background-browser-worker-policy.json']) if (!automaticExecution.requiredPolicyBindings?.includes(binding)) fail(`automatic execution policy binding missing: ${binding}`);
const remoteAutomatic = remoteComputer.nonDisruptiveExecution?.automaticExecutionLifecycle || {};
if (remoteComputer.constitutionalPolicy !== 'AUTOMATIC-EXECUTION-LIFECYCLE-001' || remoteAutomatic.constitutionalPolicy !== 'AUTOMATIC-EXECUTION-LIFECYCLE-001') fail('remote computer policy must bind the automatic execution constitution');
if (remoteAutomatic.defaultMode !== 'background-only' || remoteAutomatic.foregroundWindowDefault !== false || remoteAutomatic.userBrowserTabCreation !== false) fail('remote automatic execution must remain background-only without foreground windows or user tabs');
if (remoteAutomatic.ownedAutomationSurfaceAutoClose !== true || remoteAutomatic.authRequiredDisposition !== 'record-and-close' || remoteAutomatic.authRequiredMustNotOpenInteractiveLogin !== true || remoteAutomatic.preserveUserOwnedWindowsAndTabs !== true) fail('remote automatic execution lifecycle drifted');
if (remoteAutomatic.mandatory !== true || remoteAutomatic.localOverrideForbidden !== true || remoteAutomatic.appliesToAllCurrentAndFutureAutomaticExecution !== true || remoteAutomatic.policyRegressionBlocksCi !== true) fail('remote automatic execution constitutional enforcement metadata drifted');
const fabricAutomatic = executionFabric.automaticExecutionLifecycle || {};
if (!executionFabric.constitutionalPolicies?.includes('AUTOMATIC-EXECUTION-LIFECYCLE-001')) fail('execution fabric must register automatic execution constitution');
if (fabricAutomatic.constitutionalPolicy !== 'AUTOMATIC-EXECUTION-LIFECYCLE-001' || fabricAutomatic.mandatory !== true || fabricAutomatic.defaultMode !== 'background-only') fail('execution fabric automatic lifecycle binding drifted');
if (fabricAutomatic.foregroundWindowDefault !== false || fabricAutomatic.userBrowserTabCreation !== false || fabricAutomatic.ownedAutomationSurfaceAutoClose !== true || fabricAutomatic.authRequiredDisposition !== 'record-and-close' || fabricAutomatic.authRequiredMustNotOpenInteractiveLogin !== true || fabricAutomatic.preserveUserOwnedWindowsAndTabs !== true || fabricAutomatic.localOverrideForbidden !== true || fabricAutomatic.policyRegressionBlocksCi !== true) fail('execution fabric automatic lifecycle guards drifted');
...
if ('legacyPathAliases' in constitution) fail('retired path aliases must be absent');
...
if (workspace.userSurfaceTopologyPolicy?.customerSpecificAiSubdomains !== 'forbidden') fail('service workspace policy must forbid customer AI subdomains as canonical');
