...
if(topNav.includes('href="/my/device-care/"'))throw new Error('My EKODI validation failed: Device Care must stay out of the fixed top navigation');
must('html','class="my-utility-discovery"');
must('html','href="/my/device-care/">내 PC</a>');
...
must('membershipJs','/api/membership/entitlements');
must('membershipJs','aiEntitlementSubject');
must('membershipJs','같은 주체가 같은 AI 기능');
must('membershipJs','기관·단체');
...
must('membershipCss','.ai-entitlement-manager');
must('membershipCss','.ai-entitlement-subject-row');
must('membershipCss','.ai-entitlement-level');
...
mustNot('manifest','https://my.ekodi.kr');
must('prod','DATA_ENABLED = "true"');
must('staging','DATA_ENABLED = "false"');
mustNot('staging','my.ekodi.kr');
must('auth',"'my':{name:'My EKODI'");
must('auth',"returnTo:'https://ekodi.kr/my/'");
must('auth','function postLoginTarget()');
must('auth','const target=new URL(RETURN_TO)');
must('auth',"isPlatformMy&&!['my','portal'].includes(site)");
mustNot('auth','function myEntryTarget()');
mustNot('auth','commonServiceEntry&&proof.platformAdmin!==true');
