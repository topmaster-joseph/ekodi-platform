...
must('auth',"'my':{name:'EKODI',returnTo:'https://ekodi.kr/',open:true,kind:'portal'}");
must('auth',"if(site==='my')return new URL('https://ekodi.kr/');");
must('auth','function siteMemberHomeTarget()');
must('auth','const target=siteMemberHomeTarget()');
must('auth',"if(site==='portal'||params.get('manage')==='1'");
