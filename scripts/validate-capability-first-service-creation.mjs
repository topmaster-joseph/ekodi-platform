...
const capabilityIds = new Set([
  ...(capabilityRegistry.capabilities || []).map(item => String(item.id || '')),
  ...(capabilityRegistry.fabricCapabilities || []).map(item => String(item.id || ''))
]);
...
if (policy.id !== 'CAPABILITY-BEFORE-SERVICE-001' || policy.status !== 'enforced' || policy.mode !== 'mandatory') {
  fail('capability-before-service constitutional policy must remain mandatory and enforced');
}
...
if (foundry.serviceCreationRule?.capabilityFirst !== true) fail('Capability Foundry capability-first rule must remain enabled');
...
if (evidenceRegistry.policyId !== policy.id || evidenceRegistry.policyVersion !== constitution.version) fail('service creation evidence registry policy/version mismatch');
if (evidenceRegistry.status !== 'active') fail('service creation evidence registry must remain active');

const validateEvidence = (serviceId, service) => {
  const record = records[serviceId];
  if (!record) {
    fail(`${serviceId}: new service is outside the grandfathered baseline and has no governed creation evidence`);
    return;
  }
  ...
  const declaredCapabilities = Array.isArray(service?.capabilities) ? service.capabilities : [];
  if (!declaredCapabilities.length) fail(`${serviceId}: new canonical service must declare reusable capabilities`);
};
...
for (const id of baseline) {
  if (!manifestIds.has(id)) fail(`grandfathered canonical service disappeared from the adoption baseline: ${id}`);
}
for (const id of secondaryOnlyBaseline) {
  if (!(secondaryRegistry.services || []).some(service => service.id === id)) fail(`grandfathered secondary registry-only service disappeared from baseline: ${id}`);
}
...
console.log(`Capability-before-service gate: OK (${manifestServices.length} grandfathered/current canonical services, ${Object.keys(records).length} post-adoption evidence records).`);
