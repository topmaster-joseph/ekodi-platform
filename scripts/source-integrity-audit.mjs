...
if (!changedFiles.length) {
  ...
  const realms=new Map(); for(const match of clientAuth.matchAll(/'([^']+)-client'\s*:\s*\{[^}]*?returnTo:\s*'([^']+)'/g)){try{realms.set(match[1],new URL(match[2]));}catch{}} for(const [slug,domain] of tenants){if(!realms.has(slug))continue;const target=realms.get(slug);const canonicalApex=target.origin==='https://ekodi.kr'&&(target.pathname===`/${slug}`||target.pathname.startsWith(`/${slug}/`));const customerOwned=!domain.endsWith('.ekodi.kr')&&target.hostname===domain;check(`auth-return:${slug}`,canonicalApex||customerOwned,`tenant=${domain}, auth-return=${target.href}`);}
