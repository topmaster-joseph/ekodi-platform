...
const [capabilityConfig, packConfig, governance, ecosystem] = await Promise.all([
...
const gateCount = (capabilityConfig.capabilities ?? []).filter(item => item.actionTier === 'human_gate').length;
console.log(`EKODI AI Capability Library OK: ${capabilityIds.size} capabilities, ${packIds.size} packs, ${gateCount} human-gated capabilities.`);
