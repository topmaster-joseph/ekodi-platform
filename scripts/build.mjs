...
// System Timeline extends the existing authenticated Deployments surface instead of
// adding another admin menu item. GitHub remains the source of truth; this is a read-only view.
...
console.log(`Built EKODI root with ${homepageServices.length} registry-driven homepage services, minimal pre-auth Control Center, authenticated Mission Control/Campus/Chief AI/Device Control/Creator billing modules, GitHub-backed System Timeline, MarketingAI live ops, device bootstrap, auth hub, service hubs and trade assets: ${assets.join(', ')}`);