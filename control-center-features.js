(() => {
  ...
  function installFeatureNavigation() {
    ...
    if (books) { if (finance) nav.insertBefore(books, finance); else nav.append(books); }
  ...
  window.addEventListener('ekodi-feature-installed', normalizeShortLabels); window.addEventListener('hashchange', activateHash);
  if (app && app.hidden) {
