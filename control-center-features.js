(() => {
  ...
  function ensureBooksCoreTabs() {
    ...
    if (!tabs.querySelector('[data-books-tab="assets"]')) {
      ...
      tab.dataset.booksTab = 'assets';
      tab.textContent = 'Assets';
      tab.addEventListener('click', () => { selectBooksTab('assets'); renderAssetsShell(); });
      tabs.append(tab);

      const pane = document.createElement('div');
      pane.className = 'books-pane';
      pane.dataset.booksPane = 'assets';
      pane.hidden = true;
      ...
      pane.querySelector('[data-books-assets-refresh]').addEventListener('click', renderAssetsShell);
      section.append(pane);
    }
    ...
    if (!tabs.querySelector('[data-books-tab="governance"]')) {
      ...
      tab.type = 'button';
      tab.className = 'books-tab';
      tab.dataset.booksTab = 'governance';
      tab.textContent = 'Governance';
      tab.addEventListener('click', () => { selectBooksTab('governance'); renderGovernanceShell(); });
      tabs.append(tab);

      const pane = document.createElement('div');
      pane.className = 'books-pane';
      pane.dataset.booksPane = 'governance';
      pane.hidden = true;
      ...
      pane.querySelector('[data-books-governance-refresh]').addEventListener('click', renderGovernanceShell);
      section.append(pane);
    }
  ...
  async function loadClients() {
    await Promise.all([loadStyle('client-access.css'), loadStyle('marketing-funnel-admin.css')]);
  ...
  function normalizeShortLabels() {
    setShortLabel('admins', 'Admin'); setShortLabel('domains', 'Domains'); setShortLabel('social', 'Social'); setShortLabel('books', 'Books');
  ...
  function removeResolvedPlaceholders() {
    if (!nav) return;
    for (const [section, button] of placeholderButtons) if (nav.querySelector(`[data-section="${section}"]`)) { button.remove(); placeholderButtons.delete(section); }
  }
  ...
  function installStaticBooksNavigation() {
    if (!nav) return null;
    let button = nav.querySelector('[data-section="books"]');
    if (!button) {
      button = document.createElement('button');
      button.type = 'button';
      button.className = 'nav';
      button.dataset.section = 'books';
      button.dataset.booksLoader = 'true';
      button.append(document.createTextNode('▤ '));
      const span = document.createElement('span'); span.textContent = 'Books'; button.append(span);
      const finance = nav.querySelector('[data-section="finance"]');
      if (finance) nav.insertBefore(button, finance); else nav.append(button);
    }
    if (button.dataset.booksLazyBound !== 'true') {
      button.dataset.booksLazyBound = 'true';
      button.addEventListener('click', async event => {
        if (document.querySelector('#booksAdminSection') || !token()) return;
        ...
        try {
          await loadBooks();
          button.remove();
          notifyInstalled('books');
          const realButton = nav.querySelector('[data-section="books"]');
          if (!realButton) throw new Error('Books 관리 화면을 준비하지 못했습니다.');
          queueMicrotask(() => realButton.click());
        } catch (error) {
          console.warn('[EKODI Books feature]', error);
  ...
  function installFeatureNavigation() {
    if (!nav) return;
    const services = nav.querySelector('[data-section="services"]'); const finance = nav.querySelector('[data-section="finance"]');
    const communication = nav.querySelector('[data-section="communication"]'); const organization = nav.querySelector('[data-section="organization"]'); const policies = nav.querySelector('[data-section="policies"]');
  ...
  function requestedSection() {
    const hash = location.hash.replace(/^#/, '').toLowerCase(); if (hash) return hash;
    const first = location.pathname.split('/').filter(Boolean)[0]?.toLowerCase() || '';
    return first;
  }
  function activateHash() {
    if (!token() || !app || app.hidden) return;
    const section = requestedSection(); if (!loaders[section]) return;
    nav?.querySelector(`[data-section="${section}"], [data-lazy-section="${section}"]`)?.click();
  }
  ...
  if (app && app.hidden) {
    const observer = new MutationObserver(() => { if (!app.hidden && token()) { observer.disconnect(); normalizeShortLabels(); activateHash(); } });
