(() => {
  ...
  section.innerHTML = `
    ...
    <div class="core-health-grid" aria-label="EKODI Core 상태">
      <article data-core-card="core" data-state="pending"><div><small>코어 API</small><b data-core-badge="core">확인 전</b></div><strong data-core-value="core">—</strong><span data-core-detail="core">ekodi.kr/api</span></article>
  ...
  function renderCore(results) {
    ...
    if (core.ok && coreData.ok) {
      setFlowNode('core', 'ok', `${coreData.canonicalHosts?.api || 'ekodi.kr/api'} 정상`);
      setCoreCard('core', 'ok', '정상', `v${coreData.apiVersion || '1.0.0'}`, `${coreData.canonicalHosts?.api || 'ekodi.kr/api'} · ${coreData.architecture || 'hybrid-cloud'}`);
