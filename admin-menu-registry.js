...
export const ADMIN_MENU_ROUTES = Object.freeze({
  campus: '#campus', work: '#work', communication: '#communication', workspace: '#workspace', organization: '#organization', clients: '#clients', admins: '#admins',
  'life-ai': '#life-ai', community: '#community', books: '#books', social: '#social', aiops: '#ai-ops', devotional: '#devotional', 'marketing-ai': '#marketing-ai',
  'ai-module-spec': '#ai-module-spec', 'ai-membership': '#ai-membership', finance: '#finance', tax: 'https://tax.ekodi.kr/', affiliates: '#mall-ai-sales', storage: '#storage',
  'api-cost': '#api-cost', health: '#health', security: '#security', devices: '#devices', architecture: '#architecture',
});
...
export function getAdminMenuRoute(id) { const item = getAdminMenuItem(id); if (!item || item.internal) return ''; return item.href || ADMIN_MENU_ROUTES[item.id] || ''; }
export function getAdminMenuGroupRoute(id) { return getAdminMenuRoute(getAdminMenuGroupDefault(id)); }
