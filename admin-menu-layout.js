...
function installCompactStyle(){
  ...
  const style=document.createElement('style');style.id='ekodi-admin-menu-density';
...
function sectionOf(item){
  ...
  const raw=String(item?.dataset?.section||item?.dataset?.lazySection||'').trim();return raw==='marketing'?'marketing-ai':raw;
}
...
function enforcePolicy(){
  for(const item of allNav()){
    ...
    item.hidden=true;item.dataset.aiInternal=sectionOf(item)||item.getAttribute('href')||'internal';
    item.setAttribute('aria-hidden','true');item.tabIndex=-1;item.classList.remove('active');
  }
  applyOrder();
}
const navItemFor=section=>[...allNav()].find(item=>sectionOf(item)===section&&!isInternalNav(item))||null;
function syncTitle(section){
  const title=document.querySelector('#pageTitle');const item=navItemFor(section);
  ...
  if(last===section)return;last=section;
  window.dispatchEvent(new CustomEvent('ekodi-admin-section-changed',{detail:{section}}));
}
function activatePanel(section){
  if(!section||!hasPanel(section))return false;requestedSection=section;
  for(const panel of content.querySelectorAll('[data-panel]')){
    const visible=panelTargets(panel).includes(section);panel.classList.toggle('hidden-panel',!visible);
  ...
  syncTitle(section);const hash=CANON.get(section);if(hash&&location.hash!==hash)history.replaceState(null,'',hash);
  if(section==='architecture'&&!window.EKODISystemMap)import('./system-health-admin.js').catch(console.error);
  sidebar.classList.remove('open');return true;
}
async function openSites(){
  dc=false;requestedSection='sites';
  if(!sitesLoading)sitesLoading=import('./homepage-admin.js').then(module=>{
    module.mountHomepageAdmin();window.dispatchEvent(new CustomEvent('ekodi-feature-installed',{detail:{section:'sites'}}));return module;
  }).catch(error=>{sitesLoading=null;console.error(error);throw error;});
  await sitesLoading;applyOrder();activatePanel('sites');navItemFor('campus')?.classList.add('active');syncTitle('campus');
}
...
function requestDemand(section){
  ...
  const task=Promise.resolve(window.EKODIAdminDemand.activate(demandKey)).then(()=>{
    applyOrder();if(!activatePanel(section)){
      const real=navItemFor(section);if(real&&!real.dataset.demandFeature)real.click();queueMicrotask(()=>activatePanel(section));
    }
  }).catch(error=>{console.error(`[EKODI Admin] ${section} demand activation failed`,error);fallbackDemand(section);})
...
function reconcileNavigation(){
  ...
  try{enforcePolicy();if(!requestedSection||dc)return;if(requestedSection==='sites'&&!hasPanel('sites'))openSites();else if(!activatePanel(requestedSection))requestDemand(requestedSection);}
...
nav.addEventListener('click',event=>{
  const item=event.target.closest('.nav[data-section],.nav[data-lazy-section],.nav[data-device-control-nav],a.nav[href]');if(!item)return;
...
content.addEventListener('click',event=>{
  const control=event.target.closest('[data-campus-section]');if(!control||!isInternal(control.dataset.campusSection))return;
...
window.addEventListener('ekodi-admin-ready',()=>{
  enforcePolicy();const section=explicitHashSection();if(section&&isInternal(section))return routeInternal();
  if(section==='sites')return openSites();if(section){dc=false;requestedSection=section;if(!activatePanel(section))requestDemand(section);}else{requestedSection='campus';dc=true;}
});
window.addEventListener('hashchange',()=>{
  const section=explicitHashSection();if(!section)return;dc=false;if(isInternal(section))return routeInternal();
  if(section==='sites')return openSites();requestedSection=section;if(!activatePanel(section))requestDemand(section);
});
installCompactStyle();mountAdminSidebar(document);enforcePolicy();
const initialHash=explicitHashSection();
if(initialHash&&isInternal(initialHash))routeInternal();else if(initialHash==='sites')openSites();else if(initialHash)requestedSection=initialHash;else{requestedSection='campus';dc=true;requestDemand('campus');}
window.EKODIAdminPanels=Object.freeze({
  activate:section=>{dc=false;if(isInternal(section))return routeInternal();if(section==='sites')return openSites();requestedSection=section;return activatePanel(section)||requestDemand(section);},
  current:()=>requestedSection,internalSections:Object.freeze([...INTERNAL]),visibleMenuOrder:ORDER
});
