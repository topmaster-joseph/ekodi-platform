...
function currentDetailSegments(section){const route=adminRoutes()?.routeFromPath?.(location.pathname);return route?.section===section?(route.detailSegments||[]):[]}
function replaceSectionUrl(section){const target=adminRoutes()?.navigationTarget?.(section,location,currentDetailSegments(section))||CANON.get(section);if(!target)return;if(target.startsWith('/')){const current=location.pathname+location.search+location.hash;if(current!==target)history.replaceState(null,'',target)}else if(location.hash!==target)history.replaceState(null,'',target)}
...
function activateCommandHome(){
  dc=false;requestedSection=COMMAND_HOME;
  for(const panel of content.querySelectorAll('[data-panel]')){panel.classList.add('hidden-panel');panel.hidden=true}
  for(const item of allNav())item.classList.toggle('active',!isInternalNav(item)&&sectionOf(item)===COMMAND_HOME);
  syncTitle(COMMAND_HOME);
  replaceSectionUrl(COMMAND_HOME);
...
function reconcileNavigation(){
  ...
  try{
    ...
    if(requestedSection===COMMAND_HOME)activateCommandHome();
    else if(requestedSection==='sites'&&!hasPanel('sites'))openSites();
