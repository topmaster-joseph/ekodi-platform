...
2. `ekodi.kr/api`, `ekodi.kr/auth`, `ekodi.kr/pay`, `finance-ekodi.kr/api` 내부 DB를 Mall 편의를 위해 직접 수정하지 않는다.
...
1. `/free-ops`를 일상 운영의 기본 화면으로 사용하고, 복잡한 `/supplier-discovery`, `/supplier-ops`, `/supplier-connectors`는 필요할 때만 연다.
2. Free Ops는 **새 DB·새 Worker·유료 공급자 API를 만들지 않고** 기존 Mall Worker/D1 및 supplier safety API를 재사용한다.
3. Free Ops 입력은 공급처명·공개 HTTPS URL·카테고리·판매가·공급가·배송비와 운영자가 직접 확인한 체크 항목으로 제한한다.
4. 공급가·배송비·예상 기여이익 계산은 브라우저 보조값이며 서버 정산·실주문 권한 근거가 아니다.
5. 체크박스만으로 Supplier Partner, 판매권한, 개인정보 제공권, 자동발주권을 얻지 않는다. 기존 evidence/contract/PII gate를 그대로 통과해야 한다.
6. 손실 예상 후보는 Free Ops에서 신규 저장을 차단한다. 낮은 마진은 경고하되 최종 운영 판단과 실제 판매 전 재확인을 요구한다.
7. Free Ops에는 공급자 비밀번호·세션·API secret·고객 배송 PII 입력란을 만들지 않는다.
8. 무료 운영을 유지하기 위해 평소에는 공개 URL 수동확인과 기존 D1 저장을 사용하고, 공식 API는 거래량과 실익이 생길 때만 별도 검증 후 선택적으로 켠다.
...
- Free Ops에서 공급처 후보 등록·예상 마진 계산·기본 안전체크
...
3. `ekodi.kr/mall-api-staging` staging Worker를 배포한다.
