(() => {
  ...
  function productCard(item) {
    ...
    const heart = node('button', 'heart', '♡');
    heart.type = 'button'; heart.dataset.wish = id; heart.setAttribute('aria-label', `${article.dataset.name} 관심상품`); heart.setAttribute('aria-pressed', 'false');
    thumb.append(heart);

    const body = node('div', 'card-body');
    const kicker = node('p', 'store-kicker', store ? `${store.name} · ${seller.displayName}` : `개인상품 · ${seller.displayName || '판매자'}`);
    const h3 = node('h3'); const title = node('a', '', article.dataset.name); title.href = href; h3.append(title);
    const description = node('p', '', text(p.oneLine) || text(p.audience) || '판매자가 등록한 공개 상품입니다.');
    const meta = node('div', 'meta');
    meta.append(node('span', 'status', `${price(p.price)} · Mall 발견 8%`));
    const detail = node('a', 'smallbtn', '자세히'); detail.href = href; meta.append(detail);
    body.append(kicker, h3, description, meta);
