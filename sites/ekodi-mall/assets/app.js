(() => {
  ...
  function readWishes() { try { const value = JSON.parse(localStorage.getItem(storageKey) || '[]'); return Array.isArray(value) ? value : []; } catch { return []; } }
  function saveWishes() { localStorage.setItem(storageKey, JSON.stringify(wishes)); }
  function updateCount() {
    const count = String(wishes.length); const desktop = document.querySelector('#wishCount'); const mobile = document.querySelector('#mobileCount');
    if (desktop) desktop.textContent = count; if (mobile) mobile.textContent = count;
  }
  function syncHearts() {
  ...
  filterButtons.forEach((button) => button.addEventListener('click', () => {
    filterButtons.forEach((item) => item.classList.remove('active')); button.classList.add('active'); filter = button.dataset.filter || 'all'; render();
  }));
  search?.addEventListener('input', render);
  document.querySelector('#searchBtn')?.addEventListener('click', () => { location.hash = 'shop'; window.setTimeout(() => search?.focus(), 220); });
  document.addEventListener('click', (event) => {
    const button = event.target.closest('[data-wish]'); if (!button) return; const id = button.dataset.wish;
    wishes = wishes.includes(id) ? wishes.filter((item) => item !== id) : [...wishes, id]; saveWishes(); render();
  });
  ...
  function openWishlist() {
    ...
    if (wishlistNode) {
      ...
      if (!selected.length) { const p = document.createElement('p'); p.textContent = '관심상품이 없습니다.'; p.style.color = '#68726d'; wishlistNode.append(p); }
      else selected.forEach((card) => {
        const row = document.createElement('div'); row.className = 'wishrow';
        const info = document.createElement('div'); const link = document.createElement('a'); link.href = card.dataset.href || '#';
        const strong = document.createElement('b'); strong.textContent = card.dataset.name || '상품'; link.append(strong);
        const br = document.createElement('br'); const small = document.createElement('small'); small.textContent = card.dataset.status || '상품'; info.append(link, br, small);
