(() => {
  ...
  const providerMatches = (item) => {
    const haystack = [item.providerKey, item.providerName, item.affiliateNetworkName].map((value) => clean(value).toLowerCase()).join(' ');
    return keys.some((key) => haystack.includes(key));
  };
  const searchMatches = (item, query) => !query || [item.productName, item.category, item.providerName].map(clean).join(' ').toLowerCase().includes(query);
