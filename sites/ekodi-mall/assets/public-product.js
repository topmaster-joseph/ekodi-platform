(() => {
  ...
  function priceLabel(value) {
    return Number.isFinite(value) ? `${new Intl.NumberFormat('ko-KR').format(value)}원` : '가격 확정 전';
  }
  ...
  async function issueAttribution() {
    try {
      const response = await fetch(`${API}/api/public/attribution`, {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({ shareCode: code, refCode })
      });
      const body = await response.json();
      if (response.ok && body.attribution?.token) {
        attribution = body.attribution;
        sessionStorage.setItem(`ekodiMallAttribution:${code}`, JSON.stringify(body.attribution));
      }
    } catch {}
  }
  ...
  function storedAttribution() {
    ...
    try {
      const value = JSON.parse(sessionStorage.getItem(`ekodiMallAttribution:${code}`) || 'null');
  ...
  async function nativeShare() {
    const data = {
      title: product?.product?.name || 'EKODI MALL 상품',
      text: product?.product?.oneLine || '',
      url: location.href
    };
  ...
  function smsShare() {
    location.href = `sms:?&body=${encodeURIComponent(`${product?.product?.name || '상품'}\n${location.href}`)}`;
  }
  ...
  function quoteBlock(quote) {
    ...
    const sourceLabel = quote.attributionType === 'direct'
      ? '판매자 직접공유'
      : quote.attributionType === 'ai'
        ? 'AI 기여'
        : 'EKODI Mall';
  ...
  async function render(data) {
    ...
    main.append(
      el('p', 'eyebrow', data.store ? 'STORE PRODUCT' : 'PERSONAL PRODUCT'),
      el('h1', '', data.product.name),
      el('p', 'detail-desc', data.product.oneLine || data.product.audience || '상품 소개')
    );
    ...
    const copy = el('button', 'btn primary', '링크 복사');
    copy.type = 'button';
    copy.addEventListener('click', copyLink);
    const share = el('button', 'btn ghost', '공유');
    share.type = 'button';
    share.addEventListener('click', nativeShare);
    const sms = el('button', 'smallbtn', '문자');
    sms.type = 'button';
    sms.addEventListener('click', smsShare);
    actions.append(copy, share, sms);

    if (refCode) {
      const canonical = el('a', 'smallbtn', '일반 상품링크');
    ...
    if (data.product.saleType === 'affiliate' && data.product.affiliateUrl) {
      ...
      affiliate.href = data.product.affiliateUrl;
      affiliate.target = '_blank';
      affiliate.rel = 'noopener sponsored';
      actions.prepend(affiliate);
    }
    main.append(actions);
    ...
    const feeText = data.seller.type === 'individual'
      ? '개인상품은 판매자 서버발급 공유링크 7%, 일반 EKODI Mall 유입 8%, AI 내부발급 링크 9%입니다. PG·플랫폼 수수료 VAT 포함이며 브라우저 URL 이름이 아니라 서버 attribution token으로 판정합니다.'
    ...
    if (status) status.textContent = refCode ? '공개 상품 · 추적 공유링크' : '공개 상품 · EKODI Mall 기본경로';
  }
