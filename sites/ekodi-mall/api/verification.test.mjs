...
const readyIndividual = {
  ...
  status: 'published',
  price: 10000,
  direct_sale_status: 'verified',
  seller_type: 'individual',
  store_id: null,
  store_verification_status: null,
  checkout_ready: 1
};

test('checkout gate eligibility is independent from global payment activation', () => {
...
test('seller and business store verification are hard blockers', () => {
  ...
  assert.deepEqual(checkoutGateBlockers({
    ...readyIndividual,
...
test('product must be direct, published and have a confirmed positive price before checkout gate', () => {
  assert.deepEqual(checkoutGateBlockers({
    ...readyIndividual,
    sale_type: 'inquiry',
    status: 'draft',
    price: null
  }), ['not-direct-sale', 'product-not-published', 'price-not-confirmed']);
});

test('manual checkout gate remains a separate auditable blocker', () => {
  assert.deepEqual(livePaymentBlockers({ ...readyIndividual, checkout_ready: 0 }, {
    PAYMENTS_ENABLED: 'true',
    TOSS_SECRET_KEY: 'configured'
  }), ['product-checkout-gate']);
});
