...
export function trustedSource(sourceType = '') { return sourceType === 'direct' || sourceType === 'ai' ? sourceType : 'marketplace'; }
...
async function firstTouch(env, input = {}) {
  ...
  const token = `att_${randomCode(24)}`; const expiresAt = addDaysIso(ATTRIBUTION_WINDOW_DAYS); const id = existing?.id || visitId();
  await env.DB.batch([
    env.DB.prepare('INSERT INTO attribution_tokens (token,product_id,source_type,channel,created_at,expires_at) VALUES (?,?,?,?,?,?)').bind(token, row.id, sourceType, channel, now, expiresAt),
...
async function attributionSchemaReady(env) {
  try { const row = await env.DB.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='attribution_visits'").first(); return row?.name === 'attribution_visits'; }
  catch { return false; }
}
...
export class MallCatalog {
  ...
  async fetch() {
    return new Response(JSON.stringify({ error: 'LEGACY_MALL_CATALOG_RETIRED', message: 'Legacy MallCatalog storage is preserved but no longer serves EKODI Mall traffic.' }), {
      status: 410, headers: { 'content-type':'application/json; charset=utf-8', 'cache-control':'no-store', 'x-content-type-options':'nosniff' }
    });
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/api/public/fees') return reply({ individual:FEE_RATES,businessStore:10,pgIncluded:true,vatIncluded:true,proAiSubscriptionSeparate:true,attributionWindowDays:ATTRIBUTION_WINDOW_DAYS,serverAuthoritative:true },200,origin,env);
