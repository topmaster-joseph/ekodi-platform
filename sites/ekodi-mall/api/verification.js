...
function operationsAuthorization(request, env) {
  ...
  if (supplied !== env.MALL_OPERATIONS_TOKEN) return { ok: false, status: 401, error: 'Mall 운영자 권한이 필요합니다.' };
  return { ok: true };
}
...
async function ensureSellerProfile(env, user, sellerType) {
  ...
  return env.DB.prepare(`SELECT user_id AS userId,email,display_name AS displayName,seller_type AS sellerType,
    verification_status AS verificationStatus,direct_sale_status AS directSaleStatus
    FROM seller_profiles WHERE user_id=?`).bind(user.id).first();
}
...
async function activeRequest(env, entityType, entityId) {
  return env.DB.prepare(`SELECT id,entity_type AS entityType,entity_id AS entityId,status,request_note AS requestNote,
    ...
    FROM verification_requests WHERE entity_type=? AND entity_id=? AND status IN ('submitted','under_review')
    ORDER BY created_at DESC LIMIT 1`).bind(entityType, entityId).first();
}
...
function productReadiness(row, env) {
  ...
  return {
    ...
    storeVerificationStatus: row.store_verification_status || null,
    checkoutGateEnabled: Boolean(row.checkout_ready),
    eligibleForCheckoutGate: gateBlockers.length === 0,
    gateBlockers,
    livePaymentReady: liveBlockers.length === 0,
    liveBlockers
  };
}
...
async function sellerReadiness(env, sellerId) {
  const profile = await env.DB.prepare(`SELECT user_id AS userId,email,display_name AS displayName,seller_type AS sellerType,
    verification_status AS verificationStatus,direct_sale_status AS directSaleStatus
    FROM seller_profiles WHERE user_id=?`).bind(sellerId).first();
  const storesResult = await env.DB.prepare(`SELECT id,slug,name,status,verification_status AS verificationStatus,updated_at AS updatedAt
    FROM stores WHERE seller_id=? ORDER BY updated_at DESC LIMIT 50`).bind(sellerId).all();
  ...
  return {
    ...
    summary: {
      storeCount: storesResult.results?.length || 0,
...
async function audit(env, { actor, action, sellerId = null, storeId = null, productId = null, requestIdValue = null, metadata = {} }) {
  await env.DB.prepare(`INSERT INTO mall_ops_audit
    (actor,action,seller_id,store_id,product_id,request_id,metadata_json,created_at)
    VALUES (?,?,?,?,?,?,?,?)`)
...
async function reviewRequest(env, id, body) {
  ...
  const statements = [
    env.DB.prepare(`UPDATE verification_requests SET status=?,review_note=?,reviewed_at=?,updated_at=? WHERE id=?`)
      .bind(decision, reviewNote, now, now, id)
  ];
  let storeId = null;
  if (request.entity_type === 'seller') {
    statements.push(env.DB.prepare('UPDATE seller_profiles SET direct_sale_status=?,updated_at=? WHERE user_id=?')
      .bind(decision, now, request.seller_id));
  } else if (request.entity_type === 'store') {
    storeId = request.entity_id;
    if (decision === 'verified') {
      statements.push(env.DB.prepare("UPDATE stores SET verification_status='verified',status='active',updated_at=? WHERE id=? AND seller_id=?")
        .bind(now, request.entity_id, request.seller_id));
    } else {
      statements.push(env.DB.prepare("UPDATE stores SET verification_status='rejected',updated_at=? WHERE id=? AND seller_id=?")
        .bind(now, request.entity_id, request.seller_id));
    }
  }
  ...
  await audit(env, {
    ...
    storeId,
    requestIdValue: id,
    metadata: { entityType: request.entity_type, reviewNotePresent: Boolean(reviewNote) }
  });
...
async function setCheckoutGate(env, productId, body) {
  ...
  const now = nowIso();
  await env.DB.prepare('UPDATE products SET checkout_ready=?,updated_at=? WHERE id=?')
    .bind(body.ready ? 1 : 0, now, productId).run();
  await audit(env, {
    actor: `mall-ops:${clean(body?.reviewer, 80) || 'operator'}`,
    action: body.ready ? 'product.checkout_gate.enabled' : 'product.checkout_gate.disabled',
    sellerId: row.seller_id,
    storeId: row.store_id || null,
    productId,
    metadata: { note: clean(body?.note, 800), blockersAtDecision: blockers }
  });
  const updated = { ...row, checkout_ready: body.ready ? 1 : 0 };
  return { status: 200, body: { product: productReadiness(updated, env) } };
}
...
export async function handleVerificationRequest(request, env) {
  ...
  const isVerificationRoute = path === '/api/readiness' || path === '/api/verification/requests' || sellerVerificationSubmit || Boolean(storeVerificationMatch)
  ...
  if (path.startsWith('/api/internal/')) {
    ...
    if (request.method === 'GET' && path === '/api/internal/verification/queue') {
      const requestedStatus = clean(url.searchParams.get('status'), 30) || 'submitted';
      const status = QUEUE_STATUSES.has(requestedStatus) ? requestedStatus : 'submitted';
      const rows = await env.DB.prepare(`SELECT vr.id,vr.seller_id AS sellerId,vr.entity_type AS entityType,vr.entity_id AS entityId,vr.status,
        ...
        FROM verification_requests vr JOIN seller_profiles sp ON sp.user_id=vr.seller_id
        WHERE vr.status=? ORDER BY vr.created_at ASC LIMIT 100`).bind(status).all();
  ...
  if (request.method === 'GET' && path === '/api/readiness') {
    return { status: 200, body: { readiness: await sellerReadiness(env, user.id) } };
  }
