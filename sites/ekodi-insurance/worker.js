...
export default {async fetch(request,env){const url=new URL(request.url);const production=isProduction(env);
  ...
  if(['/guide/car-insurance','/guide/car-insurance/','/advisor/guide/car-insurance','/advisor/guide/car-insurance/'].includes(url.pathname)){const lang=preferredGuideLanguage(request);const base=url.pathname.startsWith('/advisor/')?'/advisor/guide/car-insurance':'/guide/car-insurance';return Response.redirect(`${url.origin}${base}/${lang}`,302);}
  const carGuideMatch=url.pathname.match(/^\/(advisor\/)?guide\/car-insurance\/([^/]+)\/?$/i);
  if(carGuideMatch){const lang=supportedGuideLanguage(carGuideMatch[2]);const base=carGuideMatch[1]?'/advisor/guide/car-insurance':'/guide/car-insurance';if(!lang)return Response.redirect(`${url.origin}${base}/ko`,302);if(carGuideMatch[2]!==lang)return Response.redirect(`${url.origin}${base}/${lang}`,302);return carGuideResponse(request,env,production,url,lang);}
