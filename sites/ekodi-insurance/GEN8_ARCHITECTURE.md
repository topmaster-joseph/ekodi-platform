...
`ekodi.kr/admin → ekodi.kr/api → Insurance API → Insurance D1` 순서다. 중앙 경로도 `INSURANCE_ADMIN_ENABLED=false`가 기본값이다.
...
이 조건 전에는 `ekodi.kr/ins`의 실제 상품비교·추천·자동청약 기능을 열지 않는다.
