...
Clean RC 위에 Partner Registry, Reference Catalog, Human Handoff, Outcome Feedback 계층을 추가했습니다. 개인 보험관리와 운영자 제휴망을 분리하며, 자세한 구조는 `GEN8_ARCHITECTURE.md`를 따릅니다.

- 개인: `Insurance Care Workspace`에서 보험정리·청구준비·상담연결을 사용합니다.
- 운영: `Insurance Operations Workspace`에서 제휴사·승인자료·상담성과를 관리합니다.
- 공개 비교: 기본 OFF이며 승인된 제휴 + signed 계약 + 승인 자료 + 별도 공개 플래그가 모두 충족되어야 참고자료만 표시합니다.
- 특정 상품 순위·적합성 추천·자동청약은 현재 제공하지 않습니다.
- Development staging 기준 UI/API는 `ekodi-development.workers.dev` 계정 경계를 사용합니다.
