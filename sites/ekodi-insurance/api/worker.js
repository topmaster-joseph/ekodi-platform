...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && url.pathname === '/health') {
      ...
      const networkDbReady = await networkReady(env);
      ...
      const ok = dbReady && networkDbReady && encryptionReady && internalAdminReady && rateLimitReady;
      return json({
        ok,
    ...
    const networkResult = await handleInsuranceNetwork(request, env);
    if (networkResult) return json(networkResult.body, networkResult.status, origin, env);
