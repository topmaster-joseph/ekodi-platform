...
async function putAdvisorProfile(request,env){
  ...
  const now=new Date().toISOString();
  await env.DB.prepare('UPDATE insurance_advisor_profiles SET display_name=?,insurer_name=?,role_label=?,intro=?,registration_reference=?,verification_url=?,official_company_url=?,wonder_official_url=?,direct_design_url=?,advertising_review_ref=?,advertising_review_expires_at=?,public_enabled=?,updated_at=? WHERE id=?').bind(next.displayName,next.insurerName,next.roleLabel,next.intro,next.registrationReference,next.verificationUrl,next.officialCompanyUrl,next.wonderOfficialUrl,next.directDesignUrl,next.advertisingReviewRef,next.advertisingReviewExpiresAt||null,publicEnabled?1:0,now,'adv_primary').run();
