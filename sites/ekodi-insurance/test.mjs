...
for(const marker of ["SUPPORTED_LANGUAGES=['ko','en','zh-CN','vi']",'Liability Insurance','汽车责任保险','Bảo hiểm trách nhiệm','navigator.share','navigator.clipboard.writeText','advisorScoped()'])if(!carGuideJs.includes(marker))throw new Error(`multilingual car guide behavior missing: ${marker}`);
for(const marker of ['/guide/car-insurance','/advisor/guide/car-insurance','multilingualCarGuide:true','advisorMultilingualGuide:true','CAR_GUIDE_META','injectBridge:false','content-language'])if(!worker.includes(marker))throw new Error(`multilingual car guide route missing: ${marker}`);
for(const marker of ['id="foreignGuide"','/advisor/guide/car-insurance/ko','/advisor/guide/car-insurance/en','/advisor/guide/car-insurance/zh-CN','/advisor/guide/car-insurance/vi'])if(!advisorHtml.includes(marker))throw new Error(`advisor multilingual entry missing: ${marker}`);
if(!carGuideHtml.includes('id="advisorBackLink"'))throw new Error('advisor guide back-link missing');
