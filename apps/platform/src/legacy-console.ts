// @ts-nocheck -- preserved v3 controller; new platform features use typed React modules.
...
const viewMeta = {
  ...
  cms: ['콘텐츠 CMS', '모든 EKODI 사이트의 페이지 초안, 게시 상태와 콘텐츠를 관리합니다.'],
...
function activateView(view, options = {}) {
  ...
  const map = {
    ...
    cms: document.querySelector('.cms-panel'),
  ...
  Object.entries(map).forEach(([key, element]) => {
    const visible = (isDashboard && key !== 'cms') || key === view || (view === 'domains' && key === 'registrar');
...
const MONITOR_URL = '/monitor-status.json';
...
const cmsPageList = document.querySelector('#cmsPageList');
const cmsEditor = document.querySelector('#cmsEditor');
const cmsSiteFilter = document.querySelector('#cmsSiteFilter');
const publishCmsPage = document.querySelector('#publishCmsPage');
const mediaUploadForm = document.querySelector('#mediaUploadForm');
const cmsMediaList = document.querySelector('#cmsMediaList');
let cmsPages = [];

function resetCmsEditor() {
  cmsEditor.reset();
  cmsEditor.elements.id.value = '';
  cmsEditor.elements.version.value = '';
  cmsEditor.elements.siteId.disabled = false;
  cmsEditor.elements.slug.disabled = false;
  publishCmsPage.disabled = true;
  document.querySelector('#cmsEditorStatus').textContent = '새 초안';
  document.querySelectorAll('.cms-page-item').forEach(item => item.classList.remove('active'));
}

function openCmsPage(page) {
  cmsEditor.elements.id.value = page.id;
  cmsEditor.elements.version.value = page.version;
  cmsEditor.elements.siteId.value = page.siteId;
  cmsEditor.elements.slug.value = page.slug;
  cmsEditor.elements.title.value = page.title;
  cmsEditor.elements.summary.value = page.summary;
  cmsEditor.elements.content.value = page.content;
  cmsEditor.elements.classification.value = page.classification || 'public';
  cmsEditor.elements.siteId.disabled = true;
  cmsEditor.elements.slug.disabled = true;
  publishCmsPage.disabled = (page.classification || 'public') !== 'public';
  document.querySelector('#cmsEditorStatus').textContent = `${page.status === 'published' ? '게시됨' : '초안'} · v${page.version} · ${new Date(page.updatedAt).toLocaleString('ko-KR')}`;
  document.querySelectorAll('.cms-page-item').forEach(item => item.classList.toggle('active', Number(item.dataset.pageId) === page.id));
}

function renderCmsPages(pages) {
  cmsPages = pages;
  cmsPageList.innerHTML = '';
  if (!pages.length) {
    const empty = document.createElement('p');
    empty.className = 'domain-loading';
    empty.textContent = '아직 등록된 CMS 페이지가 없습니다.';
    cmsPageList.append(empty);
    return;
  }
  pages.forEach(page => {
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'cms-page-item';
    button.dataset.pageId = page.id;
    const title = document.createElement('strong');
    title.textContent = page.title;
    const path = document.createElement('small');
    path.textContent = `${page.siteId} / ${page.slug}`;
    const meta = document.createElement('span');
    meta.className = 'cms-page-meta';
    const status = document.createElement('span');
    status.className = `cms-status ${page.status}`;
    status.textContent = page.status === 'published' ? '게시됨' : '초안';
    const version = document.createElement('span');
    version.textContent = `v${page.version}`;
    meta.append(status, version);
    button.append(title, path, meta);
    button.addEventListener('click', () => openCmsPage(page));
    cmsPageList.append(button);
  });
}

async function loadCmsPages() {
  if (!sessionStorage.getItem('ekodi-auth-token')) return;
  const site = cmsSiteFilter.value;
  cmsPageList.innerHTML = '<p class="domain-loading">콘텐츠를 불러오는 중입니다.</p>';
  try {
    const response = await fetch(`${AUTH_API}/api/cms/pages${site ? `?site=${encodeURIComponent(site)}` : ''}`, { headers: authHeaders() });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'CMS 콘텐츠를 불러오지 못했습니다.');
    renderCmsPages(data.pages || []);
  } catch (error) {
    cmsPageList.textContent = error.message;
  }
}
...
async function loadCmsMedia() {
  if (!sessionStorage.getItem('ekodi-auth-token')) return;
  const site = cmsSiteFilter.value;
  cmsMediaList.innerHTML = '<p class="domain-loading">미디어를 불러오는 중입니다.</p>';
  try {
    const response = await fetch(`${AUTH_API}/api/cms/media${site ? `?site=${encodeURIComponent(site)}` : ''}`, { headers: authHeaders() });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || '미디어를 불러오지 못했습니다.');
    cmsMediaList.innerHTML = '';
    if (!data.assets.length) {
      cmsMediaList.innerHTML = '<p class="domain-loading">등록된 미디어가 없습니다.</p>';
      return;
    }
    data.assets.forEach(asset => {
      ...
      meta.textContent = `${asset.siteId} · ${asset.visibility === 'public' ? '공개' : '비공개'} · ${Math.ceil(asset.size / 1024)}KB`;
      ...
      remove.addEventListener('click', async () => {
        ...
        try {
          const response = await fetch(`${AUTH_API}/api/cms/media/${asset.id}`, { method: 'DELETE', headers: authHeaders() });
          const data = await response.json();
          if (!response.ok) throw new Error(data.error || '미디어를 삭제하지 못했습니다.');
          notify('미디어를 삭제했습니다.');
          await loadCmsMedia();
        } catch (error) {
      ...
      cmsMediaList.append(row);
    });
  } catch (error) {
    cmsMediaList.textContent = error.message;
  }
}

mediaUploadForm?.addEventListener('submit', async event => {
  event.preventDefault();
  const submit = mediaUploadForm.querySelector('button[type="submit"]');
  submit.disabled = true;
  const form = new FormData(mediaUploadForm);
  form.set('visibility', form.get('visibility') === 'public' ? 'public' : 'private');
  try {
    const response = await fetch(`${AUTH_API}/api/cms/media`, {
      method: 'POST', headers: { authorization: `Bearer ${sessionStorage.getItem('ekodi-auth-token') || ''}` }, body: form
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || '미디어를 업로드하지 못했습니다.');
...
cmsEditor?.addEventListener('submit', async event => {
  event.preventDefault();
  const form = new FormData(cmsEditor);
  const id = Number(form.get('id'));
  const payload = {
    siteId: cmsEditor.elements.siteId.value,
    slug: cmsEditor.elements.slug.value,
    title: form.get('title'),
    summary: form.get('summary'),
    content: form.get('content'),
    classification: form.get('classification'),
    ...(id ? { version: Number(form.get('version')) } : {})
  };
  const submit = document.querySelector('#saveCmsDraft');
  submit.disabled = true;
  try {
    const response = await fetch(`${AUTH_API}/api/cms/pages${id ? `/${id}` : ''}`, {
      method: id ? 'PUT' : 'POST', headers: authHeaders(true), body: JSON.stringify(payload)
    });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'CMS 초안을 저장하지 못했습니다.');
    await Promise.all([loadCmsPages(), loadAudit()]);
    openCmsPage(data.page);
    notify('CMS 초안을 안전하게 저장했습니다.');
  } catch (error) {
    notify(error.message);
  } finally {
    submit.disabled = false;
...
cmsEditor?.addEventListener('input', () => {
  publishCmsPage.disabled = true;
});

publishCmsPage?.addEventListener('click', async () => {
  const id = Number(cmsEditor.elements.id.value);
  if (!id) return;
  publishCmsPage.disabled = true;
  try {
    const response = await fetch(`${AUTH_API}/api/cms/pages/${id}/publish`, { method: 'POST', headers: authHeaders() });
    const data = await response.json();
    if (!response.ok) throw new Error(data.error || 'CMS 페이지를 게시하지 못했습니다.');
    await Promise.all([loadCmsPages(), loadAudit()]);
    openCmsPage(data.page);
    notify('CMS 페이지를 게시했습니다.');
  } catch (error) {
    notify(error.message);
  } finally {
    publishCmsPage.disabled = false;
  }
});

document.querySelector('#newCmsPage')?.addEventListener('click', resetCmsEditor);
document.querySelector('#refreshCms')?.addEventListener('click', () => Promise.all([loadCmsPages(), loadCmsMedia()]));
cmsSiteFilter?.addEventListener('change', () => Promise.all([loadCmsPages(), loadCmsMedia()]));
...
const auditLabels = {
  ...
  'cms.create': 'CMS 페이지 생성',
  'cms.save': 'CMS 초안 저장',
  'cms.publish': 'CMS 페이지 게시',
