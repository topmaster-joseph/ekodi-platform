...
function absoluteUrl(path){if(!path)return'https://ekodi.kr/books/';try{return new URL(path,'https://ekodi.kr/books/').href}catch{return'https://ekodi.kr/books/';}}
...
function injectStructuredData(data,books){const graph=[{'@type':'Organization','@id':'https://ekodi.kr/books/#publisher',name:data.publisher||'에코디서점',url:'https://ekodi.kr/books/'},{'@type':'WebSite','@id':'https://ekodi.kr/books/#website',name:'에코디서점',url:'https://ekodi.kr/books/',publisher:{'@id':'https://ekodi.kr/books/#publisher'},inLanguage:'ko-KR'}];books.forEach(book=>{graph.push({'@type':'Book',name:book.title,alternateName:book.editions?.amazonEnglish?.title||undefined,description:book.abstract||book.subtitle||undefined,author:{'@type':'Person',name:book.author},publisher:{'@id':'https://ekodi.kr/books/#publisher'},bookFormat:'https://schema.org/EBook',inLanguage:Array.isArray(book.language)?book.language:[book.language||'ko'],identifier:book.identifiers?.googleBooks||book.identifiers?.isbnEbook||undefined,image:book.coverImage?absoluteUrl(book.coverImage):undefined,isPartOf:book.series?{'@type':'BookSeries',name:book.series,position:book.seriesNumber||undefined}:undefined,url:absoluteUrl(book.detailUrl||'/#catalog'),offers:(book.links?.upaper||book.links?.korea)?{'@type':'Offer',price:String(book.listPrice?.amount||9900),priceCurrency:book.listPrice?.currency||'KRW',availability:'https://schema.org/InStock',url:book.links?.upaper||book.links?.korea}:undefined});});const script=document.createElement('script');script.type='application/ld+json';script.textContent=JSON.stringify({'@context':'https://schema.org','@graph':graph});document.head.append(script);}
...
function renderBook(book){
  ...
  const editions=book.editions||{};const upaper=editions.upaperKoreanBilingual||{};const dist=node.querySelector('.distribution-list');
  dist.append(distributionRow('Google Play Books',identifier||editions.googleKoreanBilingual?.price||'',book.distribution?.google),distributionRow('Amazon KDP',['English Edition',editions.amazonEnglish?.price].filter(Boolean).join(' · '),book.distribution?.amazon),distributionRow('국내 판매',upaper.provider?[upaper.provider,upaper.price].filter(Boolean).join(' · '):(upaper.price||''),book.distribution?.upaper||book.distribution?.korea));
  const links=node.querySelector('.book-links');links.append(linkFor('Google Play',book.links?.google,book.distribution?.google),linkFor('Amazon',book.links?.amazon,book.distribution?.amazon),linkFor('유페이퍼 구매',book.links?.upaper||book.links?.korea,book.distribution?.upaper||book.distribution?.korea));
