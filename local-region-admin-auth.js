function clientMain(){
  ...
  function showMessage(title,copy,actions=[]){
    const main=document.querySelector('main')||document.body;main.dataset.regionAuthMessage='true';main.style.visibility='visible';main.replaceChildren();
  ...
  async function check(){
    ...
    if(!response.ok){root.dataset.regionAuthDenied='1';showMessage('관리 권한이 없습니다',data.error||'이 이메일에는 해당 관리공간 권한이 등록되어 있지 않습니다.',[{href:pass?'/cheonggye/pass':'/cheonggye',label:'사용자 페이지'},{href:authUrl(),label:'다른 Google 계정으로 로그인'}]);return}
  ...
  check().catch(error=>{console.error('regional admin auth',error);root.dataset.regionAuthDenied='1';showMessage('인증 확인 실패','관리자 로그인 상태를 확인하지 못했습니다. 다시 로그인해 주세요.',[{href:authUrl(),label:'Google 로그인'}]);});
}
