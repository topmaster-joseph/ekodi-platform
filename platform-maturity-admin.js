...
`;
  ...
  function renderServiceScopes(serviceScopes) {
    if (!serviceScopes?.summary) return null;
    const rows = [
      ...(serviceScopes.services || []).map(item => ({ ...item, detail:item.category || item.lifecycleStatus || 'service' })),
      ...(serviceScopes.workspaceSites || []).map(item => ({ ...item, detail:item.ownerKind || item.siteClass || 'workspace-site' })),
      ...(serviceScopes.systemFunctions || []).map(item => ({ ...item, detail:item.domain || item.implementationMaturity || 'system-function' })),
    ];
    const panel = document.createElement('article'); panel.className = 'ekodi-maturity-panel'; panel.dataset.maturityScope = 'subservices';
    const title = document.createElement('h3'); title.textContent = '하위서비스·사이트 적용범위';
    const summary = document.createElement('div'); summary.className = 'ekodi-maturity-scope-summary';
    const cards = [
      ['등록 서비스', serviceScopes.summary.services, `Live ${serviceScopes.summary.liveServices}`],
      ['운영 검증 서비스', serviceScopes.summary.productionVerifiedServices, `추가 증빙 ${serviceScopes.summary.servicesRequiringLocalEvidence}`],
      ['Workspace 사이트', serviceScopes.summary.workspaceSites, '각 사이트 운영권한 유지'],
      ['시스템 기능', serviceScopes.summary.systemFunctions, `Service-backed ${serviceScopes.summary.serviceBackedFunctions}`],
      ['총 적용범위', serviceScopes.summary.totalScopes, '서비스·사이트·기능별 점수는 증빙 전 미평가'],
    ];
    cards.forEach(([label, value, copy]) => { const card=document.createElement('div'); card.className='ekodi-maturity-card'; const small=document.createElement('small'); small.textContent=label; const strong=document.createElement('strong'); strong.textContent=String(value??0); const span=document.createElement('span'); span.textContent=copy; card.append(small,strong,span); summary.append(card); });
    const scroll=document.createElement('div'); scroll.className='ekodi-maturity-scope-scroll';
    const table=document.createElement('table'); table.className='ekodi-maturity-scope-table'; table.innerHTML='<thead><tr><th>범위</th><th>구분</th><th>운영 상태</th><th>성숙도 평가</th></tr></thead><tbody></tbody>';
    const body=table.querySelector('tbody');
    rows.forEach(item => {
      const tr=document.createElement('tr'); tr.dataset.maturityScopeRow=item.id;
      const name=document.createElement('td'); const strong=document.createElement('strong'); strong.textContent=item.name||item.id; const small=document.createElement('small'); small.textContent=item.id; name.append(strong,small);
      const kind=document.createElement('td'); kind.textContent=item.scopeType==='workspace-site'?'Workspace 사이트':item.scopeType==='system-function'?'시스템 기능':'서비스';
      const state=document.createElement('td'); const tag=document.createElement('span'); tag.className='ekodi-maturity-scope-tag'; tag.textContent=item.scopeType==='workspace-site'?(item.migrationState||'등록'):item.scopeType==='system-function'?`${item.domain||'기능'} · ${item.implementationMaturity||'등록'}`:`${item.lifecycleStatus||'등록'}${item.productionVerified?' · 운영검증':''}`; state.append(tag);
      const assessment=document.createElement('td'); assessment.textContent=item.localMaturityScore==null?'미평가 · 로컬 증빙 필요':String(item.localMaturityScore);
      tr.append(name,kind,state,assessment); body.append(tr);
    });
    scroll.append(table);
    const note=document.createElement('p'); note.className='ekodi-maturity-note'; note.textContent='플랫폼 공통 보안·개인정보·AI·서비스관리 통제는 하위 범위에 적용하지만, 각 서비스·사이트·시스템 기능의 숫자 성숙도는 해당 운영 증빙이 확보되기 전까지 만들지 않습니다. 서비스/사이트 운영의 원본은 각 관리자 화면에 유지됩니다.';
    panel.append(title,summary,scroll,note); return panel;
  }

  function render(model, current, history, serviceScopes) {
    ...
    const scopePanel = renderServiceScopes(serviceScopes); if (scopePanel) view.append(scopePanel);
  ...
  async function loadData(force = false) {
    ...
    loading = (async () => {
      ...
      try {
        ...
        render(payload.model, payload.current, history, payload.serviceScopes);
        loaded = true;
      } catch (error) {
