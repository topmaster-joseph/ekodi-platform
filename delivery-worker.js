...
function runtimeConfig(env={}){return{dataMode:env.DATA_MODE||'isolated-staging',authUrl:env.AUTH_URL||'https://auth.ekodi.kr/?site=delivery',centralIdentity:centralIdentityConfig(env),canonicalUrl:'https://ekodi.kr/delivery',accessPolicy:'public-guide-free-member-workspace',providerAdapterCount:0,providerAdapters:'official-adapters-only',executionEnabled:false,persistence:'browser-local-non-sensitive-mvp',aiMode:'provider-independent-decision-support',externalMutation:false,humanConfirmationRequired:true}}
...
export default{async fetch(request,env){
  ...
  if(pathname==='/health')return json({ok:true,service:'ekodi-delivery-hub-ai',surface:'delivery-platform',canonicalPath:'/delivery',dataMode:runtimeConfig(env).dataMode,accessPolicy:'public-guide-free-member-workspace',providerAdapterCount:0,executionEnabled:false,externalMutation:false,humanConfirmationRequired:true,aiMode:'provider-independent-decision-support',centralIdentityEnabled:centralIdentityConfig(env).enabled,ekodiShell:true},200,env);
