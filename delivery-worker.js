...
function securityHeaders(env={}){
  ...
  return{
    ...
    'content-security-policy':`default-src 'self'; script-src ${scriptSrc.join(' ')}; style-src 'self'; img-src 'self' data: https:; connect-src ${connectSrc.join(' ')}; frame-ancestors 'none'; base-uri 'self'; form-action 'self' https://ekodi.kr/auth; object-src 'none'; upgrade-insecure-requests`,
  };
}
