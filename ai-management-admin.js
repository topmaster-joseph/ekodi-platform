...
function render(){
  ...
  const policy=snapshot.policy||{},resources=policy.resources||{},funding=resources.funding||runtime?.costPolicy||{},core=resources.core||{},resourceRouter=resources.router||{},providerRouter=policy.router||runtime?.routerScorePolicy||{};
  ...
  root.innerHTML+=`<article class="ai-mgmt-card wide"><div class="ai-mgmt-card-head"><div><small>COST CONSTITUTION</small><h3>FREE FIRST · PAID OPT-IN</h3></div><span>AI-COST-001 · LOCK</span></div><div class="ai-mgmt-guards"><span>FREE FIRST · LOCK</span><span>PAID AUTO ESCALATION ${funding.paidApiAutoEscalation===false?'OFF':'BLOCKED'}</span><span>AUTO PAID BUDGET KRW ${Number(funding.automaticPaidBudgetKrw)||0}</span><span>EXPLICIT DELEGATED BUDGET REQUIRED</span></div><p class="ai-mgmt-note">${t('무료·기존 구독·무료 quota를 우선 사용하고, 유료 API는 명시적 위임 예산이 있는 작업에서만 허용합니다.','Free, included-subscription and free-quota resources are preferred. Paid APIs require an explicit delegated budget.')}</p></article>`;
  ...
  root.innerHTML+=`<article class="ai-mgmt-card wide"><div class="ai-mgmt-card-head"><div><small>EKODI LEARNING LOOP</small><h3>${t('수집보다 검증을 우선하는 학습','Verify-first continuous learning')}</h3></div><span>v${esc(snapshot.learningLoop?.version||'-')}</span></div><div class="ai-mgmt-guards"><span>API · CONNECTOR FIRST</span><span>CHANGED-ONLY FETCH</span><span>EXTERNAL CONTENT = UNTRUSTED DATA</span><span>RAW USER CONTENT DEFAULT OFF</span><span>PLATFORM PROMOTION HUMAN GATE</span></div><p class="ai-mgmt-note">${t('외부지식은 API·Webhook·RSS·Sitemap을 우선 사용하고, HTML 수집은 robots·약관·권리 확인 뒤 제한적으로 사용합니다. 실제 작업의 검증 결과를 학습 원장에 축적하고 플랫폼 공통지식 승격은 사람 승인을 요구합니다.','External knowledge prefers APIs, webhooks, RSS and sitemaps. HTML collection is bounded behind robots, terms and rights checks. Verified work outcomes enter the learning ledger; platform-wide promotion requires human approval.')}</p></article>`;
...
window.EKODIAIManagement=Object.freeze({mount,load,save,version:'1.2.0'});
})();
