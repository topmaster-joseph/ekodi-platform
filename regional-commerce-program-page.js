...
function doc(region,program,title,body,surface){
  return `<!doctype html><html lang="ko" data-ekodi-site-subject="${esc(region.siteSubject)}" data-ekodi-local-region="${esc(region.id)}" data-ekodi-commerce-program="${esc(program.id)}" data-ekodi-region-surface="${surface}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title>${style()}</head><body>${body}</body></html>`;
}
...
export function regionalCommerceProgramAdminPage(region,program){
  ...
  <section class="panel"><h2>외주사 연동 계약</h2><div class="grid"><div class="item"><strong>필수</strong><span class="muted">상태확인, 가맹점 동기화, 혜택 동기화, 거래·정산 요약조회, 서명 웹훅</span></div><div class="item"><strong>선택</strong><span class="muted">쿠폰발급, 포인트적립, 상품권발행, SSO</span></div><div class="item"><strong>교체</strong><span class="muted">외주사 변경 시 어댑터와 자격증명만 변경</span></div><div class="item"><strong>보존</strong><span class="muted">지역 운영권·감사이력·공개주소는 유지</span></div></div><a class="button" href="/cheonggye/admin">지역플랫폼 운영관리</a></section></main>`;
