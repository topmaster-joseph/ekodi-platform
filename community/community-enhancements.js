(() => {
  ...
  function bind() {
    document.addEventListener('click', event => { const el=event.target?.closest?.('[data-auth-href]'); if(!el)return; event.preventDefault(); openTarget(el.dataset.authHref || el.getAttribute('href'), el.dataset.authTarget || el.getAttribute('target') || '_self'); });
