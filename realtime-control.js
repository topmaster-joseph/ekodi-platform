...
async function translateLiveInterpretation(request,env,text){const authorization=clean(request.headers.get('authorization'),8192);if(!authorization||!env.CONTROL_API?.fetch)return{translations:{},status:'degraded',provider:'',model:'',error:'translation_gateway_unavailable'};const system='Translate Korean live worship speech faithfully and concisely into English, Simplified Chinese, Japanese, Vietnamese, and Mongolian. Preserve Bible references, names, numbers, and meaning. Do not add explanations or invented content. Return ONLY strict JSON with exactly these keys: {"en":"...","zh":"...","ja":"...","vi":"...","mn":"..."}.';try{const upstream=await env.CONTROL_API.fetch(new Request('https://api.ekodi.kr/api/ai-modules/v1/providers/generate',{method:'POST',headers:{authorization,'content-type':'application/json',origin:'https://ekodi.kr'},body:JSON.stringify({capability:'translation',system,input:text,maxOutputTokens:1400})}));const data=await upstream.json().catch(()=>({}));if(!upstream.ok)throw new Error(data?.error||`translation_gateway_${upstream.status}`);const translations=parseInterpretationJson(data?.text);if(!translations)throw new Error('translation_payload_invalid');return{translations,status:'ready',provider:clean(data?.provider?.id,80),model:clean(data?.provider?.model,120),error:''}}catch(error){return{translations:{},status:'degraded',provider:'',model:'',error:clean(error?.message||error,160)}}}
