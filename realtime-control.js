...
async function recordingRoutes(request,env,url,input){
  ...
  if(item&&['PATCH','DELETE'].includes(request.method)){
    ...
    if(recording.drive_file_id){
      if(!env.STORAGE?.fetch)return json(request,env,{ok:false,error:'recording_delete_storage_not_configured'},503);
      const archiveDelete=await env.STORAGE.fetch(new Request('https://storage.internal/api/storage/v1/delete-file',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({fileId:recording.drive_file_id})})).catch(()=>null);
      if(!archiveDelete?.ok)return json(request,env,{ok:false,error:'recording_delete_archive_failed'},502);
    }
    if(recording.storage_key){
      if(!env.LIVE_RECORDINGS_BUCKET)return json(request,env,{ok:false,error:'recording_delete_storage_not_configured'},503);
      try{await env.LIVE_RECORDINGS_BUCKET.delete(recording.storage_key)}
      catch(error){console.error('recording delete object',recording.id,error?.message||error);return json(request,env,{ok:false,error:'recording_delete_object_failed'},502)}
    }
    const stamp=new Date().toISOString();
