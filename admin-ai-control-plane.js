(() => {
  ...
  function classifyProviderError(status,payload){
    ...
    if(status===403)return {status:'error',httpStatus:status,message:'Cloudflare API 권한이 부족합니다.',detail:serverMessage||'Secret Manager용 API Token의 Account/Zone/Workers 읽기 권한을 확인하세요.'};
