...
function renderSpaces(spaces){
  ...
  for(const item of spaces){const a=document.createElement('a');a.className='space-card';a.href=pathFor(item);a.innerHTML=`<strong>${esc(item.name)}</strong><small>${esc(item.role)}</small>`;list.append(a)}
}
async function renderWorkspace(){
  if(!routeMatch){show('workspaceView',false);show('spaceIndex',true);return;}
  const [,slug]=routeMatch;show('spaceIndex',false);show('workspaceView',true);status('공간 권한을 확인하고 있습니다.');
  try{
    const {space}=await api(`/spaces/resolve?slug=${encodeURIComponent(slug)}`);
    $('workspaceType').textContent='WORKSPACE';$('workspaceName').textContent=space.name;$('workspaceMeta').textContent=space.role;
