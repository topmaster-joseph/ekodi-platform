...
async function renderPublicStorefront(slug){
  ...
  $('publicAddress').textContent=address;$('publicPhone').textContent=phone;$('publicHours').textContent=hours.display;$('publicHoursNote').textContent=hours.note;$('heroPhone').href=phoneHref;$('phoneLink').href=phoneHref;$('stripPhone').href=phoneHref;$('mobilePhone').href=phoneHref;
  const mapHref='https://map.naver.com/p/search/'+encodeURIComponent(address+' 요거트퍼플 목포대점');$('mapLink').href=mapHref;$('mobileMap').href=mapHref;$('mobileMap').target='_blank';$('mobileMap').rel='noopener';
...
function renderStoreDashboard(workspace){
  ...
  renderStorePageOverview(workspace);renderAgentHints(workspace);renderServiceActions(workspace.slug,workspace.role);
