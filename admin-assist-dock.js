(() => {
  ...
  function renderAi(){
    const chat=root?.querySelector('#ekodiAssistChat');if(!chat)return;const composer=root.querySelector('#ekodiAssistComposer');if(composer)composer.hidden=false;chat.replaceChildren();const session=activeSession();const title=root.querySelector('#ekodiAssistTitle');if(title)title.textContent=session?.title||'에코디와 대화하기';const commandHome=document.body.classList.contains('admin-command-home');const empty=!session?.messages?.length;chat.dataset.emptyChat=empty?'true':'false';
    if(empty&&!commandHome){
    ...
    if(!empty||!commandHome)renderAiActions(chat,false);scrollChat();
  }
