...
async function ensureSchema(db) {
  ...
  const seed = db.prepare(`INSERT OR IGNORE INTO customer_tenants
    (slug, name, domain, status, created_at) VALUES (?, ?, ?, 'active', ?)`);
...
async function writeCustomerAudit(db, tenantId, userId, action, resource, detail = '') {
  await db.prepare(`INSERT INTO customer_audit_logs
    (tenant_id, user_id, action, resource, detail, created_at)
    VALUES (?, ?, ?, ?, ?, ?)`)
...
async function issueCustomerSession(db, userId, tenantId) {
  ...
  await db.prepare(`INSERT INTO customer_sessions
    (token_hash, user_id, tenant_id, expires_at, created_at, last_seen_at)
    VALUES (?, ?, ?, ?, ?, ?)`)
...
async function customerSession(request, db) {
  ...
  const row = await db.prepare(`SELECT
      ...
      customer_users.display_name,
      customer_users.status AS user_status,
      customer_tenants.id AS tenant_id,
      customer_tenants.slug,
      customer_tenants.name AS tenant_name,
      customer_tenants.domain,
      customer_tenants.status AS tenant_status,
      customer_memberships.role,
...
async function enforceLoginRateLimit(request, db, tenantId) {
  ...
  const result = await db.prepare(`SELECT COUNT(*) AS count FROM customer_login_attempts
    WHERE tenant_id = ? AND ip_hash = ? AND attempted_at > ?`)
    .bind(tenantId, ipHash, cutoff).first();
  return { blocked: Number(result.count) >= 8, ipHash, cutoff };
}
...
async function handlePublic(request, env, path) {
  ...
  if (request.method === 'POST' && path === '/api/customer/login') {
    ...
    if (!slug || !validEmail(email) || typeof body?.password !== 'string') {
      return customerJson({ error: '고객사, 이메일, 비밀번호를 확인해 주세요.' }, 400, request, env);
    }
    ...
    const user = await db.prepare(`SELECT customer_users.*, customer_memberships.role, customer_memberships.status AS membership_status
      FROM customer_users
      JOIN customer_memberships ON customer_memberships.user_id = customer_users.id
      WHERE customer_users.email = ? AND customer_memberships.tenant_id = ?`)
    ...
    return customerJson({ ok: true, email: user.email, displayName: user.display_name || '', role: user.role, tenant: { slug: tenant.slug, name: tenant.name, domain: tenant.domain }, ...session }, 200, request, env);
  }
  ...
  if (request.method === 'POST' && path === '/api/customer/accept-invite') {
    ...
    if (!/^[a-f0-9]{64}$/i.test(rawToken) || typeof password !== 'string' || password.length < 12) {
      return customerJson({ error: '초대 링크와 12자 이상의 비밀번호를 확인해 주세요.' }, 400, request, env);
    }
    const tokenHash = await sha256(rawToken);
    const invite = await db.prepare(`SELECT customer_invites.*, customer_tenants.slug, customer_tenants.name AS tenant_name,
        customer_tenants.domain, customer_tenants.status AS tenant_status
      FROM customer_invites JOIN customer_tenants ON customer_tenants.id = customer_invites.tenant_id
      WHERE customer_invites.token_hash = ?`)
      .bind(tokenHash).first();
    const now = new Date().toISOString();
    if (!invite || invite.revoked_at || invite.accepted_at || invite.expires_at <= now || invite.tenant_status !== 'active') {
      return customerJson({ error: '초대가 만료되었거나 사용할 수 없습니다.' }, 410, request, env);
    }
    ...
    if (user) {
      ...
      if (!secureEqual(digest, user.password_hash)) {
        return customerJson({ error: '이미 등록된 이메일입니다. 기존 고객 비밀번호를 입력해 주세요.' }, 401, request, env);
      }
    } else {
      ...
      const result = await db.prepare(`INSERT INTO customer_users
        ...
        VALUES (?, ?, ?, ?, ?, 'active', ?)`)
        .bind(invite.email, displayName, digest, salt, ITERATIONS, now).run();
    ...
    return customerJson({ ok: true, email: invite.email, displayName: user.display_name || displayName, role: invite.role, tenant: { slug: invite.slug, name: invite.tenant_name, domain: invite.domain }, ...session }, 201, request, env);
  }
...
async function handleAdmin(request, env, path) {
  ...
  if (request.method === 'GET' && path === '/api/customers/tenants') {
    const tenants = await db.prepare(`SELECT t.id, t.slug, t.name, t.domain, t.status,
      ...
      LEFT JOIN customer_memberships m ON m.tenant_id = t.id
      LEFT JOIN customer_invites i ON i.tenant_id = t.id
      GROUP BY t.id ORDER BY t.name`)
      .bind(new Date().toISOString()).all();
    return customerJson({ tenants: tenants.results.map(row => ({
      slug: row.slug, name: row.name, domain: row.domain, status: row.status,
      activeUsers: Number(row.active_users || 0), pendingInvites: Number(row.pending_invites || 0),
    })) }, 200, request, env);
  }
  ...
  if (usersMatch && request.method === 'GET') {
    ...
    return customerJson({ tenant: { slug: tenant.slug, name: tenant.name, domain: tenant.domain }, users: users.results.map(row => ({
      ...
      status: row.membership_status, userStatus: row.user_status,
  ...
  if (inviteListMatch && request.method === 'GET') {
    ...
    const invites = await db.prepare(`SELECT id, email, role, expires_at, accepted_at, revoked_at, created_at
      FROM customer_invites WHERE tenant_id = ? ORDER BY id DESC LIMIT 100`)
      .bind(tenant.id).all();
    return customerJson({ invites: invites.results.map(row => ({
  ...
  if (inviteListMatch && request.method === 'POST') {
    ...
    await db.prepare(`UPDATE customer_invites SET revoked_at = ?
      WHERE tenant_id = ? AND email = ? AND accepted_at IS NULL AND revoked_at IS NULL`)
      .bind(now.toISOString(), tenant.id, email).run();
    const result = await db.prepare(`INSERT INTO customer_invites
      (tenant_id, email, role, token_hash, expires_at, created_by, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
    ...
    return customerJson({ ok: true, invite: { id: result.meta.last_row_id, email, role, tenant: tenant.slug, inviteUrl, expiresAt: expiresAt.toISOString() } }, 201, request, env);
  }
...
export async function handleCustomerAuth(request, env) {
  ...
  if (path.startsWith('/api/customers/')) {
    return (await handleAdmin(request, env, path)) || customerJson({ error: 'Customer Admin API endpoint not found' }, 404, request, env);
  }
  if (path.startsWith('/api/customer/')) {
    return (await handlePublic(request, env, path)) || customerJson({ error: 'Customer API endpoint not found' }, 404, request, env);
  }
  return null;
}
