(() => {
  ...
  const EVIDENCE = Object.freeze({
    ...
    targets: [
      { name: 'EKODI', path: '/', rps: 326.64, p95: 83.9, error: 0 },
      { name: 'Admin', path: '/admin', rps: 396.94, p95: 66.8, error: 0 },
      { name: 'API', path: '/api/health', rps: 620.26, p95: 43.4, error: 0 },
      { name: 'Biz', path: '/ekodibiz', rps: 690.79, p95: 40.7, error: 0 },
      { name: 'Marketing', path: '/marketing', rps: 204.40, p95: 98.8, error: 0 },
      { name: 'Church', path: '/ekodichurch', rps: 403.55, p95: 61.9, error: 0 },
      { name: 'Lab', path: '/ekodilab', rps: 471.48, p95: 57.2, error: 0 },
    ],
  });
  ...
  panel.innerHTML = `
    ...
    <div class="health-capacity-table-wrap">
      <table class="health-capacity-table">
        ...
        <tbody>
          ${EVIDENCE.targets.map(row => `<tr><td><strong>${row.name}</strong><small>ekodi.kr${row.path === '/' ? '' : row.path}</small></td><td>${EVIDENCE.maxVerifiedConcurrency}</td><td>${format.format(row.rps)} req/s</td><td>${format.format(row.p95)} ms</td><td>${row.error.toFixed(2)}%</td></tr>`).join('')}
        </tbody>
      </table>
    </div>
