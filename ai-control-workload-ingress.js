...
export function normalizeWorkloadEvent(input={},receivedAt=new Date().toISOString()){
  const source=object(input.source);const actor=object(input.actor);const subject=object(input.subject);const payload=object(input.payload);
  return Object.freeze({
    ...
    receivedAt,
    workspaceId:clean(input.workspace_id||input.workspaceId,120),
    source:Object.freeze({serviceId:clean(source.service_id||source.serviceId,120).toLowerCase(),adapterId:clean(source.adapter_id||source.adapterId,120)}),
...
export function validateWorkloadEvent(event={}){
  if(!event.eventId||!event.eventType||!event.workspaceId||!event.source?.serviceId||!event.subject?.type||!event.subject?.id)return Object.freeze({ok:false,code:'workload_fields_required'});
...
async function accept(request,env){
  ...
  await env.DB.prepare(`INSERT INTO ai_control_workload_events
    (event_id,event_type,event_version,workspace_id,source_service_id,source_adapter_id,subject_type,subject_id,correlation_id,payload_json,plan_json,state,accepted_by,occurred_at,received_at)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(event.eventId,event.eventType,event.eventVersion,event.workspaceId,event.source.serviceId,event.source.adapterId,event.subject.type,event.subject.id,event.correlationId,JSON.stringify(event.payload),JSON.stringify(plan),'planned',`service:${principal}`,event.occurredAt,event.receivedAt).run();
...
export function getWorkloadIngressContract(){return Object.freeze({version:'1.0.0',eventVersion:1,path:`${PREFIX}/events`,authentication:'independent_service_caller',credentialPolicy:'server_only_secret_binding',supportedEvents:Object.freeze(Object.keys(WORKLOADS)),credentialsInPayload:false});}
