(() => {
  ...
  async function validateSession() {
    ...
    // Static shell is safe to reveal immediately. Every sensitive API remains server-authorized.
    // This removes a network round-trip from perceived login time while the session validates.
    ...
    try {
      ...
      // Network trouble must not freeze the shell. Keep static navigation usable and let
      // protected feature requests report their own state when the administrator opens them.
      if (token()) {
  ...
  function installPerfDiagnostics() {
    ...
    try {
      ...
      window.addEventListener('ekodi-admin-ready', () => {
        queueMicrotask(() => console.info('[EKODI perf]', window.__EKODI_PERF__));
      }, { once: true });
    } catch {}
  }
