...
function renderMissionShareHtml(data){
  ...
  const showName=policy.name!==false,showSubmitted=policy.submitted_at!==false,showStatus=policy.status!==false,showParty=policy.party_size!==false,showPhone=policy.phone===true,showEmail=policy.email===true;
  const headers=['<th scope="col">연번</th>',showName?'<th scope="col">이름</th>':'',showSubmitted?'<th scope="col">신청일시</th>':'',showPhone?'<th scope="col">전화번호</th>':'',showEmail?'<th scope="col">이메일</th>':'',showStatus?'<th scope="col">상태</th>':'',showParty?'<th scope="col">인원</th>':''].join('');
  const rows=participants.map(item=>'<tr><td>'+htmlText(item?.seq||'')+'</td>'+(showName?'<td>'+htmlText(item?.name||'-')+'</td>':'')+(showSubmitted?'<td>'+htmlText(missionShareDate(item?.submitted_at)||'-')+'</td>':'')+(showPhone?'<td>'+htmlText(item?.phone||'-')+'</td>':'')+(showEmail?'<td>'+htmlText(item?.email||'-')+'</td>':'')+(showStatus?'<td>'+htmlText(MISSION_SHARE_STATUS_LABEL[item?.status]||item?.status||'-')+'</td>':'')+(showParty?'<td>'+htmlText(item?.party_size||1)+'</td>':'')+'</tr>').join('');
  ...
  return '<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>'+htmlText(activity.title||'신청자 명단')+' · 읽기전용</title><link rel="stylesheet" href="/ekodimission/assets/share.css"></head><body><main class="mission-share-page"><p class="mission-share-kicker">EKODI MISSION · READ ONLY SHARE</p><h1>'+htmlText(activity.title||'신청자 명단')+'</h1><p class="mission-share-meta">'+(meta||'행사 신청자 명단')+'</p><div class="mission-share-notice">이 페이지는 읽기전용 공유본입니다. 전화번호·이메일은 관리자가 명시적으로 선택한 경우에만 표시되며 역할·후속관리·내부 메모는 포함하지 않습니다.</div><div class="mission-share-table-wrap"><table class="mission-share-table"><thead><tr>'+headers+'</tr></thead><tbody>'+(rows||'<tr><td class="mission-share-empty" colspan="'+(1+Number(showName)+Number(showSubmitted)+Number(showPhone)+Number(showEmail)+Number(showStatus)+Number(showParty))+'">표시할 신청자가 없습니다.</td></tr>')+'</tbody></table></div><p class="mission-share-footer">공유 만료: '+htmlText(expiry||'관리자 설정 시각')+' · 이 링크는 만료되거나 관리자가 중지하면 더 이상 열리지 않습니다.</p></main></body></html>';
}
