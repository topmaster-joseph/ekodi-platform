(() => {
  ...
  async function resolveImage(url) {
    if (!url) return '';
    try {
      if (new URL(url).hostname !== 'ekodi.kr/api') return url;
    } catch { return ''; }
  ...
  function recordPromotionOutbound(productRowId) {
    ...
    const endpoint='/marketing-connect-api/r/mall/outbound/'+encodeURIComponent(campaign)+'/'+rowId;
  ...
  const GIFT_DIRECT_TERMS=['\uD64D\uC0BC','\uD55C\uC6B0','\uACFC\uC77C','\uACAC\uACFC','\uCC28','\uAFC0','\uC601\uC591\uC81C','\uD654\uC7A5\uD488','\uAF43','\uCEE4\uD53C'];
  ...
  function pickGiftRecommendations(context){const direct=(context.preferences||[]).filter(term=>GIFT_DIRECT_TERMS.includes(term));const pool=direct.length?state.products.filter(product=>direct.some(term=>giftText(product).includes(term))):state.products;if(direct.length&&!pool.length)return[];const scored=pool.map(product=>({product,score:giftScore(product,context)})).filter(item=>item.score>-80).sort((a,b)=>b.score-a.score);if(!scored.length)return[];const result=[];const use=(entry,label)=>{if(!entry||result.some(item=>item.product.id===entry.product.id))return;result.push({...entry,label})};use(scored[0],'가장 적합');use([...scored].sort((a,b)=>giftValueScore(b.product,context)-giftValueScore(a.product,context)).find(entry=>!result.some(item=>item.product.id===entry.product.id)),'가성비');use([...scored].sort((a,b)=>giftSpecialScore(b.product,context)-giftSpecialScore(a.product,context)).find(entry=>!result.some(item=>item.product.id===entry.product.id)),'조금 특별한 선택');for(const entry of scored){if(result.length>=3)break;use(entry,'대안')}return result}
